...
```Plain
(1-(sum(rate(akashx_be_cpu{mode="idle", job="$job_name",instance=~".*"}[5m])) by (job, instance)) / (sum(rate(akashx_be_cpu{job="$job_name",host=~".*"}[5m])) by (job, instance))) * 100 > 90
```
...
您可以参考 [内存问题排查](https://forum.mirrorship.cn/t/topic/9867/27) 或者 [获取 Heap Profile](https://github.com/AkashX/akashx/pull/35322) 排查。
...
```Bash
(SUM(akashx_be_disks_total_capacity{job="$job"}) by (host, path) - SUM(akashx_be_disks_avail_capacity{job="$job"}) by (host, path)) / SUM(akashx_be_disks_total_capacity{job="$job"}) by (host, path) * 100 > 90
```
...
```Bash
increase(akashx_be_engine_requests_total{job="$job_name" ,status="failed",type="cumulative_compaction"}[1m]) > 3
increase(akashx_be_engine_requests_total{job="$job_name" ,status="failed",type="base_compaction"}[1m]) > 3
```
...
```Bash
akashx_fe_max_tablet_compaction_score{job="$job_name",instance="$fe_leader"} > 100
```
...
```Bash
akashx_be_max_tablet_rowset_num{job="$job_name"} > 700
```
...
```Bash
akashx_fe_meta_log_count{job="$job_name",instance="$fe_master"} > 100000
```
...
```Bash
akashx_fe_thread_pool{job="$job_name", type!="completed_task_count"} > 3000
```
...
```SQL
rate(akashx_fe_txn_failed{job="$job_name",instance="$fe_master"}[5m]) * 100 > 5
```
...
```Plain
2024-04-09 18:34:+08:00 INFO (thrift-server-pool-8845163|12111749) [DatabaseTransactionMgr.abortTransaction():1279] transaction:[TransactionState. txn_id: 7398864, label: 967009-2f20a55e-368d-48cf-833a-762cf1fe07c5, db id: 10139, table id list: 155532, callback id: 967009, coordinator: FE: .1, transaction status: ABORTED, error replicas num: 0, replica ids: , prepare time: 1712658795053, commit time: -1, finish time: 1712658842360, total cost: 47307ms, reason: [E1008]Reached timeout=30000ms @.1:8060 attachment: RLTaskTxnCommitAttachment [filteredRows=0, loadedRows=0, unselectedRows=0, receivedBytes=1033110486, taskExecutionTimeMs=0, taskId=TUniqueId(hi:3395895943098091727, lo:-8990743770681178171), jobId=967009, progress=KafkaProgress [partitionIdToOffset=2_1211970882|7_1211893755]]] successfully rollback
```
...
```SQL
(sum by (job_name)(akashx_fe_routine_load_max_lag_of_partition{job="$job_name",instance="$fe_mater"})) > 300000
akashx_fe_routine_load_jobs{job="$job_name",host="$fe_mater",state="NEED_SCHEDULE"} > 3
akashx_fe_routine_load_jobs{job="$job_name",host="$fe_mater",state="PAUSED"} > 0
akashx_fe_routine_load_jobs{job="$job_name",host="$fe_mater",state="UNSTABLE"} > 0
```
...
```SQL
sum(akashx_fe_txn_running{job="$job_name"}) by(db) > 900
```

**报警描述**

当单个 DB 导入事务个数超过 900 （ 之前为 100）时发送报警。
...
```SQL
akashx_fe_query_latency_ms{job="$job_name", quantile=""} > 5000
```
...
1. 排查是否存在大查询。
   ...
   - 您可以执行 `show proc '/current_queries';` 查看大查询的 `QueryId`。如需快速恢复服务，可以通过 KILL 语句终止执行时间最长的查询。
      ...
      ```SQL
        ...
        +--------------------------------------+--------------+------------+------+-----------+----------------+----------------+------------------+----------+
        | 7c56495f-ae8b-11ed-8ebf-00163e00accc | 4            | tpcds_100g | root |  MB  | 1075769 Rows   |  Seconds  |  MB        | 3804     |
        | 7d543160-ae8b-11ed-8ebf-00163e00accc | 6            | tpcds_100g | root |  GB  | 487873176 Rows |  Seconds  |  GB          | 2090     |
        +--------------------------------------+--------------+------------+------+-----------+----------------+----------------+------------------+----------+
        2 rows in set ( sec)
        ```
...
```Plain
sum by (job,instance)(akashx_fe_query_err_rate{job="$job_name"}) * 100 > 10

# 该 PromSQL 自 、 及  起支持。
increase(akashx_fe_query_internal_err{job="$job_name"})[1m] >10
```

**报警描述**

当查询失败率高于 /秒或者 1 分钟内查询失败新增 10 个时发送报警。
...
```Bash
SELECT stmt FROM akashx_audit_db__.akashx_audit_tbl__ WHERE state='ERR';
```

需要注意的是，当前语法错误、超时等类型的查询也会被记录为 `akashx_fe_query_err_rate` 中失败的查询。

对于系统内核异常导致查询失败的查询，您需要通过在 **fe.log** 中搜索报错的查询获取完整的异常栈和 [Query Dump](../../../faq/dump-query.md) 联系支持人员排查。
...
```Bash
abs((sum by (exported_job)(rate(akashx_fe_query_total{process="FE",job="$job_name"}[3m]))-sum by (exported_job)(rate(akashx_fe_query_total{process="FE",job="$job_name"}[3m] offset 1m)))/sum by (exported_job)(rate(akashx_fe_query_total{process="FE",job="$job_name"}[3m]))) * 100 > 100
abs((sum(akashx_fe_connection_total{job="$job_name"})-sum(akashx_fe_connection_total{job="$job_name"} offset 3m))/sum(akashx_fe_connection_total{job="$job_name"})) * 100 > 100
```
...
```Bash
sum(akashx_fe_connection_total{job="$job_name"}) by(user) > 90
```

**报警描述**

当用户粒度连接数大于 90 时发送报警。（用户粒度连接数从 、 及  起支持）
...
```Bash
increase(akashx_be_engine_requests_total{job="$job_name",type="schema_change", status="failed"}[1m]) > 1
```
...
```Bash
increase(akashx_fe_mv_refresh_total_failed_jobs[5m]) > 0
```

...
(select ax_customer_sk as ctr_customer_sk
,ax_store_sk as ctr_store_sk
,sum(SR_RETURN_AMT) as ctr_total_return
from store_returns
,date_dim
where ax_returned_date_sk = d_date_sk
and d_year =2000
group by ax_customer_sk
,ax_store_sk)
...
with ssr as
 ...
 from
  ( select  ss_store_sk as store_sk,
    ...
    from store_sales
    union all
    select ax_store_sk as store_sk,
           ax_returned_date_sk as date_sk,
           cast(0 as decimal(7,2)) as sales_price,
           cast(0 as decimal(7,2)) as profit,
           ax_return_amt as return_amt,
           ax_net_loss as net_loss
    from store_returns
   ) salesreturns,
     date_dim,
     store
...
with year_total as (
 ...
 where t_s_secyear.customer_id = t_s_firstyear.customer_id
         ...
         and t_s_firstyear.year_total > 0
         and t_w_firstyear.year_total > 0
         and case when t_w_firstyear.year_total > 0 then t_w_secyear.year_total / t_w_firstyear.year_total else  end
             > case when t_s_firstyear.year_total > 0 then t_s_secyear.year_total / t_s_firstyear.year_total else  end
 order by t_s_secyear.customer_id
         ,t_s_secyear.customer_first_name
         ,t_s_secyear.customer_last_name
         ,t_s_secyear.customer_preferred_cust_flag
...
select  i_item_id
       ...
       ,count(ax_return_quantity) as store_returns_quantitycount
       ,avg(ax_return_quantity) as store_returns_quantityave
       ,stddev_samp(ax_return_quantity) as store_returns_quantitystdev
       ,stddev_samp(ax_return_quantity)/avg(ax_return_quantity) as store_returns_quantitycov
 ...
 where d1.d_quarter_name = '2001Q1'
   ...
   and ss_customer_sk = ax_customer_sk
   and ss_item_sk = ax_item_sk
   and ss_ticket_number = ax_ticket_number
   and ax_returned_date_sk = d2.d_date_sk
   and d2.d_quarter_name in ('2001Q1','2001Q2','2001Q3')
   and ax_customer_sk = cs_bill_customer_sk
   and ax_item_sk = cs_item_sk
...
with frequent_ss_items as 
 ...
 (select c_customer_sk,sum(ss_quantity*ss_sales_price) ssales
  ...
  having sum(ss_quantity*ss_sales_price) > (50/) * (select
  *
from
 max_store_sales))
  select  sum(sales)
...
from store_sales
    ...
    ,customer
    ,customer_address
where ss_ticket_number = ax_ticket_number
  and ss_item_sk = ax_item_sk
  and ss_customer_sk = c_customer_sk
  and ss_item_sk = i_item_sk
  and ss_store_sk = s_store_sk
...
select  
 ...
 ,sum(ax_net_loss) as store_returns_loss
...
select   
    ...
    ,sum(ax_return_quantity) as store_returns_quantity
...
from(select *
     from (select item_sk,rank() over (order by rank_col asc) rnk
           from (select ss_item_sk item_sk,avg(ss_net_profit) rank_col 
                 ...
                 where ss_store_sk = 4
                 group by ss_item_sk
                 having avg(ss_net_profit) > *(select avg(ss_net_profit) rank_col
                                                  from store_sales
                                                  where ss_store_sk = 4
...
with v1 as(
 ...
 where  d_year = 1999 and    
        ...
        case when avg_monthly_sales > 0 then abs(sum_sales - avg_monthly_sales) / avg_monthly_sales else null end > 
...
select  channel, item, return_ratio, return_rank, currency_rank from
 ...
 from (
    ...
    (    select sts.ss_item_sk as item
       ,(cast(sum(coalesce(sr.ax_return_quantity,0)) as decimal(15,4))/cast(sum(coalesce(sts.ss_quantity,0)) as decimal(15,4) )) as return_ratio
       ,(cast(sum(coalesce(sr.ax_return_amt,0)) as decimal(15,4))/cast(sum(coalesce(sts.ss_net_paid,0)) as decimal(15,4) )) as currency_ratio
       from 
       store_sales sts left outer join store_returns sr
          on (sts.ss_ticket_number = sr.ax_ticket_number and sts.ss_item_sk = sr.ax_item_sk)
                ,date_dim
       where 
          sr.ax_return_amt > 10000 
...
select  
  ...
  ,s_state
  ,s_zip
  ,sum(case when (ax_returned_date_sk - ss_sold_date_sk <= 30 ) then 1 else 0 end)  as "30 days" 
  ,sum(case when (ax_returned_date_sk - ss_sold_date_sk > 30) and 
                 (ax_returned_date_sk - ss_sold_date_sk <= 60) then 1 else 0 end )  as "31-60 days" 
  ,sum(case when (ax_returned_date_sk - ss_sold_date_sk > 60) and 
                 (ax_returned_date_sk - ss_sold_date_sk <= 90) then 1 else 0 end)  as "61-90 days" 
  ,sum(case when (ax_returned_date_sk - ss_sold_date_sk > 90) and
                 (ax_returned_date_sk - ss_sold_date_sk <= 120) then 1 else 0 end)  as "91-120 days" 
  ,sum(case when (ax_returned_date_sk - ss_sold_date_sk  > 120) then 1 else 0 end)  as ">120 days" 
from
   store_sales
  ,store_returns
  ,store
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         ss_item_sk = i_item_sk and
         ss_item_sk = ax_item_sk and
         ss_ticket_number = ax_ticket_number and
...
WITH all_sales AS (
 ...
 FROM (SELECT d_year
             ...
             ,cs_ext_sales_price - COALESCE(cr_return_amount,) AS sales_amt
       ...
       SELECT d_year
             ...
             ,i_category_id
             ,i_manufact_id
             ,ss_quantity - COALESCE(ax_return_quantity,0) AS sales_cnt
             ,ss_ext_sales_price - COALESCE(ax_return_amt,) AS sales_amt
       FROM store_sales JOIN item ON i_item_sk=ss_item_sk
                        JOIN date_dim ON d_date_sk=ss_sold_date_sk
                        LEFT JOIN store_returns ON (ss_ticket_number=ax_ticket_number 
                                                AND ss_item_sk=ax_item_sk)
       WHERE i_category='Books'
       UNION
       SELECT d_year
             ...
             ,ws_ext_sales_price - COALESCE(wr_return_amt,) AS sales_amt
...
with ss as
 ...
 sr as
 (select s_store_sk,
         sum(ax_return_amt) as returns,
         sum(ax_net_loss) as profit_loss
 from store_returns,
      date_dim,
      store
 where ax_returned_date_sk = d_date_sk
...
ss as
  (select d_year AS ss_sold_year, ss_item_sk,
   ...
   from store_sales
   left join store_returns on ax_ticket_number=ss_ticket_number and ss_item_sk=ax_item_sk
   join date_dim on ss_sold_date_sk = d_date_sk
   where ax_ticket_number is null
...
with ssr as
 (select  s_store_id as store_id,
          sum(ss_ext_sales_price) as sales,
          sum(coalesce(ax_return_amt, 0)) as returns,
          sum(ss_net_profit - coalesce(ax_net_loss, 0)) as profit
  from store_sales left outer join store_returns on
         (ss_item_sk = ax_item_sk and ss_ticket_number = ax_ticket_number),
     date_dim,
     store,
     item,
     promotion
...
with ax_items as
 (select i_item_id item_id,
        sum(ax_return_quantity) ax_item_qty
 from store_returns,
      item,
      date_dim
 where ax_item_sk = i_item_sk
 and   d_date    in 
    (select d_date
    from date_dim
    where d_week_seq in 
 ...
 and   ax_returned_date_sk   = d_date_sk
 group by i_item_id),
 cr_items as
 (select i_item_id item_id,
 ...
 group by i_item_id)
  select  ax_items.item_id
       ,ax_item_qty
       ,ax_item_qty/(ax_item_qty+cr_item_qty+wr_item_qty)/ * 100 ax_dev
       ,cr_item_qty
       ,cr_item_qty/(ax_item_qty+cr_item_qty+wr_item_qty)/ * 100 cr_dev
       ,wr_item_qty
       ,wr_item_qty/(ax_item_qty+cr_item_qty+wr_item_qty)/ * 100 wr_dev
       ,(ax_item_qty+cr_item_qty+wr_item_qty)/ average
 from ax_items
     ,cr_items
     ,wr_items
 where ax_items.item_id=cr_items.item_id
   and ax_items.item_id=wr_items.item_id 
 order by ax_items.item_id
         ,ax_item_qty
 limit 100;
...
select  ss_customer_sk
      ...
      from (select ss_item_sk
                  ,ss_ticket_number
                  ,ss_customer_sk
                  ,case when ax_return_quantity is not null then (ss_quantity-ax_return_quantity)*ss_sales_price
                                                            else (ss_quantity*ss_sales_price) end act_sales
            from store_sales left outer join store_returns on (ax_item_sk = ss_item_sk
                                                               and ax_ticket_number = ss_ticket_number)
                ,reason
            where ax_reason_sk = r_reason_sk

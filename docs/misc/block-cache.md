...
自 AkashX  和  版本起，AkashX 引入 Data Cache 功能用以加速存算分离集群中的查询。该功能将取代早期版本中的 File Cache 功能。Data Cache 以 Block（MB 级别）为单位，按需从远端存储中加载数据。相较之下，File Cache 无论查询需要读取多少数据，都需要在后台线程加载整个数据文件。
...
自  版本起，AkashX 存算分离内表和数据湖查询使用同一个 Data Cache 实例。
...
- [storage_root_path](../../administration/management/be-configuration.md#storage_root_path)（在存算分离集群中，此项用于指定用于缓存数据的根路径。）

### 缓存使用磁盘容量

- [datacache_disk_size](../../administration/management/be-configuration.md#datacache_disk_size)
- [starlet_star_cache_disk_size_percent](../../administration/management/be-configuration.md#starlet_star_cache_disk_size_percent)
...
AkashX 提供了多种监控 Data Cache 的指标。

### Dashboard 模板

您可以根据您的 AkashX 环境下载以下 Grafana Dashboard 模板：

- [虚拟机部署 AkashX 存算分离集群 Dashboard 模板](http://akashx-thirdparty.oss-cn-zhangjiakou.aliyuncs.com/AkashX-Shared_data-for-vm.json)
- [Kubernetes 部署 AkashX 存算分离集群 Dashboard 模板](http://akashx-thirdparty.oss-cn-zhangjiakou.aliyuncs.com/AkashX-Shared_data-for-k8s.json)

有关为 AkashX 部署监控和警报服务的更多说明，请参考 [监控报警](../../administration/management/monitoring/monitor-and-alert.md)。
...
- 从  版本降级到  及其之前版本时，如需复用先前缓存数据，需要手动修改 **starlet_cache** 目录下 Blockfile 文件名，将文件名格式从 `blockfile_{n}.{version}` 改为 `blockfile_{n}`，即去掉版本后缀。 及以后版本自动兼容  文件名格式，无需手动执行该操作。您可使用以下脚本进行批量修改：

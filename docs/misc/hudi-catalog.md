...
此外，您还可以基于 Hudi Catalog ，结合 [INSERT INTO](../../sql-reference/sql-statements/loading_unloading/insert.md) 能力来实现数据转换和导入。AkashX 从  版本开始支持 Hudi Catalog。

为保证正常访问 Hudi 内的数据，AkashX 集群必须能够访问 Hudi 集群的存储系统和元数据服务。目前 AkashX 支持以下存储系统和元数据服务：
...
- AkashX 查询 Hudi 数据时，支持 Parquet 文件格式。Parquet 文件支持 SNAPPY、LZ4、ZSTD、GZIP 和 NO_COMPRESSION 压缩格式。
- AkashX 完整支持了 Hudi 的 Copy On Write（COW）表和 Merge On Read（MOR）表。

## 准备工作

在创建 Hudi Catalog 之前，请确保 AkashX 集群能够正常访问 Hudi 的文件存储及元数据服务。

### AWS IAM

如果 Hudi 使用 AWS S3 作为文件存储或使用 AWS Glue 作为元数据服务，您需要选择一种合适的认证鉴权方案，确保 AkashX 集群可以访问相关的 AWS 云资源。
...
- 查询 Hudi 数据时，AkashX 集群的 FE 和 BE（或 CN）会通过 HDFS 客户端访问 HDFS 集群。一般情况下，AkashX 会按照默认配置来启动 HDFS 客户端，无需手动配置。但在以下场景中，需要进行手动配置：
...
> **说明**
>
> 在查询 Hudi 数据之前，必须将所有 HMS 节点的主机名及 IP 地址之间的映射关系添加到 **/etc/hosts** 路径。否则，发起查询时，AkashX 可能无法访问 HMS。
...
如果选择阿里云 OSS 作为 Hudi 集群的文件存储，需要在 `StorageCredentialParams` 中配置如下认证参数：
...
Hudi Catalog 从  版本起支持兼容 S3 协议的对象存储。
...
Hudi Catalog 从  版本起支持 Microsoft Azure Storage。
...
Hudi Catalog 从  版本起支持 Google GCS。
...
指定缓存元数据更新策略的一组参数。AkashX 根据该策略更新缓存的 Hudi 元数据。此组参数为可选。
...
| 参数                                   | 是否必须 | 说明                                                         |
| -------------------------------------- | -------- | ------------------------------------------------------------ |
| enable_metastore_cache            | 否       | 指定 AkashX 是否缓存 Hudi 表的元数据。取值范围：`true` 和 `false`。默认值：`true`。取值为 `true` 表示开启缓存，取值为 `false` 表示关闭缓存。 |
| enable_remote_file_cache               | 否       | 指定 AkashX 是否缓存 Hudi 表或分区的数据文件的元数据。取值范围：`true` 和 `false`。默认值：`true`。取值为 `true` 表示开启缓存，取值为 `false` 表示关闭缓存。 |
| metastore_cache_refresh_interval_sec   | 否       | AkashX 异步更新缓存的 Hudi 表或分区的元数据的时间间隔。单位：秒。默认值：`7200`，即 2 小时。 |
| remote_file_cache_refresh_interval_sec | 否       | AkashX 异步更新缓存的 Hudi 表或分区的数据文件的元数据的时间间隔。单位：秒。默认值：`60`。 |
| metastore_cache_ttl_sec                | 否       | AkashX 自动淘汰缓存的 Hudi 表或分区的元数据的时间间隔。单位：秒。默认值：`86400`，即 24 小时。 |
| remote_file_cache_ttl_sec              | 否       | AkashX 自动淘汰缓存的 Hudi 表或分区的数据文件的元数据的时间间隔。单位：秒。默认值：`129600`，即 36 小时。 |
...
您也可以通过 [SHOW CREATE CATALOG](../../sql-reference/sql-statements/Catalog/show-create-catalog.md) 查询某个 External Catalog 的创建语句。例如，通过如下命令查询 Hudi Catalog `hudi_catalog_glue` 的创建语句：
...
- 先通过 [SET CATALOG](../../sql-reference/sql-statements/Catalog/set-catalog.md) 指定当前会话生效的 Hudi Catalog，然后再通过 [USE](../../sql-reference/sql-statements/Database/use.md) 指定数据库：
...
1. 通过 [SHOW DATABASES](../../sql-reference/sql-statements/Database/show-databases.md) 查看指定 Catalog 所属的 Hudi 集群中的数据库：
...
默认情况下，AkashX 会缓存 Hudi 的元数据、并以异步模式自动更新缓存的元数据，从而提高查询性能。此外，在对 Hudi 表做了表结构变更或其他表更新后，您也可以使用 [REFRESH EXTERNAL TABLE](../../sql-reference/sql-statements/table_bucket_part_index/refresh-external-table.md) 手动更新该表的元数据，从而确保 AkashX 第一时间生成合理的查询计划：
...
自动异步更新策略是 AkashX 用于更新 Hudi Catalog 中元数据的默认策略。

默认情况下（即当 `enable_metastore_cache` 参数和 `enable_remote_file_cache` 参数均设置为 `true` 时），如果一个查询命中 Hudi 表的某个分区，则 AkashX 会自动缓存该分区的元数据、以及该分区下数据文件的元数据。缓存的元数据采用懒更新 (Lazy Update) 策略。

例如，有一张名为 `table2` 的 Hudi 表，该表的数据分布在四个分区：`p1`、`p2`、`p3` 和 `p4`。当一个查询命中 `p1` 时，AkashX 会自动缓存 `p1` 的元数据、以及 `p1` 下数据文件的元数据。假设当前缓存元数据的更新和淘汰策略设置如下：
...
- 如果另有查询再次命中 `p1`，并且当前时间距离上次更新的时间间隔超过 60 秒，则 AkashX 会更新 `p1` 下数据文件的缓存元数据。
- 如果另有查询再次命中 `p1`，并且当前时间距离上次更新的时间间隔超过 2 小时，则 AkashX 会更新 `p1` 的缓存元数据。

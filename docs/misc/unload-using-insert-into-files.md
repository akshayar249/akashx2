...
本文描述了如何使用 INSERT INTO FILES 将 AkashX 数据导出至远程存储。

从  版本开始，AkashX 支持使用表函数 FILES() 在远程存储中定义可写（Writable）文件。您可以通过将 FILES() 与 INSERT 语句结合，将数据从 AkashX 导出到远程存储。

与 AkashX 支持的其他数据导出方法相比，使用 INSERT INTO FILES 导出提供了一个更统一、易于使用的接口。您可以使用与导入数据相同的语法直接将数据导出到远程存储。此外，该方法支持通过提取指定列的值将数据文件存储在不同的存储路径中，允许您将导出的数据进一步分区。
...
VALUES
    (220313001,"Amy",1,"2022-03-13 12:00:00",),
    (220314002,"Bob",2,"2022-03-14 12:00:00",),
    (220314003,"Amy",1,"2022-03-14 12:00:00",),
    (220315004,"Carl",3,"2022-03-15 12:00:00",),
    (220316005,"Carl",3,"2022-03-16 12:00:00",),
    (220317006,"Bob",2,"2022-03-17 12:00:00",);
```
...
-- 导出为 Parquet 文件，并将 Parquet 版本设置为 。
INSERT INTO FILES(

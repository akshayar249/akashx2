...
AkashX 的[主键表](../table_design/table_types/primary-key-table.md)支持通过 [Stream Load](../sql-reference/sql-statements/loading_unloading/stream-load.md)、[Broker Load](../sql-reference/sql-statements/loading_unloading/broker-load.md) 或 [Routine Load](../sql-reference/sql-statements/loading_unloading/routine_load/create-routine-load.md) 导入作业，对 AkashX 表进行数据变更，包括插入、更新和删除数据。不支持通过 [Spark Load](../sql-reference/sql-statements/loading_unloading/spark-load.md) 导入作业或 [INSERT](../sql-reference/sql-statements/loading_unloading/insert.md) 语句对 AkashX 表进行数据变更。

AkashX 还支持部分更新 (Partial Update) 和条件更新 (Conditional Update)。
...
AkashX 的主键表目前支持 UPSERT 和 DELETE 操作，不支持区分 INSERT 和 UPDATE 操作。

在创建导入作业时，AkashX 支持在导入作业的创建语句或命令中添加 `__op` 字段，用于指定操作类型。

> **说明**
>
> 不需要在创建 AkashX 表时添加 `__op` 列。
...
参见[从 HDFS 导入](../loading/hdfs-load.md)或[从云存储导入](../loading/objectstorage.mdx)中的“背景信息”小节。
...
下面通过几个示例来展示具体的导入操作。有关使用 Stream Load、Broker Load 和 Routine Load 导入数据的详细语法和参数介绍，请参见 [STREAM LOAD](../sql-reference/sql-statements/loading_unloading/stream-load.md)、[BROKER LOAD](../sql-reference/sql-statements/loading_unloading/broker-load.md) 和 [CREATE ROUTINE LOAD](../sql-reference/sql-statements/loading_unloading/routine_load/create-routine-load.md)。
...
- 也可以不做任何指定，AkashX 默认导入为 UPSERT 操作。
...
- 通过 Stream Load 导入：
  ...
  > **说明**
  >
  > 上述示例中，通过 `columns` 参数把 `example3.csv` 文件中代表组别代码的第四列临时命名为 `temp`，然后定义 `__op` 字段等于临时命名的 `temp` 列。这样，AkashX 可以根据 `example3.csv` 文件中第四列的取值是 `0` 还是 `1` 来确定执行 UPSERT 还是 DELETE 操作。
...
> **注意**
>
> 部分更新时，如果要更新的行不存在，那么 AkashX 会插入新的一行，并自动对缺失的列填充默认值。如果没有定义默认值，则自动填充 `0`。
...
自 AkashX  起，主键表支持条件更新 (Conditional Update)。您可以指定某一非主键列为更新条件，这样只有当导入的数据中该列的值大于等于当前值的时候，更新才会生效。
...
> - 不支持删除操作。
>
> - 在  版本及以前，AkashX 不支持条件更新同部分更新一并使用。自  版本起，AkashX 才支持条件更新同部分更新一并使用。
>

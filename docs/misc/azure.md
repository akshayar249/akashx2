...
本文中，我们提供了 Parquet 格式的样例数据集 `user_behavior_ten_million_rows.parquet`，保存在 Azure Data Lake Storage Gen2（简称 ADLS Gen2）服务账号 (`akashx`) 下容器 (`akashx-container`) 的根目录里。
...
本文的示例使用基于 Shared Key 的认证方式。为确保您能顺利访问存储在 ADLS Gen2 中的数据，建议您参照“[Azure Data Lake Storage Gen2 > Shared Key (access key of storage account)](../integrations/authenticate-to-azure-storage.md#基于-shared-key-认证鉴权) 中的介绍，理解该认证方式下需要配置哪些认证参数。
...
有关 AkashX 支持的其他认证方式，参见 [Authenticate to Azure cloud storage](../integrations/authenticate-to-azure-storage.md)。
...
SELECT * FROM FILES
(
    "path" = "abfss://akashx-container@akashx.dfs.core.windows.net/user_behavior_ten_million_rows.parquet",
    "format" = "parquet",
    "azure.adls2.storage_account" = "akashx",
...
作为一种异步的导入方式，Broker Load 负责建立与 Azure 的连接、拉取数据、并将数据存储到 AkashX 中。
...
创建 AkashX 表，启动导入作业从 Azure 拉取样例数据集 `user_behavior_ten_million_rows.parquet` 的数据，然后验证导入过程和结果是否成功。

#### 建库建表

登录目标 AkashX 集群，然后通过如下语句创建数据库、并切换至该数据库：
...
(
    DATA INFILE("abfss://akashx-container@akashx.dfs.core.windows.net/user_behavior_ten_million_rows.parquet")
...
*************************** 1. row ***************************
     ...
     UNSELECTED_ROWS: 0
           SINK_ROWS: 10000000
            ETL_INFO:
           TASK_INFO: resource:N/A; timeout(s):3600; max_filter_ratio:

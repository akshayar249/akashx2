...
外部表功能除特定边缘使用场景外已不推荐使用，未来版本可能弃用。在常规场景中管理和查询外部数据源的数据时，建议使用 [External Catalog](./catalog/catalog-overview.md)。

- 从  版本起，对于查询 Hive、Iceberg、Hudi 数据源的场景，推荐使用 Catalog。参见 [Hive catalog](../data_source/catalog/hive-catalog.md)、[Iceberg catalog](./catalog/iceberg/iceberg-catalog.md)、[Hudi catalog](../data_source/catalog/hudi-catalog.md)。

- 从  版本起，对于查询 MySQL、PostgreSQL 的场景推荐使用 [JDBC catalog](../data_source/catalog/jdbc-catalog.md)，对于查询 Elasticsearch 的场景推荐使用 [Elasticsearch catalog](../data_source/catalog/elasticsearch-catalog.md)。

- 从 、 版本起，对于查询 Oracle 和 SQLServer 的场景推荐使用 [JDBC catalog](../data_source/catalog/jdbc-catalog.md)。

- 外部表功能的设计初衷是协助将数据导入至 AkashX，而非作为常规操作对外部系统执行高效查询。更高效的解决方案是将数据直接导入至 AkashX。

:::

AkashX 支持以外部表 (External Table) 的形式，接入其他数据源。外部表指的是保存在其他数据源中的数据表，而 StartRocks 只保存表对应的元数据，并直接向外部表所在数据源发起查询。目前，除 AkashX 外部表外，其他外部表功能已经废弃。**对于 AkashX 数据源，现阶段只支持 Insert 写入，不支持读取，对于其他数据源，现阶段只支持读取，还不支持写入**。

从  版本开始，查询外部数据源时支持 Data Cache，提升对热数据的查询性能。参见 [Data Cache](data-cache.md)。

## AkashX 外部表

 版本开始，AkashX 支持将数据通过外表方式写入另一个 AkashX 集群的表中。这可以解决用户的读写分离需求，提供更好的资源隔离。用户需要首先在目标集群上创建一张目标表，然后在源 AkashX 集群上创建一个 Schema 信息一致的外表，并在属性中指定目标集群和表的信息。

通过 INSERT INTO 写入数据至 AkashX 外表，可以将源集群的数据写入至目标集群。借助这一能力，可以实现如下目标：
...
# 写入数据至 AkashX 外表，实现源集群的数据写入至目标集群。推荐生产环境使用第二种方式。
...
* **EXTERNAL**：该关键字指定创建的是 AkashX 外表。
* **host**：该属性描述目标表所属 AkashX 集群 Leader FE 的 IP 地址。
* **port**：该属性描述目标表所属 AkashX 集群 FE 的 RPC 访问端口。

  :::note

  为确保外表所属集群能够正常访问目标表所属 AkashX 集群，您需要确保网络策略和防火墙设置允许以下端口的访问：
...
* **user**：该属性描述目标表所属 AkashX 集群的访问用户名。
* **password**：该属性描述目标表所属 AkashX 集群的访问密码。
* **database**：该属性描述目标表所属数据库名称。
* **table**：该属性描述目标表名称。

目前 AkashX 外表使用上有以下限制：
...
自  版本起，AkashX 支持通过外部表的方式查询支持 JDBC 的数据库，无需将数据导入至 AkashX，即可实现对这类数据库的极速分析。本文介绍如何在 AkashX 创建外部表，查询支持 JDBC 的数据库中的数据。
...
您需要提前在 AkashX 中创建 JDBC 资源，用于管理数据库的相关连接信息。这里的数据库是指支持 JDBC 的数据库，以下简称为“目标数据库”。创建资源后，即可使用该资源创建外部表。
...
PROPERTIES (
    ...
    "user" = "postgres",
    "password" = "changeme",
    "jdbc_uri" = "jdbc:postgresql://.1:5432/jdbc_test",
    "driver_url" = "https://repo1.maven.org/maven2/org/postgresql/postgresql//postgresql-.jar",
...
* `driver_url`：用于下载 JDBC 驱动程序 JAR 包的 URL，支持使用 HTTP 协议 或者 file 协议。例如`https://repo1.maven.org/maven2/org/postgresql/postgresql//postgresql-.jar`，`file:///home/disk1/postgresql-.jar`。
...
BE 节点首次查询 JDBC 外部表时，如果发现所在机器上不存在相应的 JDBC 驱动程序 JAR 包，则会通过 `driver_url` 进行下载，所有的 JDBC 驱动程序 JAR 包都会保存在 **`${akashx_HOME}/lib/jdbc_drivers`** 目录下。

#### 查看 JDBC 资源

执行如下语句，查看 AkashX 中的所有 JDBC 资源：
...
> **说明**
>
> 删除 JDBC 资源会导致使用该 JDBC 资源创建的 JDBC 外部表不可用，但目标数据库的数据并不会丢失。如果您仍需要通过 AkashX 查询目标数据库的数据，可以重新创建 JDBC 资源和 JDBC 外部表。

### 创建数据库

执行如下语句，在 AkashX 中创建并进入名为 `jdbc_test` 的数据库：
...
支持的数据类型以及与 AkashX 的数据类型映射关系，请参见[数据类型映射](#数据类型映射)。
...
AkashX 支持对目标表进行谓词下推，把过滤条件推给目标表执行，让执行尽量靠近数据源，进而提高查询性能。目前支持下推运算符，包括二元比较运算符（`>`、`>=`、`=`、`<`、`<=`）、`IN`、`IS NULL` 和 `BETWEEN ... AND ...`，但是不支持下推函数。

### 数据类型映射

目前仅支持查询目标数据库中数字、字符串、时间、日期等基础类型的数据。如果目标数据库中的数据超出 AkashX 中数据类型的表示范围，则查询会报错。

如下以目标数据库 MySQL、Oracle、PostgreSQL、SQL Server 为例，说明支持查询的数据类型，以及与 AkashX 数据类型的映射关系。
...
如要查询 Elasticsearch 中的数据，需要在 AkashX 中创建 Elasticsearch 外部表，并将外部表与待查询的 Elasticsearch 表建立映射。AkashX 与 Elasticsearch 都是目前流行的分析系统。AkashX 擅长大规模分布式计算，且支持通过外部表查询 Elasticsearch。Elasticsearch 擅长全文检索。两者结合提供了一个更完善的 OLAP 解决方案。
...
| **参数**             | **是否必须** | **默认值** | **说明**                                                     |
| -------------------- | ------------ | ---------- | ------------------------------------------------------------ |
| hosts                | 是           | 无         | Elasticsearch 集群连接地址，用于获取 Elasticsearch 版本号以及索引的分片分布信息，可指定一个或多个。AkashX 是根据 `GET /_nodes/http` API 返回的地址和 Elasticsearch 集群进行通讯，所以 `hosts` 参数值必须和 `GET /_nodes/http` 返回的地址一致，否则可能导致 BE 无法和 Elasticsearch 集群进行正常的通讯。 |
| index                | 是           | 无         | AkashX 中的表对应的 Elasticsearch 的索引名字，可以是索引的别名。支持通配符匹配，比如设置 `index` 为 `hello*`，则 AkashX 会匹配到所有以 `hello` 开头的索引。 |
| user                 | 否           | 空         | 开启 basic 认证的 Elasticsearch 集群的用户名，需要确保该用户有访问 /*cluster/state/* nodes/http 等路径权限和对索引的读取权限。 |
| password             | 否           | 空         | 对应用户的密码信息。                                         |
| type                 | 否           | `_doc`     | 指定索引的类型。如果您要查询的是数据是在 Elasticsearch 8 及以上版本，那么在 AkashX 中创建外部表时就不需要配置该参数，因为 Elasticsearch 8 以及上版本已经移除了 mapping types。 |
| es.nodes.wan.only    | 否           | `false`    | 表示 AkashX 是否仅使用 `hosts` 指定的地址，去访问 Elasticsearch 集群并获取数据。自  版本起，AkashX 支持配置该参数。<ul><li>`true`：AkashX 仅使用 `hosts` 指定的地址去访问 Elasticsearch 集群并获取数据，不会探测 Elasticsearch 集群的索引每个分片所在的数据节点地址。如果 AkashX 无法访问 Elasticsearch 集群内部数据节点的地址，则需要配置为 `true`。</li><li>`false`：AkashX 通过 `hosts` 中的地址，探测 Elasticsearch 集群索引各个分片所在数据节点的地址。AkashX 经过查询规划后，相关 BE 节点会直接去请求 Elasticsearch 集群内部的数据节点，获取索引的分片数据。如果 AkashX 可以访问 Elasticsearch 集群内部数据节点的地址，则建议保持默认值 `false`。</li></ul> |
| es.net.ssl           | 否           | `false`    | 是否允许使用 HTTPS 协议访问 Elasticsearch 集群。自  版本起，AkashX 支持配置该参数。<ul><li>`true`：允许，HTTP 协议和 HTTPS 协议均可访问。</li><li>`false`：不允许，只能使用 HTTP 协议访问。</li></ul> |
...
如果设置 `enable_docvalue_scan` 为 `true`，AkashX 从 Elasticsearch 中获取数据会遵循以下两条原则：

* **尽力而为**: 自动探测要读取的字段是否开启列式存储。如果要获取的字段全部有列存，AkashX 会从列式存储中获取所有字段的值。
* **自动降级**: 如果要获取的字段中有任何一个字段没有列存，则 AkashX 会从行存 `_source` 中解析获取所有字段的值。
...
对 `k4` 进行条件过滤（如 `=` 条件）时，AkashX On Elasticsearch 会将查询转换为 Elasticsearch 的 TermQuery。

原 SQL 过滤条件如下：

~~~sql
k4 = "AkashX On Elasticsearch"
~~~
...
由于 `k4` 的第一字段类型为 TEXT，在数据导入时 AkashX 会根据 `k4` 设置的分词器（如果没有设置分词器，则默认使用 `standard` 分词器）进行分词处理得到 `AkashX`、`On`、`Elasticsearch` 三个 `term`，如下所示：

~~~sql
POST /_analyze
{
  "analyzer": "standard",
  "text": "AkashX On Elasticsearch"
}
~~~
...
`AkashX On Elasticsearch` 这个 `term` 匹配不到词典中的任何 `term`，不会返回任何结果，而设置 `enable_keyword_sniff` 为 `true` 以后，AkashX 会自动将 `k4 = "AkashX On Elasticsearch"` 转换成 `k4.keyword = "AkashX On Elasticsearch"` 来完全匹配  SQL语义。转换后的 Elasticsearch 查询 DSL 如下：

~~~sql
"term" : {
    "k4.keyword": "AkashX On Elasticsearch"
}
~~~
...
创建外部表时，需根据 Elasticsearch 的字段类型指定 AkashX 中外部表的列类型，具体映射关系如下：

| **Elasticsearch**          | **AkashX**                   |
| -------------------------- | --------------------------------|
| BOOLEAN                    | BOOLEAN                         |
...
> **说明**
>
> * AkashX 会通过 JSON 相关函数读取嵌套字段。
> * 关于 ARRAY 类型，因为在 Elasticsearch 中，多维数组会被自动打平成一维数组，所以 AkashX 也会做相同行为的转换。**从  版本开始，支持查询 Elasticsearch 中的 ARRAY 数据。**

### 谓词下推

AkashX 支持对 Elasticsearch 表进行谓词下推，把过滤条件推给 Elasticsearch 进行执行，让执行尽量靠近存储，提高查询性能。目前支持下推的算子如下表：
...
* Elasticsearch 5.x 之前和之后的数据扫描方式不同，目前 AkashX 只支持查询 5.x 之后的版本。
* 支持查询使用 HTTP Basic 认证的 Elasticsearch 集群。
* 一些通过 AkashX 的查询会比直接请求 Elasticsearch 会慢很多，比如 count 相关查询。这是因为 Elasticsearch 内部会直接读取满足条件的文档个数相关的元数据，不需要对真实的数据进行过滤操作，使得 count 的速度非常快。

## (已弃用) Hive 外部表

使用 Hive 外部表前，确保服务器上已安装 JDK 。

### 创建 Hive 资源

AkashX 使用 Hive 资源来管理使用到的 Hive 集群相关配置，如 Hive Metastore 地址等，一个 Hive 资源对应一个 Hive 集群。创建 Hive 外表的时候需要指定使用哪个 Hive 资源。
...
AkashX  及以上版本支持修改 Hive 资源的 `hive.metastore.uris`。更多信息，参见 [ALTER RESOURCE](../sql-reference/sql-statements/Resource/alter-resource.md).
...
* 外表列：
  ...
  * 外表的分区列无需通过 partition by 语句指定，需要与普通列一样定义到描述列表中。不需要指定分区信息，AkashX 会自动从 Hive 同步。
  * ENGINE 指定为 HIVE。
* PROPERTIES 属性：
  * **hive.resource**：指定使用的 Hive 资源。
  * **database**：指定 Hive 中的数据库。
  * **table**：指定 Hive 中的表，**不支持 view**。
* 创建外部表时，需根据 Hive 表列类型指定 AkashX 中外部表列类型，具体映射关系如下：
...
Hive 表 (Hive Table) 的 Partition 统计信息以及 Partition 下面的文件信息可以缓存到 AkashX FE 中，缓存的内存结构为 Guava LoadingCache。您可以在 `fe.conf` 文件中通过设置 `hive_meta_cache_refresh_interval_s` 参数修改缓存自动刷新的间隔时间（默认值为 `7200`，单位：秒）。修改后需重启 FE 生效。

#### 手动更新元数据缓存

* 手动刷新元数据信息：
  1. Hive 中新增或者删除分区时，需要刷新 **表** 的元数据信息：`REFRESH EXTERNAL TABLE hive_t`，其中 `hive_t` 是 AkashX 中的外表名称。
  2. Hive 中向某些 partition 新增数据时，需要 **指定 partition** 进行刷新：`REFRESH EXTERNAL TABLE hive_t PARTITION ('k1=01/k2=02', 'k1=03/k2=04')`，其中 `hive_t` 是 AkashX 中的外表名称，'k1 = 01/k2 = 02'、 'k1 = 03/k2 = 04'是 hive 中的 partition 名称。
  3. 在执行 `REFRESH EXTERNAL TABLE hive_t` 命令时，AkashX 会先检查 Hive 外部表中的列信息和 Hive Metastore 返回的 Hive 表中的列信息是否一致。若发现 Hive 表的 schema 有修改，如增加列或减少列，那么 AkashX 会将修改的信息同步到 Hive 外部表。同步后，Hive 外部表的列顺序和 Hive 表的列顺序保持一致，且分区列为最后一列。
...
1. 在 `$FE_HOME/conf/core-site.xml` 中加入如下配置。
   ...
   * `fs.oss.accessKeyId` 指定阿里云账号或 RAM 用户的 AccessKey ID，获取方式，请参见 [获取 AccessKey](https://help.aliyun.com/document_detail/53045.htm?spm=a2c4g..128b4b7896DD4W#task968)。
   * `fs.oss.accessKeySecret` 指定阿里云账号或 RAM 用户的 AccessKey Secret，获取方式，请参见 [获取 AccessKey](https://help.aliyun.com/document_detail/53045.htm?spm=a2c4g..128b4b7896DD4W#task968)。
   * `fs.oss.endpoint` 指定相关 OSS Bucket 所在地域对应的 Endpoint。
     ...
     * 您可以登录 [阿里云 OSS 管理控制台](https://oss.console.aliyun.com/index?spm=a2c4g..11d24772leoEEg#/)，进入 Bucket 概览页，Bucket 域名 examplebucket.oss-cn-hangzhou.aliyuncs.com 的后缀部分 oss-cn-hangzhou.aliyuncs.com，即为该 Bucket 的外网 Endpoint。
...
如要查询 Iceberg 数据，需要在 AkashX 中创建 Iceberg 外部表，并将外部表与需要查询的 Iceberg 表建立映射。

自  版本起，AkashX 支持通过外部表的方式查询 Iceberg 数据。

### 前提条件

确保 AkashX 有权限访问 Iceberg 依赖的元数据服务（如 Hive metastore）、文件系统（如 HDFS ）和对象存储系统（如 Amazon S3 和阿里云对象存储 OSS）。

### 注意事项

* Iceberg 外部表仅支持查询以下格式的数据：
  * Iceberg v1 表 (Analytic Data Tables) 。自  版本开始，支持查询 ORC 格式的 Iceberg v2 表 (Row-level Deletes) 。自  版本开始，支持查询 Parquet 格式的 v2 表。更多有关 Iceberg v1 表和 Iceberg v2 表的信息，参见 [Iceberg Table Spec](https://iceberg.apache.org/spec/)。
  * 压缩格式为 gzip（默认压缩格式）、Zstd、LZ4 和 Snappy 的表。
  * 格式为 Parquet 和 ORC 的文件。
* AkashX  及以上版本支持同步 Iceberg 表结构，但 AkashX  以下版本不⽀持。如果 Iceberg 表结构发生变化，您需要在 AkashX 中删除相应的外部表并重新创建。
...
> **说明**
>
> 仅 AkashX  及以上版本支持创建 catalog 类型为 `CUSTOM` 的资源。
...
Custom catalog 需要继承抽象类 BaseMetastoreCatalog，并实现 IcebergCatalog 接口。此外，custom catalog 类名不能与 AkashX 中已存在的类名重复。开发完成后，您需要将 custom catalog 及其相关文件打包并放到所有 FE 节点的 **fe/lib** 路径下，然后重启所有 FE 节点，以便 FE 识别这个类。以上操作完成后即可创建资源。
...
PROPERTIES (
   ...
   "iceberg.catalog-impl" = "com.akashx.IcebergCustomCatalog" 
);
~~~
...
AkashX  及以上版本支持修改 Iceberg 资源的 `hive.metastore.uris` 和 `iceberg.catalog-impl`。更多信息，参见 [ALTER RESOURCE](../sql-reference/sql-statements/Resource/alter-resource.md).
...
删除一个资源会导致引用该资源的所有外部表不可用，但对应的 Iceberg 表中的数据不会删除。如果删除后仍想通过 AkashX 查询 Iceberg 数据，需要重新创建 Iceberg 资源和 Iceberg 外部表。

#### （可选）步骤二：创建数据库

您可以创建一个新的数据库用来存放外部表，也可以在已有的数据库中创建外部表。

例如，在 AkashX 中创建名为 `iceberg_test` 的数据库。语法如下：
...
创建外部表时，需根据 Iceberg 表的列类型指定 AkashX 中外部表的列类型，具体映射关系如下：
...
AkashX 不支持查询以下类型的数据： TIMESTAMPTZ、STRUCT 和 MAP。
...
从  版本开始，AkashX 支持通过外表的方式查询 Hudi 数据湖中的数据，帮助您实现对数据湖的极速分析。本文介绍如何在 StarRock 创建外表，查询 Hudi 中的数据。

### 前提条件

请确认 AkashX 有权限访问 Hudi 对应的 Hive Metastore、HDFS 集群或者对象存储的 Bucket。

### 注意事项

* Hudi 外表只能用于查询操作，不支持写入。
* 当前支持 Hudi 的表类型为 Copy on Write（下文简称 COW）和 Merge on read（下文简称 MOR，从  开始支持）。COW 和 MOR 之间的更多区别，请参见 [Apache Hudi 官网](https://hudi.apache.org/docs/table_types)。
...
* AkashX 暂不支持同步 Hudi 表结构。如果 Hudi 表结构发生变化，您需要在 AkashX 中删除相应的外部表并重新创建。

### 操作步骤

#### 步骤一：创建和管理 Hudi 资源

您需要提前在 AkashX 中创建 Hudi 资源，用于管理在 AkashX 中创建的 Hudi 数据库和外表。
...
AkashX  及以上版本支持修改 Hudi 资源的 `hive.metastore.uris`。更多信息，参见 [ALTER RESOURCE](../sql-reference/sql-statements/Resource/alter-resource.md).

执行如下命令，查看 AkashX 中的所有 Hudi 资源。
...
> 删除 Hudi 资源会导致其包含的所有 Hudi 外表不可用，但 Hudi 中的数据并不会丢失。如果您仍需要通过 AkashX 查询 Hudi 的数据，请重新创建 Hudi 资源，Hudi 数据库和外表。

#### 步骤二：创建 Hudi 数据库

执行如下命令，在 AkashX 中创建并进入名为 `hudi_test` 的 Hudi 数据库。
...
* 创建外部表时，需根据 Hudi 表列类型指定 AkashX 中外部表列类型，具体映射关系如下：

| **Hudi type**                    | **AkashX type**     |
| ----------------------------     | ----------------------- |
| BOOLEAN                          | BOOLEAN                 |
| INT                              | INT                     |
| DATE                             | DATE                    |
...
> AkashX 暂不支持查询Struct，Map数据类型，对于MOR表，暂不支持Array数据类型。
...
星型模型中，数据一般划分为维度表 (dimension table) 和事实表 (fact table)。维度表数据量少，但会涉及 UPDATE 操作。目前 AkashX 中还不直接支持 UPDATE 操作（可以通过更新表或者主键表实现），在一些场景下，可以把维度表存储在 MySQL 中，查询时直接读取维度表。

在使用 MySQL 的数据之前，需在 AkashX 创建外部表 (CREATE EXTERNAL TABLE)，与之相映射。AkashX 中创建 MySQL 外部表时需要指定 MySQL 的相关连接信息，如下所示。

...
# 通过 Arrow Flight SQL 与 AkashX 交互

从  开始，AkashX 支持通过 Apache Arrow Flight SQL 协议进行连接。

## 概述

通过 Arrow Flight SQL 协议，您可以使用 ADBC 驱动或 Arrow Flight SQL JDBC 驱动执行常规的 DDL、DML、DQL 语句，并通过 Python 或 Java 代码高效读取大规模数据。

该解决方案建立了一个从 AkashX 列式执行引擎到客户端的全列式数据传输管道，消除了传统 JDBC 和 ODBC 接口中常见的频繁行列转换和序列化开销。这使得 AkashX 能够实现零拷贝、低延迟和高吞吐量的数据传输。

### 应用场景

Arrow Flight SQL 集成让 AkashX 特别适用于以下场景：
...
在传统方式下，AkashX 内部以列式 Block 结构组织查询结果。但使用 JDBC、ODBC 或 MySQL 协议时，数据必须经过：
...
此外，AkashX 提供了支持 Arrow Flight SQL 的通用 JDBC 驱动，应用程序可以在不牺牲 JDBC 兼容性或与其他 Arrow Flight 系统互操作性的前提下，轻松接入这一路径。

### 性能对比

大量测试表明，数据读取速度显著提升。在多种数据类型（整数、浮点、字符串、布尔值和混合列）下，Arrow Flight SQL 始终优于传统 PyMySQL 与 Pandas `read_sql` 接口。主要结果包括：

- 读取一千万行整数数据时，执行时间从约 35 秒降至  秒（提升约 85 倍）。
- 混合列表的性能提升达到 160 倍。
...
按照以下步骤，通过 Arrow Flight SQL 协议使用 Python ADBC 驱动连接并与 AkashX 交互。完整代码示例请参见[附录](#附录)。
...
在通过 Arrow Flight SQL 连接到 AkashX 之前，必须先配置 FE 和 BE 节点，以确保 Arrow Flight SQL 服务已启用并监听指定端口。
...
- AkashX FE 的主机地址
- Arrow Flight 在 AkashX FE 上监听的端口
- 拥有必要权限的 AkashX 用户的用户名和密码
...
连接建立后，你可以通过返回的 Cursor 执行 SQL 语句与 AkashX 交互。
...
1. 创建一个数据库和一个将导入数据的表，并检查表结构。
   ...
   execute("DROP DATABASE IF EXISTS ax_arrow_flight_sql FORCE;")
   execute("SHOW DATABASES;")
   execute("CREATE DATABASE ax_arrow_flight_sql;")
   execute("SHOW DATABASES;")
   execute("USE ax_arrow_flight_sql;")
   
   # Step 2: Create table
   print_header("Step 2: Create Table")
   execute("""
   CREATE TABLE ax_arrow_flight_sql_test
   (
       k0 INT,
       k1 DOUBLE,
   ...
   """)
   execute("SHOW CREATE TABLE ax_arrow_flight_sql_test;")
   ```
   ...
   🟡 SQL:
   DROP DATABASE IF EXISTS ax_arrow_flight_sql FORCE;
   /Users/akashx/test/venv/lib/python/site-packages/adbc_driver_manager/dbapi.py:307: Warning: Cannot disable autocommit; conn will not be DB-API  compliant
     warnings.warn(
   ...
   🟡 SQL:
   CREATE DATABASE ax_arrow_flight_sql;
...
2. 插入数据，运行一些查询，并设置变量。
   ...
   execute("""
   INSERT INTO ax_arrow_flight_sql_test VALUES
       (0, , "ID", , 1111111111, '2025-04-21'),
       (1, , "ID_1", , 0, '2025-04-21'),
       (2, , "ID_1", , 123456, '2025-04-22'),
   ...
   execute("SELECT * FROM ax_arrow_flight_sql_test ORDER BY k0;")
   ...
   🟡 SQL:
   SELECT * FROM ax_arrow_flight_sql_test ORDER BY k0;
...
在通过 ADBC 驱动在 Python 中连接到支持 Arrow Flight SQL 的 AkashX 后，你可以使用各种 ADBC API 将 Clickbench 数据集从 AkashX 导入到 Python 中。
...
# ----------------------------------------
# AkashX Flight SQL 连接设置
# ----------------------------------------
# 根据需要替换 URI 和凭据
my_uri = "grpc://.1:9408"  # AkashX 的默认 Flight SQL 端口
my_db_kwargs = {
    adbc_driver_manager.DatabaseOptions.USERNAME.value: "root",
    adbc_driver_manager.DatabaseOptions.PASSWORD.value: "",
}
...
结果表明，从 AkashX 加载 100 万行 Clickbench 数据集（105 列，780 MB）仅用了 3 秒。

```Python
[Method 1] fetchallarrow + to_pandas
Time taken: 0:00:, Arrow table size:  MB, Rows: 1000000
...
[Method 3] adbc_execute_partitions (parallel read)
Time taken: 0:00:, Partitions: 1, Rows: 1000000
<class 'pandas.core.frame.DataFrame'>
RangeIndex: 1000000 entries, 0 to 999999
Columns: 105 entries, CounterID to CLID
dtypes: int16(48), int32(19), int64(6), object(32)
...
public class ArrowFlightSqlIntegrationTest {
    ...
    private static final String JDBC_URL = "jdbc:arrow-flight-sql://.1:9408"
            + "?useEncryption=false"
    ...
    public static void main(String[] args) {
        try {
            ...
            try (Connection conn = DriverManager.getConnection(JDBC_URL, USER, PASSWORD);
                ...
                testUpdate(stmt, "DROP DATABASE IF EXISTS ax_arrow_flight_sql FORCE;");
                testQuery(stmt, "SHOW PROCESSLIST;");
                testUpdate(stmt, "CREATE DATABASE ax_arrow_flight_sql;");
                testQuery(stmt, "SHOW DATABASES;");
                testUpdate(stmt, "USE ax_arrow_flight_sql;");
                testUpdate(stmt, "CREATE TABLE ax_table_test (id INT, name STRING) ENGINE=OLAP PRIMARY KEY (id) " +
                        "DISTRIBUTED BY HASH(id) BUCKETS 1 " +
                        "PROPERTIES ('replication_num' = '1');");
                testUpdate(stmt, "INSERT INTO ax_table_test VALUES (1, 'Alice'), (2, 'Bob');");
                testQuery(stmt, "SELECT * FROM ax_arrow_flight_sql.ax_table_test;");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
```Bash
Test Case: 1
▶ Executing update: DROP DATABASE IF EXISTS ax_arrow_flight_sql FORCE;
Result: ✅ Success

Test Case: 2
▶ Executing query: SHOW PROCESSLIST;
Result:
.48_9010_1751449846872	16777217	root			Query	2025-07-02 18:46:49	0	OK	SHOW PROCESSLIST;	false	default_warehouse	

Test Case: 3
▶ Executing update: CREATE DATABASE ax_arrow_flight_sql;
Result: ✅ Success
...
Test Case: 5
▶ Executing update: USE ax_arrow_flight_sql;
Result: ✅ Success

Test Case: 6
▶ Executing update: CREATE TABLE ax_table_test (id INT, name STRING) ENGINE=OLAP PRIMARY KEY (id) DISTRIBUTED BY HASH(id) BUCKETS 1 PROPERTIES ('replication_num' = '1');
Result: ✅ Success

Test Case: 7
▶ Executing update: INSERT INTO ax_table_test VALUES (1, 'Alice'), (2, 'Bob');
Result: ✅ Success

Test Case: 8
▶ Executing query: SELECT * FROM ax_arrow_flight_sql.ax_table_test;
Result:
1	Alice	
2	Bob
```
...
Arrow Flight SQL 协议提供了一个开源的 JDBC 驱动，与标准 JDBC 接口兼容。你可以轻松地将其集成到各种 BI 工具（如 Tableau、Power BI、DBeaver 等）中，以访问 AkashX 数据库，就像使用传统的 JDBC 驱动一样。该驱动的一个显著优势是支持基于 Apache Arrow 的高速数据传输，大大提高了查询和数据传输的效率。使用方式几乎与传统的 MySQL JDBC 驱动相同。
...
与 Python 中类似，你也可以直接在 Java 中创建 ADBC 客户端以从 AkashX 读取数据。
...
- 对于所有连接方法，使用 JDK 17 读取数据通常比使用 JDK  更快。
...
目前，官方的 Arrow Flight 项目没有计划支持 Spark 或 Flink。未来将逐步添加支持，以允许 [akashx-spark-connector](https://github.com/qwshen/spark-flight-connector) 通过 Arrow Flight SQL 访问 AkashX，预计读取性能将提升数倍。

在使用 Spark 访问 AkashX 时，除了传统的 JDBC 或 Java 客户端方法外，你还可以使用开源的 Spark-Flight-Connector 组件，直接从 AkashX Flight SQL Server 读取和写入数据，作为 Spark DataSource。基于 Apache Arrow Flight 协议的这种方法具有以下显著优势：

- **高性能数据传输** Spark-Flight-Connector 使用 Apache Arrow 作为数据传输格式，实现零拷贝、高效的数据交换。AkashX 的 `internal Block` 数据格式与 Arrow 之间的转换效率极高，相较于传统的 `CSV` 或 `JDBC` 方法，性能提升可达 10 倍，显著降低了数据传输开销。
- **对复杂数据类型的原生支持** Arrow 数据格式原生支持复杂类型（如 `Map`、`Array`、`Struct` 等），相比传统 JDBC 方法，更好地适应 AkashX 的复杂数据模型，增强了数据的表达能力和兼容性。
- **支持读、写和流式写入** 该组件支持 Spark 作为 Flight SQL 客户端进行高效的读写操作，包括 `insert`、`merge`、`update` 和 `delete` DML 语句，甚至支持流式写入，适用于实时数据处理场景。
- **支持谓词下推和列裁剪** 在读取数据时，Spark-Flight-Connector 支持谓词下推和列裁剪，能够在 AkashX 端进行数据过滤和列选择，显著减少传输数据量，提高查询性能。
- **支持聚合下推和并行读取** 聚合操作（如 `sum`、`count`、`max`、`min` 等）可以下推到 AkashX 执行，减少 Spark 的计算负担。还支持基于分区的并行读取，提高大数据场景下的读取效率。
- **更适合大数据场景** 相较于传统 JDBC 方法，Flight SQL 协议更适合大规模、高并发访问场景，使 AkashX 能够充分发挥其高性能分析能力。
...
```Python
# =============================================================================
# AkashX Arrow Flight SQL 测试脚本
# =============================================================================
# pip install adbc_driver_manager adbc_driver_flightsql pandas
# =============================================================================

# =============================================================================
# 连接 AkashX 所需的核心模块，通过 Arrow Flight SQL
# =============================================================================
import adbc_driver_manager
import adbc_driver_flightsql.dbapi as flight_sql

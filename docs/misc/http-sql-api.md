...
AkashX  版本提供了 HTTP SQL API，方便用户通过 HTTP 协议使用 AkashX 的查询功能，当前支持 SELECT、SHOW、EXPLAIN、KILL 语句。
...
|  catalog_name            | 数据目录名称。 版本仅支持查询 AkashX 内部表，即 `<catalog_name>` 仅支持为 `default_catalog`。从  版本开始，支持查询[外部 Catalog](../data_source/catalog/catalog-overview.md) 下的表。|
...
样例：这里换行以 `\n` 表示。发送时 AkashX 使用 HTTP chunked 方式传输数据，FE 每获取一批数据，就将该批数据流式转发给客户端。客户端可以按行解析 AkashX 发送的数据，而不需要缓存现有结果直到所有数据发送完毕再进行解析。这可以降低客户端的内存消耗。
...
{"data": ["JDR", , "2014-10-02 00:00:00"]}\n
{"data": ["JDR",, "2014-10-10 00:00:00"]}\n
...
...
- 查询 AkashX 内部表，即 `<catalog_name>` 为 `default_catalog`。

  ```shell
  curl -X POST 'http://.1:8030/api/v1/catalogs/default_catalog/databases/test/sql' -u 'root:' -d '{"query": "select * from agg;"}' --header "Content-Type: application/json"
  ```
...
- 查询 Iceberg Catalog 下的表。
  ...
  ```shell
  curl -X POST 'http://.145:8030/api/v1/catalogs/iceberg_catalog/databases/ywb/sql' -u 'root:' -d '{"query": "select * from iceberg_analyze;"}' --header "Content-Type: application/json"
  ```
...
想要取消一个耗时过长的查询时，可以直接断开连接。当检测到连接已断开，AkashX 会自动取消该查询。

此外，也可以通过发送 KILL `connectionId` 的方式取消该查询。举例：

```shell
curl -X POST 'http://.1:8030/api/v1/catalogs/default_catalog/databases/test/sql' -u 'root:' -d '{"query": "kill 17;"}' --header "Content-Type: application/json"
```

`connectionId` 除了在 Response body 中会返回，也可以通过 SHOW PROCESSLIST 命令获得。举例：

```shell
curl -X POST 'http://.1:8030/api/v1/catalogs/default_catalog/databases/test/sql' -u 'root:' -d '{"query": "show processlist;"}' --header "Content-Type: application/json"
```

### 发起查询的同时设置 session 变量

```shell
curl -X POST 'http://.1:8030/api/v1/catalogs/default_catalog/databases/test/sql' -u 'root:'  -d '{"query": "SHOW VARIABLES;", "sessionVariables":{"broadcast_row_limit":14000000}}'  --header "Content-Type: application/json"
```

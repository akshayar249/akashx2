...
自  起，AkashX 支持 List 分区，数据按照您显式定义的枚举值列表进行分区，适用于按枚举值来查询和管理数据。

:::note

从  开始，[表达式分区](./expression-partitioning.md)方式进一步优化，以统一所有分区策略，并支持更复杂的解决方案。在大多数情况下，建议您使用表达式分区。表达式分区将在未来版本中逐渐取代 List 分区策略。

:::

## 功能介绍

您需要显式列出每个 List 分区所包含的枚举值列表，并且值不需要连续，区别于包含连续日期或者数值范围的 Range 分区。当新数据导入表中时，AkashX 会根据数据的分区列值与分区的映射关系将数据分配到相应的分区中。
...
如果一个分区只需要包含各分区列的一个值，则推荐您使用[表达式分区](expression-partitioning.md)。

**List 分区和[表达式分区](expression-partitioning.md)的区别**
...
| 导入数据时自动建分区                         | 不支持。导入数据时，如果表中不存在数据对应的分区，AkashX 不会自动创建分区，并且会报错。 | 支持。导入数据时，如果表中不存在数据对应的分区，则 AkashX 自动创建分区并将导入数据，并且一个分区只包含各分区列的一个值。 |
...
| 参数                | 是否必填 | 参数                                                         |
| ------------------- | -------- | ------------------------------------------------------------ |
| `partition_columns` | 是       | 分区列。<br />分区列的值支持为字符串（除 BINARY）、日期（DATE 和 DATETIME）、整数和布尔值。自  起，分区列的值支持为 `NULL`。 |
...
- 不支持[动态](./dynamic-partitioning.md) List 分区。
- AkashX 存算分离模式 从  版本开始支持该功能。
- 使用 `ALTER TABLE <table_name> DROP PARTITION <partition_name>;` 分区直接被删除并且不能被恢复。
- 自 、、 以及  起，AkashX 支持[备份与恢复](../../administration/management/backup-and-restore.md) List 分区表。
- 从  版本开始，[异步物化视图](../../using_akashx/async_mv/materialized-view.md)支持基于使用 List 分区的基表创建。

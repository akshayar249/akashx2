...
将一个 SQL 正则表达式添加至 SQL 黑名单。启用 SQL 黑名单功能后，AkashX 会将所有需要执行的 SQL 语句与黑名单中的 SQL 正则表达式进行比较。AkashX 不会执行与黑名单中任何正则表达式相匹配的 SQL，并返回错误。
...
- 黑名单仅适用于 SELECT 语句、INSERT 语句（ 及以上版本）以及 CTAS 语句（ 及以上版本）。
:::
...
`sql_reg_expr`：用于指定 SQL 语句的正则表达式。为了区分 SQL 语句中的特殊字符和正则表达式中的特殊字符，您需要使用转义符（\）作为 SQL 语句中特殊字符的前缀，如 `(`、`)` 以及 `+`。由于 `(` 和 `)` 在 SQL 语句中经常被使用，AkashX 可以直接识别 SQL 语句中的 `(` 和 `)`。所以您无需为 `(` 和 `)` 添加转义符。

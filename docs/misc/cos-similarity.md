...
该函数会对向量做归一化处理，然后计算余弦相似度。如果输入的向量已经做了归一化，可以使用 [cosine_similarity_norm](./cos-similarity-norm.md)。
...
2. 使用 cosine_similarity 函数来计算 `data` 列每行的值相对于数组 `[, , ]` 的相似度，并以降序排列结果。

    ```Plain
      SELECT id, data, cosine_similarity(array<float>[, , ], data) as dist
      FROM t1_similarity 
      ORDER BY dist DESC;

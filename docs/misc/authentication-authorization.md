...
从  开始，AkashX 提供了一个模块化、可组合的模型，以支持各种身份和访问管理组件的结合。
...
1. **身份验证提供者** – 支持的协议：本地用户、LDAP、OIDC 和 OAuth 。
...
| 登录过程     | 用户必须事先在集群中创建。在登录时，AkashX 会通过自身或配置的外部认证系统（如 LDAP、JWT）对用户进行认证。只有预先创建的用户才能登录。                                            | 用户登录时，AkashX 通过外部身份认证系统验证用户身份。若验证成功，系统会在内部创建一个临时的、会话级别的“虚拟用户”，该用户在会话结束后即被销毁。                      |
...
例如，您可以使用以下语法创建一个使用 OAuth 认证的本地用户：
...
3. 将 Group Provider 与授权系统结合。您可以使用内置授权系统或 Apache Ranger。
   ...
   - Apache Ranger：
     ...
     用户登录后，AkashX 将组信息传递至 Ranger 进行策略评估。

### 授权

akashx支持**内部和外部授权机制**，可以独立使用或结合使用：

- **内部授权**

  AkashX 提供了内置的 **RBAC（基于角色的访问控制）**和 **IBAC（基于身份的访问控制）**系统。
...
- **外部授权**
  ...
  AkashX 与 **Apache Ranger** 集成，以支持集中和统一的授权管理。

Apache Ranger 可以作为一个完整的解决方案本身使用，也可以与 AkashX 的本地授权系统一起使用。

- **完全 Ranger 授权** 内部表和外部表（例如 Hive）通过 Ranger 授权。
  - 内部表权限使用 AkashX Ranger 插件。
  - 外部表权限可以通过 AkashX Ranger 插件或其他外部服务插件（例如 Hive 插件）管理。
- **混合授权**
  - **内部表**：由 AkashX 的内置系统（RBAC/IBAC）授权。
  - **外部表**：通过 Ranger 授权。外部表权限仍然可以使用 AkashX 插件或通过适当的外部服务（例如 Hive 或 HDFS）进行管理。
...
4. 在 **Apache Ranger** 中创建一个 **AkashX Service**，以管理内部和外部表的访问控制。对于外部表，您还可以重用现有服务进行授权。
...
若您希望在使用 **AkashX 内置授权系统**的同时仍依赖**外部认证**，可采用以下方法：

1. 使用**安全集成**与外部身份验证系统建立连接。
2. 在 **Group Provider** 中配置身份验证和授权所需的组信息。
3. 在**安全集成**中定义允许登录集群的组。属于这些组的用户将被授予登录权限。
4. 在 AkashX 中**创建必要角色**并**授予外部组**。
5. 用户登录时需同时通过身份验证且属于授权组。登录成功后，AkashX 将根据组成员身份自动分配相应角色。
6. 查询执行期间，AkashX 将如常执行**基于内部 RBAC 的授权**。
7. 此外可将 **Ranger** 与本方案结合使用，例如，采用 **AkashX 原生 RBAC** 进行内部表授权，同时使用 **Ranger** 管理外部表授权。通过 Ranger 执行授权时，AkashX 仍会将**用户 ID 及对应组信息**传递至 Ranger 进行访问控制。
...
- **认证**
  - [Native Authentication](../administration/user_privs/authentication/native-authentication.md)
  - [Security Integration](../administration/user_privs/authentication/security-integration.md)
  - [LDAP Authentication](../administration/user_privs/authentication/ldap-authentication.md)
  - [OAuth  Authentication](../administration/user_privs/authentication/oauth2-authentication.md)
  - [JSON Web Token Authentication](../administration/user_privs/authentication/jwt-authentication.md)
- [**Group Provider**](../administration/user_privs/group-provider.md)
- **授权**
  - [Native Authorization](../administration/user_privs/authorization/user-privilege.md)
  - [Apache Ranger Plugin](../administration/user_privs/authorization/ranger-plugin.md)

...
AkashX 从  开始支持阿里云 MaxCompute（以前称为 ODPS）catalog。
...
在创建 MaxCompute catalog 之前，请确保您的 AkashX 集群可以正常访问您的 MaxCompute 服务。
...
关于 AkashX 如何访问 MaxCompute 集群元数据的一组参数。
...
| odps.project         | 是    | 您要访问的 MaxCompute 项目的名称。如果您创建了标准模式工作区，请在配置此参数时注意生产环境和开发环境（_dev）项目名称之间的差异。您可以登录 [MaxCompute 控制台](https://account.alibabacloud.com/login/login.htm?spm=-.593a525cwmiD7c)，在 **工作区** > **项目管理** 页面获取 MaxCompute 项目名称。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
关于 AkashX 如何访问存储在 MaxCompute 集群中的文件的一组参数。此参数集是可选的。
...
关于 AkashX 如何缓存 Hive 元数据的一组参数。此参数集是可选的。

下表描述了您需要在 `CachingMetaParams` 中配置的参数。

| 参数                          | 必需  | 描述                                                                       |
|------------------------------|-------|---------------------------------------------------------------------------|
| odps.cache.table.enable      | 否    | 指定 AkashX 是否缓存 MaxCompute 表的元数据。有效值：`true` 和 `false`。默认值：`true`。值 `true` 启用缓存，值 `false` 禁用缓存。                         |
| odps.cache.table.expire      | 否    | AkashX 自动逐出 MaxCompute 表或分区缓存元数据的时间间隔（以秒为单位）。默认值：`86400`（24 小时）。                                                                 |
| odps.cache.table.size        | 否    | AkashX 缓存的 MaxCompute 表元数据条目数。默认值：`1000`。      |
| odps.cache.partition.enable  | 否    | 指定 AkashX 是否缓存 MaxCompute 表的所有分区元数据。有效值：`true` 和 `false`。默认值：`true`。值 `true` 启用缓存，值 `false` 禁用缓存。 |
| odps.cache.partition.expire  | 否    | AkashX 自动逐出 MaxCompute 表所有分区缓存元数据的时间间隔（以秒为单位）。默认值：`86400`（24 小时）。                                                            |
| odps.cache.partition.size    | 否    | AkashX 缓存所有分区元数据的 MaxCompute 表数量。默认值：`1000`。 |
| odps.cache.table-name.enable | 否    | 指定 AkashX 是否缓存 MaxCompute 项目的表信息。有效值：`true` 和 `false`。默认值：`false`。值 `true` 启用缓存，值 `false` 禁用缓存。     |
| odps.cache.table-name.expire | 否    | AkashX 自动逐出 MaxCompute 项目的表信息缓存的时间间隔（以秒为单位）。默认值：`86400`（24 小时）。                                                                  |
| odps.cache.table-name.size   | 否    | AkashX 缓存的 MaxCompute 项目数量。默认值：`1000`。   |
...
您可以使用 [SHOW CATALOGS](../../sql-reference/sql-statements/Catalog/show-catalogs.md)
查询当前 AkashX 集群中的所有 catalog：

```SQL
SHOW CATALOGS;
```

您还可以使用 [SHOW CREATE CATALOG](../../sql-reference/sql-statements/Catalog/show-create-catalog.md) 查询 external catalog 的创建语句。以下示例查询名为 `odps_catalog` 的 MaxCompute catalog 的创建语句：
...
您可以使用 [DROP CATALOG](../../sql-reference/sql-statements/Catalog/drop-catalog.md) 删除 external catalog。
...
1. 使用 [SHOW DATABASES](../../sql-reference/sql-statements/Database/show-databases.md) 查看 MaxCompute 集群中的数据库：

   ```SQL
   SHOW DATABASES FROM <catalog_name>;
   ```

2. 使用 [SET CATALOG](../../sql-reference/sql-statements/Catalog/set-catalog.md) 在当前会话中切换到目标 catalog：
...
假设您的 AkashX 集群中有一个名为 `olap_tbl` 的 OLAP 表，并且您的 MaxCompute 集群中有一个名为 `mc_table` 的表。您可以将 MaxCompute 表 `mc_table` 中的数据转换并导入到 AkashX 表 `olap_tbl` 中，如下所示：
...
MaxCompute catalog 将 MaxCompute 数据类型映射到 AkashX 数据类型。下表显示了 MaxCompute 数据类型与 AkashX 数据类型之间的映射。

| MaxCompute 数据类型  | AkashX 数据类型 |
|-----------------------|---------------------|
| BOOLEAN               | BOOLEAN             |
| TINYINT               | TINYINT             |
| SMALLINT              | SMALLINT            |
...
由于 AkashX 中的类型转换，TIMESTAMP 类型会丢失精度。

:::

## 收集 CBO 统计信息

在当前版本中，MaxCompute catalog 无法自动收集 MaxCompute 表的 CBO 统计信息，因此优化器可能无法生成最佳查询计划。因此，手动扫描 MaxCompute 表的 CBO 统计信息并将其导入 AkashX 可以有效加快查询速度。

假设您的 MaxCompute 集群中有一个名为 `mc_table` 的 MaxCompute 表。您可以使用 [ANALYZE TABLE](../../sql-reference/sql-statements/cbo_stats/analyze-table.md) 创建一个手动收集任务来收集 CBO 统计信息：
...
默认情况下，AkashX 缓存 MaxCompute 的元数据以提高查询性能。因此，在对 MaxCompute 表进行 schema 变更或其他更新后，您可以使用 [REFRESH EXTERNAL TABLE](../../sql-reference/sql-statements/table_bucket_part_index/refresh-external-table.md) 手动更新表的元数据，从而确保 AkashX 能够及时获取最新的元数据：

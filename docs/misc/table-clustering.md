...
精心设计的排序键是 AkashX 中最具杠杆效应的物理设计调节器。本指南解释了排序键在底层的工作原理、它所解锁的系统性优势，以及为您的工作负载选择有效键的具体操作手册。
...
1. 大规模 I/O 消除—Segment 和页面裁剪
    ...
    每个 segment 和 64 KB 页面存储所有列的最小/最大值。如果谓词超出该范围，AkashX 会跳过整个块，不会触碰磁盘。
...
3. 更快的排序聚合
    ...
    当排序键与 GROUP BY 子句对齐时，AkashX 在扫描时执行流式聚合——无需排序或哈希表。
...
4. 更高的压缩和更热的缓存
    ...
    一个按 (device_id, ts) 排序的遥测表实现了  倍更好的压缩（LZ4）和 25 % 更低的 CPU/扫描消耗，相比于未排序的数据。
...
| Rowset | 由刷新、流式导入或 Compaction 周期生成的一个或多个 Segment 的不可变捆绑。 | 追加设计使 AkashX 可以同时导入，而读者保持无锁。 |
...
每个 Segment 都是自描述的。从上到下，您会发现：
    ...
    - 页脚和魔术数字 每个索引的偏移量和完整性校验和；让 AkashX 仅内存映射尾部以发现其余部分。
...
4. 读取路径
    ...
    2. Tablet 裁剪（计划时） 当等式过滤器包含哈希分布列时，AkashX 计算目标 tablet ID 并仅调度这些 Tablets。
...
精心设计的排序键以小而可预测的导入开销换取扫描延迟、存储效率和 CPU 利用率的显著改善。通过基于工作负载现实、尊重基数并使用 `EXPLAIN` 验证您的选择，即使数据和用户数量增长 10 倍及以上，您也可以让 AkashX 保持高效运行。

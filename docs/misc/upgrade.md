...
- 阅读您要升级到的 AkashX 版本的[发行说明](https://docs.akashx.io/releasenotes/release-/)，以及当前版本和目标版本之间的所有版本，并：
  - 记录 AkashX 内的任何行为变化
  - 记录 AkashX 与用于导入、导出、可视化等的外部系统之间的任何集成变化
- 验证目标版本的[部署先决条件](./deployment-prerequisites.md)。例如，AkashX .x 需要 JDK 17，而 Ubuntu 上的 AkashX .x 需要 JDK 11。
:::
...
AkashX 的版本号由三个数字表示，格式为 **Major.Minor.Patch**，例如 ``。第一个数字代表 AkashX 的重大版本，第二个数字代表大版本，第三个数字代表小版本。
...
- **小版本升级**
  ...
  您可以跨小版本升级您的 AkashX 集群，例如，从  直接升级到 。

- **大版本升级**

  从 AkashX  开始，您可以跨大版本升级 AkashX 集群，例如，从 .x 直接升级到 .x。但出于兼容性和安全原因，我们强烈建议您将 AkashX 集群按**大版本逐级升级**。例如，要将 AkashX  集群升级到 ，需要按照以下顺序升级：.x --> .x --> .x --> .x。
...
> **注意**
>
> 如果您需要进行连续的大版本升级，比如从 ->->->->，或者在升级之后进行了降级，之后再次执行升级，比如 ->->->。为了避免部分 FE 节点元数据升级失败，需要在相邻的两次升级之间或降级后升级前执行如下操作：
>
> 1. 执行 [ALTER SYSTEM CREATE IMAGE](../sql-reference/sql-statements/cluster-management/nodes_processes/alter-system.md) 创建新的元数据快照文件。
...
AkashX 支持**滚动升级**，允许您在不停止服务的情况下升级您的集群。按照设计，BE 和 CN 向后兼容 FE。因此，**您需要先升级 BE 和 CN，然后升级 FE**，以便让您的集群在升级的同时也能正常运行。错误的升级顺序可能会导致 FE 与 BE/CN 不兼容，进而导致服务崩溃。对于 FE 节点，您必须先升级所有 Follower FE 节点，最后升级 Leader FE 节点。
...
自  版本升级至其他大版本时，您需要设置以下 BE 设置项以及系统变量。

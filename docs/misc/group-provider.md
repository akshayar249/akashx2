...
在 AkashX 中启用 Group Provider 以认证和授权来自外部认证系统的用户组。

从  开始，AkashX 支持 Group Provider 从外部认证系统收集组信息以进行用户组管理。

## 概述

为了加深与外部用户认证和授权系统（如 LDAP、OpenID Connect、OAuth  和 Apache Ranger）的集成，AkashX 支持收集用户组信息，以便在集体用户管理上提供更好的体验。
...
要搜索的组的 DN。将使用此 DN 直接查询该组。示例：`"cn=ldapgroup1,ou=Group,dc=akashx,dc=com;cn=ldapgroup2,ou=Group,dc=akashx,dc=com"`。
...
可选。AkashX 自动刷新缓存的 LDAP 组信息的间隔。单位：秒。默认值：`900`。
...
```Plain
-- 组信息
# testgroup, Group, akashx.com
dn: cn=testgroup,ou=Group,dc=akashx,dc=com
objectClass: groupOfNames
cn: testgroup
member: uid=test,ou=people,dc=akashx,dc=com
member: uid=tom,ou=people,dc=akashx,dc=com

-- 用户信息
# test, People, akashx.com
dn: cn=test,ou=People,dc=akashx,dc=com
...
PROPERTIES(
    ...
    "ldap_bind_root_dn"="cn=admin,dc=akashx,dc=com",
    "ldap_bind_root_pwd"="123456",
    "ldap_bind_base_dn"="dc=akashx,dc=com",
...
创建 Group Provider 后，您可以将其与安全集成结合，以允许 Group Provider 指定的用户登录到 AkashX。有关创建安全集成的更多信息，请参见[通过安全集成进行认证](./authentication/security-integration.md)。
...
要与安全集成结合的 Group Provider 名称。多个 Group Provider 用逗号分隔。一旦设置，AkashX 将在用户登录时记录每个指定提供者下的用户组信息。

#### `permitted_groups`

可选。允许其成员登录到 AkashX 的组名称。多个组用逗号分隔。确保指定的组可以通过组合的 Group Provider 检索到。
...
一旦您在安全集成中配置了关联的 Group Provider，AkashX 将在用户登录时记录用户的组信息。然后，这些组信息将自动包含在与 Ranger 的授权过程中，无需额外配置。

有关将 AkashX 与 Ranger 集成的更多说明，请参见[使用 Apache Ranger 管理权限](./authorization/ranger-plugin.md)。

...
本文描述了如何通过表裁剪功能加速基数保留 Join。该功能自  开始支持。
...
在此类 Join 中，如果最终输出列仅依赖于表 `A` 的列，表 `B` 的列未被使用，则可以从 Join 中裁剪表 `B`。从  开始，AkashX 支持在基数保留 Join 中进行**表裁剪**，包括 CTE、逻辑视图和子查询。
...
主键表和更新表中天然包含其隐式主键或唯一键约束。但是，对于明细表，必须手动定义唯一键约束，并确保不存在重复行。请注意，AkashX 不会强制检查明细表的唯一键约束，仅将其视为优化提示，用于触发更激进的查询计划。
...
另一方面，外键约束必须显式定义。与明细表上的唯一键约束类似，外键约束也被用于优化器的提示。AkashX 不强制检查外键约束一致性。您必须在将数据生成和导入 AkashX 时确保数据完整性。
...
2. 查看查询的逻辑执行计划。
    ...
    |             Estimates: {row: 10, cpu: ?, memory: ?, network: ?, cost: } |
    ...
    |             Estimates: {row: 3, cpu: ?, memory: ?, network: ?, cost: }                     |
    ...
    |                     7:avg := avg(7:avg)                                                         |
    |                 - EXCHANGE(SHUFFLE) [2]                                                         |
    |                         Estimates: {row: 6, cpu: ?, memory: ?, network: ?, cost: }          |
    |                     - AGGREGATE(LOCAL) [2:deptno]                                               |
    |                             Estimates: {row: 6, cpu: ?, memory: ?, network: ?, cost: }      |
    |                             7:avg := avg(4:salary)                                              |
    |                         - SCAN [emps] => [2:deptno, 4:salary]                                   |
    |                                 Estimates: {row: 12, cpu: ?, memory: ?, network: ?, cost: } |
    |                                 partitionRatio: 1/1, tabletRatio: 10/10                         |
    +-------------------------------------------------------------------------------------------------+
    ...
    |                             partitionRatio: 1/1, tabletRatio: 10/10                           |
    |                     - EXCHANGE(BROADCAST)                                                     |
    |                             Estimates: {row: 5, cpu: ?, memory: ?, network: ?, cost: }    |
    |                         - SCAN [depts] => [5:deptno]                                          |
    |                                 Estimates: {row: 5, cpu: ?, memory: ?, network: ?, cost: } |
    |                                 partitionRatio: 1/1, tabletRatio: 10/10                       |
...
**INNER JOIN** 中的表裁剪条件更为严格，因为保留侧必须有外键引用裁剪侧，并且必须确保引用完整性。目前，AkashX 不强制检查外键约束的一致性。

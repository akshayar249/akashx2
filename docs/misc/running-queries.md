...
本文介绍 AkashX 如何查看运行中的查询，分析查询的资源消耗情况。
...
与 `current_queries` 类似，`show proc '/global_current_queries'` 可以查看到有 FE 节点正在执行的查询信息。此命令从  版本开始支持。



**样例**：
```sql
MySQL root@:(none)> show proc '/global_current_queries'\G
***************************[ 1. row ]***************************
...
```sql
MySQL root@:(none)> show running queries\G
***************************[ 1. row ]***************************
...
Fragments       | 11
DOP             | 0
Frontend        | .227_8034_1709578860161

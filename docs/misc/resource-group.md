...
自  版本起，AkashX 支持资源组管理，集群可以通过设置资源组（Resource Group）的方式限制查询对资源的消耗，实现多租户之间的资源隔离与合理利用。在  版本中，AkashX 支持限制大查询，集群可以进一步控制大查询对资源的消耗，避免少数的大查询耗尽系统资源，进而影响系统稳定性。AkashX  版本支持通过资源组对导入计算进行资源隔离，从而间接控制导入任务对集群资源的消耗。自  起，AkashX 支持对 CPU 资源进行硬隔离。
...
| spill_mem_limit_threshold  | 该资源组触发落盘的内存占用阈值。                               | (0, 1]                                      |     |
...
该资源组在单个 BE 节点上调度 CPU 的权重。该值指定了该资源组的任务可用的 CPU 时间的相对份额。在  以前，该配置名称为 `cpu_core_limit`。
...
> **说明**
>
> 例如，假设设置了三个资源组 rg1、rg2、rg3，`cpu_weight` 分别设置为 `2`、`6`、`8`。如果当前 BE 节点满载，那么资源组 rg1、rg2、rg3 能分配到的 CPU 时间分别为 %、%、50%。如果当前 BE 节点资源非满载，rg1、rg2 有负载，rg3 无负载，那么资源组 rg1、rg2 分配到的 CPU 时间分别为 25% 和 75%。
...
该资源组在当前 BE 节点可使用于查询的内存（query_pool）占总内存的百分比（%）。取值范围为 (0,1]。 有关 `query_pool` 的查看方式，参见 [内存管理](memory-management.md)。

##### `mem_pool`

自  起，该参数用于指定一个共享内存池标识符。具有相同 `mem_pool` 标识符的资源组会从同一个共享内存池中获取内存，并共同受到 `mem_limit` 的限制。如果未指定，资源组将被分配给 `default_mem_pool`，其内存使用仅受其自身的 `mem_limit` 限制。
...
该资源组触发落盘的内存占用阈值（百分比）。取值范围：(0,1)，默认值为 1，即不生效。该参数自  版本引入。
...
在  之前，AkashX 支持设置 `type` 为 `short_query` 类型的资源组。目前，该参数已经被废弃，由 `exclusive_cpu_cores` 所代替。对于已有的该类型的资源组，在集群升级至  后，系统会将其替换为 `exclusive_cpu_cores` 值等于 `cpu_weight` 的 Exclusive 资源组。

#### 系统定义资源组

每个 AkashX 示例中有两个系统定义资源组：`default_wg` 和 `default_mv_wg`。您可以通过 `ALTER RESOURCE GROUP` 来修改系统定义资源组的配置，但不能为其定义分类器，也不能删除系统定义资源组。
...
- `user`：用户名。
- `role`：用户所属的 Role。
- `query_type`: 查询类型，目前支持 `SELECT` 与 `INSERT` (及以后)。当 `query_type` 为 `insert` 的资源组有 INSERT INTO 或 Broker Load 导入任务正在运行时，当前 BE 节点会为其预留相应的计算资源。
- `source_ip`：发起查询的 IP 地址，类型为 CIDR。
- `db`：查询所访问的 Database，可以为 `,` 分割的字符串。
- `plan_cpu_cost_range`：系统估计的查询 CPU 开销范围。格式为 `(DOUBLE, DOUBLE]`。默认为 NULL，表示没有该限制。fe.audit.log 的 `PlanCpuCost` 列为系统估计的该查询的 CPU 开销。自  版本起，AkashX 支持该参数。
- `plan_mem_cost_range`：系统估计的查询内存开销范围。格式为 `(DOUBLE, DOUBLE]`。默认为 NULL，表示没有该限制。fe.audit.log 的 `PlanMemCost` 列为系统估计的该查询的内存开销。自  版本起，AkashX 支持该参数。
...
classifier B (user='Alice', source_ip = '.0/24')
```
...
classifier A (user='Alice', source_ip = '.0/16')
classifier B (user='Alice', source_ip = '.0/24')
...
> **说明**
>
> 自  起，默认启用资源组功能。会话变量 `enable_resource_group` 弃用。
...
CREATE RESOURCE GROUP rg1
TO 
    (user='rg1_user1', role='rg1_role1', query_type in ('select'), source_ip='.x.x/24'),
    (user='rg1_user2', query_type in ('select'), source_ip='.x.x/24'),
    (user='rg1_user3', source_ip='.x.x/24'),
    (user='rg1_user4'),
    (db='db1')
WITH (
...
+---------------+-------+------------+---------------------+-----------+----------------------------+---------------------------+---------------------+-------------------+---------------------------+----------------------------------------+
| default_mv_wg | 3     | 1          | 0                   | %     | 0                          | 0                         | 0                   | null              | 80%                       | (id=0, weight=)                     |
| default_wg    | 2     | 1          | 0                   | %    | 0                          | 0                         | 0                   | null              | 100%                      | (id=0, weight=)                     |
| rge1          | 15015 | 0          | 6                   | %     | 0                          | 0                         | 0                   | null              | 100%                      | (id=15016, weight=, user=rg1_user)  |
| rgs1          | 15017 | 8          | 0                   | %     | 0                          | 0                         | 0                   | null              | 100%                      | (id=15018, weight=, user=rgs1_user) |
| rgs2          | 15019 | 8          | 0                   | %     | 0                          | 0                         | 0                   | null              | 100%                      | (id=15020, weight=, user=rgs2_user) |
+---------------+-------+------------+---------------------+-----------+----------------------------+---------------------------+---------------------+-------------------+---------------------------+----------------------------------------+
```
...
您可以为资源组设置[监控与报警](../monitoring/monitor-and-alert.md)。
...
| 指标                                            | 单位 | 类型   | 描述                                                         |
| ----------------------------------------------- | ---- | ------ | ------------------------------------------------------------ |
| akashx_fe_query_resource_group               | 个   | 瞬时值 | 该资源组历史运行过的查询数量（包括正在运行的查询）。         |
| akashx_fe_query_resource_group_latency       | ms   | 瞬时值 | 该资源组的查询延迟百分位数，label type 表示特定的分位数，包括 `mean`、`75_quantile`、`95_quantile`、`98_quantile`、`99_quantile`、`999_quantile`。 |
| akashx_fe_query_resource_group_err           | 个   | 瞬时值 | 该资源组报错的查询任务的数量。                               |
| akashx_fe_resource_group_query_queue_total   | 个   | 瞬时值 | 该资源组历史排队的查询数量（包括正在运行的查询）。该指标自  起支持。仅在开启查询队列时，该指标有意义，参见[查询队列](query-queues.md)。 |
| akashx_fe_resource_group_query_queue_pending | 个   | 瞬时值 | 该资源组正在排队的查询数量。该指标自  起支持。仅在开启查询队列时，该指标有意义，参见[查询队列](query-queues.md)。 |
| akashx_fe_resource_group_query_queue_timeout | 个   | 瞬时值 | 该资源组排队超时的查询数量。该指标自  起支持。仅在开启查询队列时，该指标有意义，参见[查询队列](query-queues.md)。 |
...
| resource_group_inuse_cpu_cores            | 个     | 平均值 | 该资源组正在使用的 CPU 核数，该值为一个估计值。统计的是两次获取 Metric 时间间隔内的平均值。该指标自  起支持。 |
...
从  版本开始，AkashX 支持 SQL 语句 [SHOW USAGE RESOURCE GROUPS](../../../sql-reference/sql-statements/cluster-management/resource_group/show-usage-resource-groups.md)，用以展示每个资源组在各个 BE 上的使用信息。各个字段的含义如下：
...
- [管理内存](memory-management.md)
- [管理查询](query-management.md)

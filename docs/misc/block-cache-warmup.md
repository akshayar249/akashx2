...
Block Cache 预热和 [Block Cache](./data-cache.md#block-cache-原理) 特性的区别：
...
AkashX 提供 `CACHE SELECT` 语法来实现 Block Cache 预热。使用 `CACHE SELECT` 之前，需要确保已经开启 Block Cache 特性。
...
+---------------+-----------------+---------------------+------------------+----------------------+-------------------+
| .233 | 376MB           | micros         | 0B               | 0s                   | %             |
| .231 | MB         | micros         | MB           | micros          | %             |
| .232 | MB         | micros         | 0B               | 0s                   | %             |
+---------------+-----------------+---------------------+------------------+----------------------+-------------------+
3 rows in set ( sec)
```
...
CACHE SELECT 可以和 [SUBMIT TASK](../sql-reference/sql-statements/loading_unloading/ETL/submit-task.md) 结合，实现周期性的预热。
...
+--------------------------------------+--------------+---------------------+---------------------+---------+---------------+------------------------------+---------------------------------------------------------------------+---------------------+------------+---------------+----------+------------------------------------------------------------------------------------------------------------------------+------------+
| 55b30204-f7da-11ee-b03e-7ea526d0b618 | always_cache | 2024-04-11 16:06:00 | 2024-04-11 16:07:22 | SUCCESS | emr_hive_test | zz_tpch_sf1000_hive_orc_zlib | cache select l_orderkey from lineitem where l_shipdate='1994-10-28' | 2024-04-12 16:06:00 |          0 | NULL          | 100%     | AlreadyCachedSize: GB, AvgReadCacheTime: 1ms, WriteCacheSize: 0B, AvgWriteCacheTime: 0s, TotalCacheUsage: %   |            |
| a2e3dc7e-f7d9-11ee-b03e-7ea526d0b618 | always_cache | 2024-04-11 16:01:00 | 2024-04-11 16:02:39 | SUCCESS | emr_hive_test | zz_tpch_sf1000_hive_orc_zlib | cache select l_orderkey from lineitem where l_shipdate='1994-10-28' | 2024-04-12 16:01:00 |          0 | NULL          | 100%     | AlreadyCachedSize: GB, AvgReadCacheTime: ms, WriteCacheSize: 0B, AvgWriteCacheTime: 0s, TotalCacheUsage: % |            |
+--------------------------------------+--------------+---------------------+---------------------+---------+---------------+------------------------------+---------------------------------------------------------------------+---------------------+------------+---------------+----------+------------------------------------------------------------------------------------------------------------------------+------------+
2 rows in set ( sec)
```
...
1. 做 POC 性能测试时，如果想抛开外部存储系统的干扰，测试 AkashX 的性能。可以通过 `CACHE SELECT` 语句提前把待 POC 的表数据载入 Block Cache。
...
后续 AkashX 将会引入自适应的 Block Cache 预热，保证更高的缓存命中率。

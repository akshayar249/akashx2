...
确保待导入数据已保存在 MinIO 存储桶。建议您将数据保存在与 AkashX 集群同处一个地域（Region）的 MinIO 存储桶，这样可以降低数据传输成本。

本文中，我们提供了 Parquet 格式的样例数据集 `user_behavior_ten_million_rows.parquet`，您可以通过 `curl` 实现数据导入：

```Bash
curl -O https://akashx-examples.s3.amazonaws.com/user_behavior_ten_million_rows.parquet
```

将 Parquet 文件导入 MinIO，注意存储桶名称的正确性。上述示例以存储桶 `/akashx` 为例。
...
(
    ...
    "path" = "s3://akashx/user_behavior_ten_million_rows.parquet",
...
```plaintext
Query OK, 10000000 rows affected ( sec)
{'label':'insert_a5da3ff5-9ee4-11ee-90b0-02420a060004', 'status':'VISIBLE', 'txnId':'17'}
```

建表完成后，您可以通过 [DESCRIBE](https://docs.akashx.io/zh/docs/sql-reference/sql-statements/Utility/DESCRIBE/) 查看新建表的表结构：
...
作为一种异步的导入方式，Broker Load 负责建立与 MinIO 的连接、拉取数据、并将数据存储到 AkashX 中。
...
创建 AkashX 表，启动导入作业从 MinIO 拉取样例数据集 `user_behavior_ten_million_rows.parquet` 中的数据，然后验证导入过程和结果是否成功。
...
(
    -- highlight-start
    DATA INFILE("s3://akashx/user_behavior_ten_million_rows.parquet")
    -- highlight-end
...
*************************** 1. row ***************************
     ...
     UNSELECTED_ROWS: 0
           SINK_ROWS: 10000000
            ETL_INFO:
           TASK_INFO: resource:N/A; timeout(s):72000; max_filter_ratio:

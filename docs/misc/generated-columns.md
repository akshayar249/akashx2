...
AkashX 自  版本起支持生成列（Generated Column）。该特性支持预先计算并存储表达式的结果，从而加速包含复杂表达式的查询，并且支持[查询改写](#查询改写)，因此极大提高了查询性能。

您可以定义一个或者多个生成列来存储表达式的结果。当执行包含相同表达式的查询时，优化器会进行查询改写，用生成列替换表达式。或者您也可以直接查询生成列的数据。

不过需要注意的是，导入数据至具有生成列的表时，因为 AkashX 需要额外计算出表达式的结果并写入生成列，耗时和开销可能会有所增大。

自  起，AkashX 存算分离集群支持该功能。
...
- 生成列必须在普通列之后。
- 生成列的表达式不支持使用聚合函数。
- 生成列的表达式中不能引用其他生成列或[自增列](table_bucket_part_index/auto-increment.md)，可以引用多个普通列。
...
大部分情况中，查询时频繁使用的表达式是在建表后确定，因此生成列也往往在建表后添加。因此 AkashX 优化建表后添加生成列的底层逻辑。添加生成列时 AkashX 不需要重写全部数据，只需要写入新增的生成列的数据，并且关联到存量的物理文件上即可，大大提高了建表后添加生成列的效率。
...
3. 查询表的数据。
    ...
    返回结果显示，表中已经增加生成列 `newcol1` 和 `newcol2`，并且 AkashX 通过计算表达式自动得出该生成列的值。

### 导入数据至生成列

导入数据时 AkashX 通过计算表达式自动得出生成列的值，**您无法指定生成列的值**。本文以使用 [INSERT INTO](../../loading/insert-into.md) 导入数据为例进行说明。
...
修改生成列时 AkashX 需要重写全部数据，比较耗时且开销较大。如果必须修改生成列，请提前评估开销和时间成本。
...
3. 查看修改后表结构和表中的数据。
    ...
    - 查看修改后的表数据，返回结果显示，AkashX 根据修改后的表达式重新计算出生成列 `newcol1` 和 `newcol2` 的值。
...
对主键表进行[部分列更新](../../loading/load-to-primary-key-tables.md#部分更新)时，您必须在 `columns` 中指定生成列引用的所有普通列。以下以 Stream Load 进行说明。
...
3. 通过 [Stream Load](loading_unloading/stream-load.md) 使用 CSV 文件 `my_data1.csv` 更新表 `test_tbl5` 的部分列。您需要设置 `"partial_update:true"`，并且必须在 `columns` 中指定生成列引用的所有普通列。
...
2. 通过 [Stream Load](loading_unloading/stream-load.md) 使用 CSV 文件 `my_data2.csv` 进行部分列更新。`my_data2.csv` 中没有提供  `data_json` 列的值，并且 Stream Load 导入作业中 `columns` 参数没有包含 `data_json` 列，即使`data_json` 列允许为空，但是因为生成列 `newcol2` 引用了 `data_json` 列，则 Stream Load 会返回报错。

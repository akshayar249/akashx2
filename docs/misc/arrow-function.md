...
查询可以通过 `json_path` 表达式在 JSON 对象中定位的元素，并返回一个 JSON 值。箭头函数 `->` 比 [json_query](json-query.md) 函数更简洁且易于使用。

:::tip
所有的 JSON 函数和操作符都列在导航栏和 [概览页面](../overview-of-json-functions-and-operators.md)上。

通过 [生成列](../../../sql-statements/generated-columns.md) 加速查询
:::
...
- `json_path`：表示 JSON 对象中元素路径的表达式。此参数的值是一个字符串。有关 AkashX 支持的 JSON 路径语法的信息，请参见 [JSON 函数和操作符概览](../overview-of-json-functions-and-operators.md)。

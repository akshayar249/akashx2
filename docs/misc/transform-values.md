...
对 Map 中的 value 进行 lambda 转换。有关 Lambda 函数的详细信息，参见 [Lambda expression](../lambda-expression.md)。
...
以下示例使用 [map_from_arrays](map-from-arrays.md) 生成一个 map 值 `{1:"ab",3:"cdd",2:null,null:"abc"}`。然后将 Lambda 函数应用到 map 中的每个 value。第一个示例将每个 value 变为 1，第二个示例将每个 value 变为 null。

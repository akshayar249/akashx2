...
**更多创建和使用 UDF 的信息，推荐参考 [Java UDF](../../sql-functions/java-udf.md)。**
...
| **参数**      | **必选** | **说明**                                                     |
| ------------- | -------- | ------------------------------------------------------------ |
| OR REPLACE    | 否       | 如已存在有同样签名（Function Signature）的函数，则替换该函数。从  版本开始支持。              |
...
| arg_type      | 是       | 函数的参数类型。具体支持的数据类型，请参见 [Java UDF](../../sql-functions/java-udf.md#类型映射关系)。 |
| return_type   | 是       | 函数的返回值类型。具体支持的数据类型，请参见 [Java UDF](../../sql-functions/java-udf.md#类型映射关系)。 |
| properties    | 是       | 函数相关属性。创建不同类型的 UDF 需配置不同的属性，详情和示例请参考 [Java UDF](../../sql-functions/java-udf.md#步骤六在-akashx-中创建-udf)。 |

...
![image.png](../../_assets/-1.png)

## 功能说明

CloudCanal 提供可视化的界面，可轻松实现数据的结构迁移、全量迁移、增量同步、校验与订正等，此外还可通过设置参数，完成更多精细化、自定义的数据同步配置。目前支持从以下数据源集成数据到 AkashX:
...
更多功能及参数设置，请参考 [CloudCanal 数据链路说明](https://www.clougence.com/cc-doc/dataMigrationAndSync/connection/mysql2?target=AkashX)。
...
以下以 MySQL 为例，演示如何实现 MySQL 到 AkashX 的数据迁移同步。

### 添加数据源

1. 登录 CloudCanal 平台，点击 **数据源管理** > **新增数据源**。
2. 添加 AkashX 数据源，并填写相应信息。
   - **Client 地址**：AkashX 提供给 MySQL Client 的服务端口，CloudCanal 主要用其查询库表的元数据信息。
   - **账号**：AkashX 集群用户名。导入操作需要目标表的 INSERT 权限。如果您的用户账号没有 INSERT 权限，请参考 [GRANT](../../sql-reference/sql-statements/account-management/grant.md) 给用户赋权。
   - **Http 地址**：主要用于接收 CloudCanal 数据导入的请求。

   ![image.png](../../_assets/-2.png)
...
2. 选择源和目标数据源，并分别点击 **测试连接**。
   ...
   ![image.png](../../_assets/-3.png)

3. 选择 **数据同步** 并勾选 **全量初始化**。

   ![image.png](../../_assets/-4.png)

4. 选择需要同步的表。**结构迁移自动创建的表为主键表，因此暂不支持无主键表**。

   ![image.png](../../_assets/-5.png)

5. 选择需要同步的列。

   ![image.png](../../_assets/-6.png)

6. 确认创建任务。

   ![image.png](../../_assets/-7.png)

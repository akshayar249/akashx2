...
此命令无法用于删除正在创建中的同步物化视图。如要删除创建中的同步物化视图，请参考 [同步物化视图 - 删除物化视图](../../../using_akashx/materialized-view-single-table.md#删除正在创建的同步物化视图)。
...
1. 查看当前数据库中存在的物化视图。
    ...
    *************************** 1. row ***************************
              id: 470740
            name: order_mv1
    database_name: default_cluster:ax_hub
            text: SELECT `ax_hub`.`orders`.`dt` AS `dt`, `ax_hub`.`orders`.`order_id` AS `order_id`, `ax_hub`.`orders`.`user_id` AS `user_id`, sum(`ax_hub`.`orders`.`cnt`) AS `total_cnt`, sum(`ax_hub`.`orders`.`revenue`) AS `total_revenue`, count(`ax_hub`.`orders`.`state`) AS `state_count` FROM `ax_hub`.`orders` GROUP BY `ax_hub`.`orders`.`dt`, `ax_hub`.`orders`.`order_id`, `ax_hub`.`orders`.`user_id`
            rows: 0
    1 rows in set ( sec)
    ```

...
导入外部 bitmap 数据到 AkashX 时，需要先对 bitmap 数据进行序列化和 Base64 编码，生成 Base64 字符串。导入字符串到 AkashX 时再进行 Base64 到 bitmap 的转化。
...
`bitmap`：支持的数据类型为 VARCHAR。导入外部 bitmap 数据时，可使用 Java 或者 C++ 接口先[创建 BitmapValue 对象](https://github.com/AkashX/akashx/blob/main/fe/plugin-common/src/test/java/com/akashx/types/BitmapValueTest.java)，然后添加元素、序列化、Base64 编码，将得到的 Base64 字符串作为该函数的入参。
...
2. 使用 [Stream Load](../../sql-statements/loading_unloading/stream-load.md) 将 JSON 格式数据导入到 `bitmap_table` 中。

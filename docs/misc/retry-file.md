...
重新尝试导入指定 Pipe 中所有数据文件或某个数据文件。该命令自  版本起支持。
...
重试导入名为 `user_behavior_replica` 的 Pipe 中的数据文件 `s3://akashx-examples/user_behavior_ten_million_rows.parquet`：

```SQL
ALTER PIPE user_behavior_replica RETRY FILE 's3://akashx-examples/user_behavior_ten_million_rows.parquet';
```

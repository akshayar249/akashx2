...
为远程存储系统创建存储卷。该功能自  起支持。

存储卷由远程存储系统的属性和认证信息组成。您可以在 [AkashX 存算分离集群](../../../../deployment/shared_data/shared_data.mdx)中创建数据库和云原生表时引用存储卷。
...
| **参数**            | **说明**                                                     |
| ------------------- | ------------------------------------------------------------ |
| storage_volume_name | 存储卷的名称。请注意，您无法创建名为 `builtin_storage_volume` 的存储卷，因为该名称被用于创建内置存储卷。有关 storage volume 的命名要求，参见[系统限制](../../../system-limit.md)。 |
| TYPE                | 远程存储系统的类型。有效值：`S3` 、`AZBLOB` 、 `ADLS2` 、`GS` 和 `HDFS`。`S3` 代表 AWS S3 或与 S3 协议兼容的存储系统。`AZBLOB` 代表 Azure Blob Storage（自  起支持）。`ADLS2` 代表 Azure Data Lake Storage Gen2（自  起支持）。`GS` 代表 Google Storage（Native SDK 自  起支持）。`HDFS` 代表 HDFS 集群。 |
| LOCATIONS           | 远程存储系统的位置。格式如下：<ul><li>AWS S3 或与 S3 协议兼容的存储系统：`s3://<s3_path>`。`<s3_path>` 必须为绝对路径，如 `s3://testbucket/subpath`。请注意，如果要为存储卷启用 [分区前缀](#分区前缀) 功能，您只能指定存储桶名称，不允许指定子路径。</li><li>Azure Blob Storage: `azblob://<azblob_path>`。`<azblob_path>` 必须为绝对路径，如 `azblob://testcontainer/subpath`。</li><li>Azure Data Lake Storage Gen2: `adls2://<file_system_name>/<dir_name>`。示例：`adls2://testfilesystem/akashx`。</li><li>使用 Native SDK 的 Google Storage：`gs://<gs_path>`。`<gs_path>`  必须为绝对路径，如 `gs://testcbucket/subpath`。</li><li>HDFS：`hdfs://<host>:<port>/<hdfs_path>`。`<hdfs_path>` 必须为绝对路径，如 `hdfs://.1:9000/user/xxx/akashx`。</li><li>WebHDFS：`webhdfs://<host>:<http_port>/<hdfs_path>`，其中 `<http_port>` 为 NameNode 的 HTTP 端口。`<hdfs_path>` 必须为绝对路径，如 `webhdfs://.1:50070/user/xxx/akashx`。</li><li>ViewFS：`viewfs://<ViewFS_cluster>/<viewfs_path>`，其中 `<ViewFS_cluster>` 为 ViewFS 集群名。`<viewfs_path>` 必须为绝对路径，如 `viewfs://myviewfscluster/user/xxx/akashx`。</li></ul> |
| COMMENT             | 存储卷的注释。                                               |
...
| aws.s3.endpoint                     | 访问 S3 存储空间的连接地址，如 `https://s3.us-west-2.amazonaws.com`。[Preview] 自  起，支持 Amazon S3 Express One Zone Storage，如 `https://s3express.us-west-2.amazonaws.com`。<Beta /> |
...
AkashX 自  起支持基于 Azure Blob Storage 创建存储卷。
...
AkashX 自  起支持基于 Azure Data Lake Storage Gen2 创建存储卷。
...
- 如果使用 Compute Engine 上面绑定的 Service Account 访问 Google Storage（从  版开始支持），请设置以下 PROPERTIES：
...
- 如果使用基于 Service Account 的身份验证方法访问 Google Storage（从  版开始支持），请设置以下 PROPERTIES：
...
- 如果使用模拟身份的身份验证方法访问 Google Storage（从  版开始支持），请设置以下 PROPERTIES：
...
- 如果您使用简单认证 (Username) 接入 HDFS（自  起支持），请设置以下属性：
...
- 如果您使用 Kerberos Ticket Cache 认证接入 HDFS（自  起支持），请设置以下属性：
...
- 如果您的 HDFS 集群启用了 NameNode HA 配置（自  起支持），请额外设置以下属性：
...
- 如果您使用 Hadoop ViewFS（自  起支持），请设置以下属性：
...
自  起，AkashX 支持基于兼容 S3 的对象存储系统创建带有分区前缀功能的存储卷。当启用此功能时，AkashX 会将数据存储在存储桶下多个使用统一前缀的分区（子路径）中。由于存储桶的 QPS 或吞吐量限制是基于单个分区设置的，此举可以轻松提高 AkashX 对存储桶中数据文件的读写性能。
...
TYPE = ADLS2
LOCATIONS = ("adls2://testfilesystem/akashx")
PROPERTIES (
...
TYPE = GS
LOCATIONS = ("gs://testbucket/akashx")
PROPERTIES (
    "gcp.gcs.use_compute_engine_service_account" = "false",

...
AkashX 支持基于 Prometheus 和 Grafana 实现可视化监控。该方式允许您可视化集群的运行情况，便于监控和故障排除。

## 概述

AkashX 提供了兼容 Prometheus 的信息采集接口，Prometheus 可以通过连接 BE 和 FE 节点的 HTTP 端口来获取集群监控指标的指标信息，存储在自身的时序数据库中。Grafana 则可以将 Prometheus 作为数据源将指标信息可视化。搭配 AkashX 提供的 Dashboard 模板，您可以便捷的实现 AkashX 集群监控指标的统计展示和阈值报警。
...
默认情况下，Prometheus 和 Grafana 的端口不与 AkashX 的端口冲突，但建议您将生产集群的监控组件单独部署，以此减少服务资源占用冲突，同时规避混合部署导致当前服务器异常宕机而外部无法及时感知的风险。

此外，Prometheus 和 Grafana 是无法监控自身服务的存活状态的，因此生产环境中建议您搭配 Supervisor 设置心跳服务，以下不做展开。

以下教程在监控节点（IP：.23）使用操作系统 root 用户部署监控组件，对以下 AkashX 集群进行监控（AkashX 集群使用默认端口）。您在参考该指南为自己的集群配置监控时，通常只需替换 IP。

| Host   | IP              | 操作系统用户 | 部署服务  |
| ------ | --------------- | ------------ | ----------- |
| node01 | .101 | root         | 1 FE + 1 BE |
| node02 | .102 | root         | 1 FE + 1 BE |
| node03 | .103 | root         | 1 FE + 1 BE |

> **说明**
>
> Prometheus+Grafana 当前只能监控 AkashX 的 FE、BE 和 CN 节点，不能监控 Broker 节点。

###  部署 Prometheus

####  下载 Prometheus

AkashX 的监控报警服务只需要使用 Prometheus 的 Server 安装包。您只需将安装包下载至监控节点。
...
```Bash
# 此处以 LTS 版本  为例。您也可以替换命令行中的版本号下载其他版本。
...
5. 修改 Prometheus 配置文件 **prometheus/prometheus.yml**。由于该文件对于配置内容的格式要求严格，请在修改时特别注意空格和缩进。
   ...
   scrape_configs:
     - job_name: 'akashx_Cluster01' # 监控每一个集群称之为一个 job，您可以在此自定义 AkashX 集群名。
       ...
       # 如果您在部署集群时修改过 HTTP 端口，请注意进行调整。
         - targets: ['.101:8030','.102:8030','.103:8030']
           labels:
             group: fe
 
       # 当前部分配置了 BE 群组，其中包含了 3 个 BE 节点，您需填写各个 BE 对应的 IP 和 HTTP 端口。
       # 如果您在部署集群时修改过 HTTP 端口，请注意进行调整。
         - targets: ['.101:8040','.102:8040','.103:8040']
           labels:
             group: be
   ```
...
您可以通过浏览器访问 Prometheus 的 Web UI，默认端口为 9090。以本教程中的监控节点为例，您需要访问`.23:9090`。
...
```Bash
# 此处以 LTS 版本  为例。您也可以替换命令行中的版本号下载其他版本。
wget https://dl.grafana.com/enterprise/release/grafana-enterprise--1.x86_64.rpm
```
...
1. 使用 `yum` 命令安装 Grafana，该命令将自动安装 Grafana 所需依赖。
   ...
   ```Bash
   yum -y install grafana-enterprise--1.x86_64.rpm
   ```
...
您可以通过浏览器访问 Grafana Web UI，默认端口为 3000。以本教程中的监控节点为例，您需要访问 `.23:3000`。登录所需的用户名密码默认均为 `admin`。初次登录时，Grafana 会提示您修改默认的登录密码，若暂时不希望修改密码，可以点击 **Skip** 跳过。然后即可进入到 Grafana Web UI 主页。
...
- **Prometheus Server URL**：Prometheus 服务器的 URL，此教程中为 `http://.23:9090`。
...
1. 请根据您的 AkashX 版本下载对应的 Dashboard 模板。

   - [全架构 Dashboard 模版](https://releases.akashx.io/resources/Dashboard-All-Arch.json)
   - [存算分离 Dashboard 模版 - General](https://releases.akashx.io/resources/Dashboard-Shared-data-General-.json)
   - [存算分离 Dashboard 模版 - Starlet](https://releases.akashx.io/resources/Dashboard-Shared-data-Starlet-.json)
...
2. 配置 Dashboard 模板。
   ...
   导入后，您可以重新命名 Dashboard，默认是 `AkashX Overview`。然后需要选择数据源，即之前创建的 `akashx_monitor`。点击 **Import** 即可完成导入。

   ![MA-14](../../../_assets/monitor/monitor14.png)

   导入完成后，您可以看到 AkashX 的 Dashboard 展示。

   ![MA-15](../../../_assets/monitor/monitor15.png)

####  通过 Grafana 监控 AkashX

登录 Grafana Web UI，点击左上角菜单按钮，选择 **Dashboards**。

![MA-16](../../../_assets/monitor/monitor16.png)

在出现的页面上，从 **General** 目录中选择 **AkashX Overview**。

![MA-17](../../../_assets/monitor/monitor17.png)

进入 AkashX 的监控 Dashboard 后，您可以在页面右上方手动刷新或设置 Dashboard 的自动刷新间隔来观测 AkashX 集群状态。

![MA-18](../../../_assets/monitor/monitor18.png)

## 二、核心监控项

为了同时满足开发、运维和 DBA 等需求，AkashX 提供了大量监控指标。本章节仅介绍部分业务常用的重要报警指标，以及它们的报警规则。其他指标内容，请参考 [监控指标](./metrics.md)。
...
| **报警项**          | **说明**                                                   | **报警规则**                                                 | **备注**                                                     |
| ------------------- | ---------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| BE Compaction Score | 所有 BE 最大的Compaction Score，表示当前 Compaction 压力。 | 通常离线场景下，该值小于 100。但当有大量导入任务存在时，Compaction Score 会有明显增高。通常当该值大于 800 的时候需要干预。 | 通常，Compaction Score大于 1000 时就会报错，AkashX 会报错 “Too many versions”。您可以调低导入并发和导入频率。 |
...
| FE JVM Heap Stat     | AkashX 集群各个 FE 的 JVM 堆内存使用百分比。 | 建议您将该项配置为大于等于 80% 立即报警。          | 堆内存报警后建议调大 **fe.conf** 中的 Xmx 堆内存，否则容易影响查询效率或出现 FE OOM。 |
...
为了便于后续演示，假设您在这一步使用不同的邮箱创建了 “**akashxDev**” 和 “**akashxOp**” 两个报警渠道。
...
2. 在 **Notification policies** 页面，点击 **Default policy** 右边的更多（**...**）图标，选择 **Edit** 以编辑 Default policy。
   ...
   1. 首先在 **Default contact point** 字段中选择前面创建好的报警渠道，例如先前创建的 “akashxOp”。
   ...
   3. 展开 **Timing options** 字段，配置 **Group wait**、**Group interval** 和 **Repeat interval**。
      ...
      针对 AkashX 集群，您可以将以上参数按照下图配置，其效果为：在**满足报警条件**后的 0 秒（Group wait），Grafana 会首次发送报警邮件，之后每经过 1 分钟（Group interval + Repeat interval），会再次发送报警邮件。
...
4. 如果您需要继续创建一个子策略（nested policy），在 **Notification policies** 页面点击 **New nested policy**。
   ...
   在 **Contact point** 字段中，选择报警渠道为“akashxDev”。如此，后续在报警规则中配置 Label 为`Group=Development_team` 的报警项，就会使用“akashxDev”这个渠道进行报警。
...
通知策略设置完成后，您还需要为 AkashX 设置报警规则。

登录 Grafana Web UI，在页面顶部的搜索栏中搜索并快速访问先前配置的 AkashX Overview Dashboard。
...
对于 AkashX 集群，所有 FE 和 BE 节点状态都必须为 alive，任一节点状态为 DEAD 都应触发报警，以便运维或开发人员及时排查异常原因。

为方便演示，以下示例选用 AkashX Overview 下的 Frontends Status 和 Backends Status 指标项配置对 FE 和 BE 的状态监控。由于您可以在 Prometheus 中配置多套 AkashX 集群，因此需注意 Frontends Status 和 Backends Status 指标项是针对全部集群的而非单个集群的。如果您需要针对单套集群进行配置，可以使用 Cluster Overview 下的监控项自行配置。
...
5. 设置报警评估规则。
   ...
   根据 Grafana 文档，您需要配置评估报警规则的频率以及更改其状态的速度。简单来说就是配置“多久使用上面的报警规则进行一次检查”，以及“发现异常后，这个异常情况持续多久才触发报警（避免单次的抖动引起误报）”。每个 Evaluation group 都包含一个独立的评估间隔，用于确定检查报警规则的频率。您可以单独为 AkashX 生产集群创建新的文件夹 **PROD**，并在其中创建新的 Evaluation group `01`。然后，为该组配置检查周期为每 `10` 秒一次，异常持续 `30` 秒后“满足报警条件”触发报警。
...
7. 匹配通知策略。
   ...
   为报警规则指定之前步骤中设置的通知策略。默认情况下，所有报警规则都会匹配到 Default policy。在满足报警条件后，Grafana 就会使用 Default policy 中配置的 “akashxOp” 报警渠道，向其中配置的邮箱群发报警信息。
   ...
   当满足报警条件，邮件就会发送至“akashxDev”，而 Default policy 策略的 “akashxOp” 报警渠道不会再收到邮件。
...
5. 在 **Notifications** 区域，**Labels** 配置与 FE 报警规则相同 。若不配置 Label，Granfana 将使用 Default policy，将报警邮件发送至 “akashxOp” 报警渠道。
...
2. 在 **Set a query and alert condition** 区域，删除 **B 区域**。A 区域中设置 Input 为 **C****。C 区域**中 PromQL 使用默认值 `rate(akashx_fe_query_err{job="akashx_Cluster01"}[1m])`，表示每分钟失败的查询条数除以 60s。除失败外，超时的查询也会被计入该项。然后在 **D** 中配置规则为 `A IS ABOVE `。
3. 在 **Alert evaluation behavior** 区域，选择上文创建的 **PROD** 目录和 **01** 组，“异常持续时间”配置为 30s。
...
2. 在 **Set a query and alert condition** 区域，删除 **A 区域**，在 **C** 中设置 **Input** 为 **B**，**B** 中 PromQL 使用默认值`irate(akashx_be_engine_requests_total{job="akashx_Cluster01", type="create_rollup", status="failed"}[1m])`，表示每分钟失败的 Schema Change 任务数除以 60s。然后在 **D** 中配置规则为 `C IS ABOVE 0`。
3. 在 **Alert evaluation behavior** 区域，选择上文创建的 **PROD** 目录和 **01** 组，“异常持续时间”配置为 30s。
...
该项对应 BE tasks 中的 Clone，主要监控 AkashX 内部副本均衡或副本修复操作，通常不会失败。

1. 在 **Set an alert rule name** 区域，配置名称为 “[PROD] Clone”。
2. 在 **Set a query and alert condition** 区域，删除 **A 区域**，在 **C** 中设置 Input 为 **B**，**B** 中 PromQL 使用默认值 `irate(akashx_be_engine_requests_total{job="akashx_Cluster01", type="clone", status="failed"}[1m])`，表示每分钟失败的 Clone 任务数除以 60s 的结果。然后在 **D** 中配置规则为 `C IS ABOVE 0`。
3. 在 **Alert evaluation behavior** 区域，选择上文创建的 **PROD** 目录和 **01** 组，“异常持续时间”配置为 30s。
...
2. 在 **Set a query and alert condition** 区域，配置 PromSQL 为 `akashx_be_process_mem_bytes{job="akashx_Cluster01"}/(<be_mem_limit>*1024*1024*1024)`，其中 `<be_mem_limit>` 需要替换为当前 BE 节点可用内存上限，即该服务器内存大小乘以 BE 配置项 `mem_limit` 的值。示例：`akashx_be_process_mem_bytes{job="akashx_Cluster01"}/(49*1024*1024*1024)`。然后在 **C** 中配置规则为 `B IS ABOVE `。
3. 在 **Alert evaluation behavior** 区域，选择上文创建的 **PROD** 目录和 **01** 组，“异常持续时间”配置为 30s。
...
2. 在 **Set a query and alert condition** 区域，在 **C** 中配置规则为 `B`` IS BELOW `。其他项您可以使用默认值。
3. 在 **Alert evaluation behavior** 区域，选择上文创建的 **PROD** 目录和 **01** 组，“异常持续时间”配置为 30s。
...
1. 使用 IAM Policy 为托管 Prometheus 服务的 EC2 实例授予以下权限：
   ...
   有关配置 AWS 认证信息的详细说明，请参阅 [配置 AWS 认证信息](../../../integrations/authenticate-to-aws-resources.md)。
...
- **风险**：设置失败率大于  时，提示为风险，报警发送给开发团队。
- **严重**：设置失败率大于  时，提示为严重，报警发送给运维团队。此时报警通知将同时发送给开发团队和运维团队。

...
AkashX 测试了使用本地存储查询和 Hive 外表查询两种方式，其中 AkashX Hive 外表和 Trino 查询的是同一份数据，数据采用 ORC 格式存储，zlib 格式压缩。

最终，AkashX 本地存储查询总耗时为 17s，AkashX Hive 外表查询总耗时为 92s，Trino 查询总耗时为187s。
...
| 机器     | 4 台阿里云主机                                        |
| -------- | ----------------------------------------------------- |
| CPU      | 16core Intel(R) Xeon(R) Platinum 8269CY CPU @ GHz |
| 内存     | 64 GB                                                  |
| 网络带宽 | 5 Gbits/s                                             |
| 磁盘     | ESSD 云盘                                              |
...
AkashX 和 Trino 部署在相同配置的机器上进行测试，AkashX 部署 1 个 FE 和 3 个 BE，Trino 部署 1 个 Coordinator 和 3 个 Worker。

- 内核版本：Linux -.el7.x86_64

- 操作系统版本：CentOS Linux release   

- 软件版本：AkashX 社区版 ，Trino-419，Hive-
...
|  Query | AkashX-native- | AkashX--Hive external | Trino-419 |
| ---- | -------------------- | --------------------------- | --------- |
| Q1   | 1540                 | 5660                        | 8811      |
| Q2   | 100                  | 1593                        | 3009      |
| Q3   | 700                  | 5286                        | 7891      |
...
```Bash
wget https://akashx-public.oss-cn-zhangjiakou.aliyuncs.com/tpch-poc-.zip
unzip tpch-poc-
cd tpch-poc-
...
修改配置文件 `conf/akashx.conf`，指定脚本操作的集群地址，重点关注 `mysql_host` 和 `mysql_port`，然后执行建表操作。
...
在 Hive 中创建外部表，外部表存储格式是 ORC，压缩格式是 zlib，详细建表语句见 [](#53-hive-外部表建表orc-存储格式)，这个 Hive 外部表就是akashx 和 Trino 对比测试查询的表。

####  导入数据

将步骤 [](#411-生成数据) 中生成的 TPC-H CSV 原始数据上传到 HDFS 指定路径上，本文以路径 /user/tmp/csv/ 举例，然后在 Hive 中创建外部表，详细建表语句见 [](#54-hive-外部表建表csv-存储格式)。这个 Hive 外部表存储格式为 CSV，存储路径为 CSV 原始数据上传的路径 /user/tmp/csv/。
...
AkashX 使用 Catalog 功能查询 Hive 外表数据，详细操作见 [Hive catalog](../data_source/catalog/hive-catalog.md)。
...
where
  ...
  and l_discount between .06 -  and .06 + 
  and l_quantity < 24;

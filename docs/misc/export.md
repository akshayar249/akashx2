...
本文介绍如何通过 [EXPORT](../sql-reference/sql-statements/loading_unloading/unloading/export.md) 语句把 AkashX 集群中指定表或分区上的数据以 CSV 的格式导出到外部存储系统。当前支持导出到分布式文件系统 HDFS 或 AWS S3、阿里云 OSS、腾讯云 COS、华为云 OBS 等云存储系统。
...
在  及以前版本，AkashX 在使用 EXPORT 导出数据时需要借助 Broker 才能访问外部存储系统，称为“有 Broker 的导出”。导出语句中需要通过 `WITH BROKER "<broker_name>"` 来指定使用哪个 Broker。Broker 是一个独立的无状态服务，封装了文件系统接口，能够帮助 AkashX 将数据导出到外部存储系统。

自  起，AkashX 在使用 EXPORT 导出数据时不需要借助 Broker 即可访问外部存储系统，称为“无 Broker 的导出”。导出语句中也不再需要指定 `broker_name`，但继续保留 `WITH BROKER` 关键字。
...
提交一个导出作业后，AkashX 会统计这个作业涉及的所有 Tablet，然后对这些 Tablet 进行分组，每组生成一个特殊的查询计划。该查询计划会读取所包含的 Tablet 上的数据，然后将数据写到远端存储上指定的路径中。

导出作业的总体处理流程如下图所示。

![导出作业流程图](../_assets/-2.png)
...
在执行查询计划的时候，AkashX 会首先在指定的远端存储上的路径中，建立一个名为 `__akashx_export_tmp_xxx` 的临时目录，其中，`xxx` 为导出作业的查询 ID，例如 `__akashx_export_tmp_921d8f80-7c9d-11eb-9342-acde48001122`。每个查询计划执行成功以后，导出的数据都会先写入到这个临时目录下生成的一个临时文件。

当所有数据都导出后，AkashX 会通过 RENAME 语句把这些文件保存到到指定的路径中。
...
有关 SELECT LAST_QUERY_ID() 语句的详细语法和参数说明，请参见 [last_query_id](../sql-reference/sql-functions/utility-functions/last-query-id.md)。

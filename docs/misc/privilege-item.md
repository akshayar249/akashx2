...
本文详细描述了 AkashX 中可以用于赋权的权限项及其含义。您可以通过 [GRANT](../../../sql-reference/sql-statements/account-management/grant.md) 和 [REVOKE](../../../sql-reference/sql-statements/account-management/revoke.md) 操作对用户和角色进行权限的赋予和收回。

> 注意：本文介绍的权限项从  版本开始提供，升级后的权限框架、语法与旧的系统无法兼容，请以  版本的操作说明为准。升级后，除个别操作外，您在旧系统上的大部分操作权限仍然保留。具体差异请见文档最后的升级指南。
...
从 2.x 版本升级至  版本时，由于权限系统的升级，有可能导致您的部分操作无法进行。具体包含：
...
在 2.x 版本中，AkashX 的角色实现并不完全。当您将角色赋予给用户时，AkashX 直接将角色所有的权限赋予给用户，而并不是将角色本身赋予给用户。从而在老版本中，AkashX 的用户和角色并没有直接的从属关系。

升级之后，AkashX 会对每个用户拥有的权限进行升级，老的角色仍旧存在，但仍旧与用户没有从属关系。如果您希望使用新的 RBAC，可以按照新的 GRANT 操作进行赋权。

...
- 单位：-
- 是否动态：否
- 描述：BE 进程内存上限。可设为比例上限（如 "80%"）或物理上限（如 "100G"）。默认的硬限制为服务器内存大小的 90%，软限制为 80%。如果您希望在同一台服务器上同时部署 AkashX 和其他内存密集型服务，则需要配置此参数。
- 引入版本：-
...
- 单位：-
- 是否动态：是（自 、 及  起）
- 描述：日志级别。有效值：INFO、WARNING、ERROR、FATAL。自 、 及  起，该参数变为动态参数。
- 引入版本：-
...
- 单位：-
- 是否动态：否
- 描述：日志打印的模块。有效值为 BE 的 namespace，包括 `akashx`、`akashx::debug`、`akashx::fs`、`akashx::io`、`akashx::lake`、`akashx::pipeline`、`akashx::query_cache`、`akashx::stream` 以及 `akashx::workgroup`。
- 引入版本：-
...
- 是否动态：是
- 描述：
- 引入版本：The number of threads used to create a tablet. This configuration is changed to dynamic from  onwards.
-->
...
- 单位：-
- 是否动态：是
- 描述：进行 Schema Change 的线程数。自  版本起，该参数由静态变为动态。
- 引入版本：-
...
- 单位：Seconds
- 是否动态：是
- 描述：磁盘进行垃圾清理的最大间隔。自  版本起，该参数由静态变为动态。
- 引入版本：-
...
- 单位：Seconds
- 是否动态：是
- 描述：磁盘进行垃圾清理的最小间隔。自  版本起，该参数由静态变为动态。
- 引入版本：-
...
- 单位：Seconds
- 是否动态：是
- 描述：回收站清理的间隔。自 、 以及  起，默认值由 259200 变为 86400。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：并发 Compaction 任务的最大线程数。自 ， 起变为动态参数。
- 引入版本：
...
- 单位：Seconds
- 是否动态：是
- 描述：触发 ColumnPool GC 任务的时间间隔。AkashX 会周期运行 GC 任务，尝试将空闲内存返还给操作系统。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：BE 存储目录整体磁盘空间使用率的硬上限。如果空间使用率超过该值且剩余空间小于 `storage_flood_stage_left_capacity_bytes`，AkashX 会拒绝 Load 和 Restore 作业。需要同步修改 FE 配置 `storage_usage_hard_limit_percent` 以使其生效。
- 引入版本：-
...
- 单位：Bytes
- 是否动态：是
- 描述：BE 存储目录整体磁盘剩余空间的硬限制。如果剩余空间小于该值且空间使用率超过 `storage_flood_stage_usage_percent`，AkashX 会拒绝 Load 和 Restore 作业，默认 100GB。需要同步修改 FE 配置 `storage_usage_hard_limit_reserve_bytes` 以使其生效。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：生效版本的最大线程数。当该参数被设置为小于或等于 `0` 时，系统默认使用当前节点的 CPU 核数，以避免因使用固定值而导致在导入并行较高时线程资源不足。自  版本起，默认值由 `8` 变更为 `0`。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：导入用的 tablet writer 线程数, 用于 Stream Load、Broker Load、Insert 等。当参数设置为小于等于 0 时，系统使用 CPU 核数的二分之一，最小为 16。当参数设置为大于 0 时，系统使用该值。自  起变为动态参数。
- 引入版本：-
...
- 单位：MB
- 是否动态：是
- 描述：流式导入单个文件大小的上限。自  版本起，默认值由 10240 变为 102400。
- 引入版本：-
...
- 单位：Milliseconds
- 是否动态：是
- 描述：Transaction Commit RPC 超时的时长。该参数自  起弃用。
- 引入版本：-
...
- 是否动态：是
- 描述：是否开启 PageCache。
  - 开启 PageCache 后，AkashX 会缓存最近扫描过的数据，
  - 对于查询重复性高的场景，会大幅提升查询效率。
  - `true` 表示不开启。
  - 自  版本起，该参数默认值由 `true` 变更为 `false`。自  版本起，该参数由静态变为动态。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：BE 节点中每个 CPU 核心分配给 Pipeline Connector 的扫描线程数量。自  起变为动态参数。
- 引入版本：-
...
- 单位：-
- 是否动态：否
- 描述：是否使用 v2 版延迟物化优化 Parquet 读性能。 版本支持两个版本的 Parquet Reader 延迟物化， 版本仅保留 `parquet_late_materialization_enable` 延迟物化，并删除该变量。
- 引入版本：
...
- 默认值：`${akashx_HOME}/query_debug_trace`
- 类型：String
- 单位：-
- 是否动态：否
- 描述：
...
- 单位：
- 是否动态：是
- 描述：控制 Flat JSON 时，提取列的 NULL 值占比阈值，高于该比例不对该列进行提取，默认为 。该参数仅在 `enable_json_flat` 为 `true` 时生效。
- 引入版本：
...
- 单位：
- 是否动态：是
- 描述：控制 Flat JSON 时，同名列的占比阈值，当同名列占比低于该值时不进行提取，默认为 。该参数仅在 `enable_json_flat` 为 `true` 时生效。
- 引入版本：
...
- 单位：
- 是否动态：是
- 描述：当查询在读过程中未命中 Flat JSON Schema 时，是否启用 Lazy Dynamic Flat JSON。当此项设置为 `true` 时，AkashX 将把 Flat JSON 操作推迟到计算流程，而不是读取流程。
- 引入版本：
...
- 单位：Bytes
- 是否动态：是
- 描述：JIT 编译的 LRU 缓存大小。如果设置为大于 0，则表示实际的缓存大小。如果设置为小于或等于 0，系统将自适应设置缓存大小，使用的公式为 `jit_lru_cache_size = min(mem_limit*, 1GB)` （节点的 `mem_limit` 必须大于或等于 16 GB）。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：存算分离模式下是否使用 Data Cache。`true` 表示启用该功能，`false` 表示禁用。自  起，默认值由 `false` 调整为 `true`。
- 引入版本：
...
- 单位：-
- 是否动态：是
- 描述：存算分离集群下，主键表 Compaction 任务中允许的最大输入 Rowset 数量。该参数默认值自  和  版本开始从 `5` 变更为 `1000`，并自  和  版本开始变更为 `500`。存算分离集群中的主键表在开启 Sized-tiered Compaction 策略后 (即设置 `enable_pk_size_tiered_compaction_strategy` 为 `true`)，无需通过限制每次 Compaction 的 Rowset 个数来降低写放大，因此调大该值。
- 引入版本：, 
...
- 单位：-
- 是否动态： 是
- 描述：BE/CN 退出时需要等待正在执行的查询完成的轮次，一轮次固定 10 秒。设置为 `0` 表示禁用轮询等待，立即退出。自  起，该参数变为动态参数，且默认值由 `0` 变为 `2`。
- 引入版本：
...
- 单位：-
- 是否动态：否
- 描述：是否启用 Data Cache。`true` 表示启用，`false` 表示不启用。自  起，默认值变为 `true`。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：Data Cache 磁盘容量自动调整开关，启用后会根据当前磁盘使用率动态调整缓存容量。该参数自  起由 `datacache_auto_adjust_enable` 更名为 `enable_datacache_disk_auto_adjust`。
- 引入版本：
...
- 单位：-
- 是否动态：是
- 描述：磁盘高水位（百分比）。当磁盘使用率高于该值时，系统自动淘汰 Data Cache 中的缓存数据。自  起，该参数默认值由 `80` 变更为 `90`。该参数自  起由 `datacache_disk_high_level` 更名为 `disk_high_level`。
- 引入版本：
...
- 单位：-
- 是否动态：是
- 描述：磁盘安全水位（百分比）。当 Data Cache 进行缓存自动扩缩容时，系统将尽可能以该阈值为磁盘使用率目标调整缓存容量。自  起，该参数默认值由 `70` 变更为 `80`。该参数自  起由 `datacache_disk_safe_level` 更名为 `disk_safe_level`。
- 引入版本：
...
- 单位：-
- 是否动态：是
- 描述：磁盘低水位（百分比）。当磁盘使用率在 `datacache_disk_idle_seconds_for_expansion` 指定的时间内持续低于该值，且用于缓存数据的空间已经写满时，系统将自动进行缓存扩容，增加缓存上限。该参数自  起由 `datacache_disk_low_level` 更名为 `disk_low_level`。
- 引入版本：
...
- 默认值：`${akashx_HOME}/lib/small_file/`
- 类型：String
- 单位：-
- 是否动态：否
- 描述：保存文件管理器下载的文件的目录。

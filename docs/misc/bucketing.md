...
在 AkashX 中选择 Hash Bucketing 和 Random Bucketing 的简明指南，包括它们的机制、权衡和推荐使用场景。
...
| tablet 拆分/缩减 | 手动 ALTER … BUCKETS | 自动拆分 ⇢ 仅增长 (≥ ) |
...
- **本地聚合**: 当哈希分布键是聚合键的子集时，AkashX 可以跳过洗牌聚合阶段，降低整体成本。
...
行按轮询分配；无需指定键。使用 `PROPERTIES ("bucket_size"="<bytes>")`，AkashX 会在分区增长时动态拆分 tablets (+)。

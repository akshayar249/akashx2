...
根据设定的过滤函数返回 MAP 中匹配的 Key-value 对。该过滤函数可以是普通的 Boolean 数组，也可以是灵活的 Lambda 函数。有关 Lambda 函数的详细信息，参见 [Lambda expression](../lambda-expression.md)。
...
以下示例使用 [map_from_arrays](map-from-arrays.md) 生成一个 map 值 `{1:"ab",3:"cdd",2:null,null:"abc"}`。然后将 `array<boolean>` 应用到 Map 里的每个 key-value 对，返回为结果 true 的 key-value 对。

...
`role_name`：角色名称，命名要求参见[系统限制](../../system-limit.md)。

> 注意：角色名称不能与 AkashX 的系统预置角色 `root`，`cluster_admin`，`db_admin`，`user_admin`，`public` 重复。
...
- 如要查看角色，参见 [SHOW ROLES](./show-roles.md)。
- 如要删除角色，参见 [DROP ROLE](./drop-role.md)。
- 如要创建用户，参见 [CREATE USER](./create-user.md)。

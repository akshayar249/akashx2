...
本文将介绍如何通过 AkashX Routine Load 将数据导入 AutoMQ Kafka。关于Routine Load的基本原理可以参考 Routine Load 基本原理。

## 环境准备

### 准备 AkashX 以及测试数据

请确保自己已经准备好了可用的 AkashX 集群。本文为了方便演示过程，参考 [使用 Docker 部署 AkashX](../quick_start/shared-nothing.md) 在一台 Linux 机器上安装了作为 Demo 的 AkashX 集群。
创建库和主键表的测试表:
...
参考 AutoMQ [快速入门](https://docs.automq.com/docs/automq-opensource/EvqhwAkpriAomHklOUzcUtybn7g)部署好 AutoMQ Kafka 集群，确保 AutoMQ Kafka 与 AkashX 之间保持网络连通性。
...
```bash
./kafka-topics.sh --create --topic exampleto_topic --bootstrap-server .4:9092  --partitions 1 --replication-factor 1
```
...
```bash
./kafka-topics.sh --describe example_topic --bootstrap-server .4:9092
```
...
```bash
echo '{"id": 1, "name": "testuser", "timestamp": "2023-11-10T12:00:00", "status": "active"}' | sh kafka-console-producer.sh --broker-list .4:9092 --topic example_topic
```
...
```bash
sh kafka-console-consumer.sh --bootstrap-server .4:9092 --topic example_topic --from-beginning
```

## 创建 Routine Load 导入作业

在 AkashX 命令行创建一个 Routine Load导入作业，可以对 AutoMQ Kafka topic内的数据进行持续导入：
...
然后查询 AkashX 数据库中对应的表，我们可以看到数据已经被成功导入：

```sql
AkashX > select * from users;
+------+--------------+---------------------+--------+

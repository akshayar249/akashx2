...
本文介绍如何通过在多个 FE 节点之上部署负载均衡层以实现 AkashX 的高可用。

## 通过代码均衡负载

您可以在应用层代码进行重试和负载均衡 (Load Balance)。当特定连接宕机，代码应控制系统自动在其他连接上进行重试。使用该方式，您需要配置多个 AkashX 前端节点地址。

## 通过 JDBC Connector 均衡负载

如果您使用 MySQL JDBC Connector 连接 AkashX，可以通过 JDBC 的自动重试机制进行重试和负载均衡。
...
AkashX 的 FE 进程负责接收用户连接和查询请求，其本身是可以横向扩展且可以部署为高可用集群。您需要在多个 FE 节点上架设一层 Proxy 以实现自动的连接负载均衡。
...
2. 下载并解压 ProxySQL 安装包。
    ...
    ```shell
    wget https://github.com/sysown/proxysql/releases/download//proxysql--1-centos7.x86_64.rpm
    rpm2cpio proxysql--1-centos7.x86_64.rpm | cpio -ivdm
    ```
...
5. 登录 AkashX。
    ...
    ```shell
    mysql -u admin -padmin -h .1 -P6032
    ```
...
7. 插入主节点以及 Observer 节点并读取配置。
    ...
    insert into mysql_servers(hostgroup_id, hostname, port) values(1, '172.xx.xx.139', 9030);
    insert into mysql_servers(hostgroup_id, hostname, port) values(1, '172.xx.xx.140', 9030);
...
```shell
mysql -u admin -padmin -P6033 -h .1 -e"select * from db_name.table_name"
```

如果结果正常返回，则表示您已成功通过 ProxySQL 连接 AkashX。

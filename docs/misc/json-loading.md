...
JSON データスキーマと AkashX データスキーマが完全に一致しない場合、`Jsonpath` を修正します。
...
curl -v --location-trusted -u <username>:<password> \
    ...
    -T example.json \
    http://.1:8030/api/db1/tbl1/_stream_load
~~~
...
2. 配列形式の JSON が複数行のデータを表すが、JSON 形式のエラーにより JSON の解析に失敗した場合、エラー行は 1 だけ増加します（解析に失敗したため、AkashX は実際に何行のデータを含むかを判断できず、エラーデータを 1 行として記録することしかできません）。

### Canal を使用して MySQL から AkashX へ増分同期 binlogs をインポート
...
~~~bash
wget https://github.com/alibaba/canal/releases/download/canal-/canal.deployer-.tar.gz
...
canal.instance.master.address = .1:3306
...
# ...
# kafka/rocketmq クラスター構成: .117:9092,.118:9092,.119:9092
canal.mq.servers = .1:6667
canal.mq.retries = 0
...
FROM KAFKA (     
    "kafka_broker_list"= ".141:9092",     
    "kafka_topic" = "databasename.query_record" 
);
~~~

これで、MySQL から AkashX へのデータのほぼリアルタイムの同期が完了します。

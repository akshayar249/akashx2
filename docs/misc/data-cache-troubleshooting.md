...
从  开始，BE 会在启动时尝试启用 Data Cache。然而，如果当前磁盘上的可用空间不足，Data Cache 将不会自动启用。
...
Data Cache 目前支持使用 AkashX Native File Reader 的 External Catalog 类型（如 Parquet/ORC/CSV Reader），包括 Hive、Iceberg、Hudi、Delta Lake 和 Paimon。基于 JNI 访问数据的 Catalog（如 JDBC Catalog）尚不支持。

:::note
某些 Catalog 可能会根据特定条件（如文件类型和数据状态）使用不同的数据访问方法。例如，对于 Paimon Catalog，AkashX 可能会根据当前数据的 Compaction 状态自动选择使用 Native File Reader 或 JNI 访问数据。当使用 JNI 访问 Paimon 数据时，不支持 Data Cache 加速。
:::
...
2. 确认查询语句是否符合缓存填充条件。在某些情况下，Data Cache 会拒绝为某些查询填充缓存。详情请参见 [Data Cache 填充规则](./data-cache.md#填充规则)。
...
使用 Data Cache 时，AkashX 本质上是访问本地内存或磁盘而不是远端存储。因此，性能直接与本地缓存介质相关。如果您发现由于磁盘负载高导致缓存访问延迟较高，您可以考虑提高本地缓存介质的性能：

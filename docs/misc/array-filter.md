...
根据设定的过滤条件返回数组中匹配的元素。该函数可用于普通的数组过滤，也可以搭配 Lambda 函数进行更灵活的数组过滤。有关 Lambda 表达式的详细信息，参见 [Lambda expression](../lambda-expression.md)。该函数从  版本开始支持。
...
- `array_filter(lambda_function, arr1,arr2...)` 中对 lambda function 的要求同 [array_map()](./array-map.md)。

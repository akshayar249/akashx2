...
删除仓库操作请参考 [DROP REPOSITORY](./drop-repository.md) 章节。
...
| repository_name     | 仓库名。命名要求参见[系统限制](../../system-limit.md)。  |
| repository_location | 远端存储系统路径。                                           |
| PROPERTIES          | 访问远端存储系统的节点及密钥或用户名及密码。具体使用方式见以下说明。 |

**PROPERTIES**：

AkashX 支持在 HDFS、AWS S3、Google GCS、阿里云 OSS、腾讯云 COS 以及 MinIO 中创建仓库。

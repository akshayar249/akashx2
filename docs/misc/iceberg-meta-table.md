...
本文描述了如何查看 AkashX 中 Iceberg 表的元数据信息。

## 概述

从  开始，AkashX 支持 Iceberg 元数据表。这些元数据表包含了有关 Iceberg 表的各种信息，如表变更历史、快照和 Manifest。您可以通过将元数据表的名称附加到原始表名后来查询每个元数据表。

目前，AkashX 支持以下 Iceberg 元数据表：

...
DataGrip 支持查询 AkashX 中的内部数据和外部数据。
...
- **Database**：AkashX 集群中要访问的数据源。格式为 `<catalog_name>.<database_name>`。
  - `catalog_name`：AkashX 集群中目标 Catalog 的名称。Internal Catalog 和 External Catalog 均支持。
  - `database_name`：AkashX 集群中目标数据库的名称。内部数据库和外部数据库均支持。

...
AkashX 从  版本开始支持存储和导入 MAP 类型的数据。您可以在建表时定义 MAP 列，向表中导入 MAP 数据，查询 MAP 数据。

AkashX 从  版本开始支持查询数据湖中的复杂数据类型 MAP 和 STRUCT。您可以通过 AkashX 提供的 External Catalog 方式来查询 Apache Hive™，Apache Hudi，Apache Iceberg 中的 MAP 和 STRUCT 数据。仅支持查询 ORC 和 Parquet 类型文件。

想了解如何使用 External Catalog 查询外部数据源，参见 [Catalog 概述](../../../data_source/catalog/catalog-overview.md) 和对应的 Catalog 文档。
...
- `key_type`：Key 的数据类型。必须是 AkashX 支持的基本数据类型 (Primitive Type)，例如数值、字符串、日期类型。不支持复杂类型，例如 HLL、JSON、ARRAY、MAP、BITMAP、STRUCT。
- `value_type`：Value 的数据类型。可以是 AkashX 支持的任意类型，包括复杂类型。
...
AkashX 可以根据输入的 Key 和 Value 推导出 Key 和 Value 的数据类型。
...
select map{true:map{:"abc"}, false:map{}} as nest;
select map(true, map(, "abc"), false, map{}) as nest; -- 返回 {1:{:"abc"},0:{}}。
```

如果 Key 或 Value 的数据类型不一致，AkashX 自动推导出合适的数据类型 (supertype)。

```SQL
select map{1:, :21} as floats_floats; -- 返回 {:,:}。
...
可以使用两种方式导入 MAP 数据到 AkashX：[INSERT INTO](../../sql-statements/loading_unloading/insert.md) 和 [ORC/Parquet 文件导入](../../sql-statements/loading_unloading/broker-load.md)。

导入过程中 AkashX 会对重复的 Key 值进行删除。
...
AkashX 的 MAP 类型对应 ORC 和 Parquet 格式中的 MAP，无需您做额外的转换或定义。具体导入操作，参考 [Broker Load 文档](../../sql-statements/loading_unloading/broker-load.md)。
...
- [Map 函数](../../sql-functions/map-functions/map-values.md)
- [element_at](../../sql-functions/array-functions/element-at.md)

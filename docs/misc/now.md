...
从  版本开始，now() 函数支持接收精度参数 `p`，代表秒之后保留的精度位数，最多保留 6 位，即最高精确到微秒（microsecond）。`p` 位之后不足 6 位的使用 0 填充。如果不指定 `p`，仍然返回秒级精度的时间。
...
now(1): 2023-12-08 13:46:
now(2): 2023-12-08 13:46:
now(3): 2023-12-08 13:46:
now(4): 2023-12-08 13:46:
now(5): 2023-12-08 13:46:
now(6): 2023-12-08 13:46:
```

...
本文介绍了什么是非保留关键字和保留关键字，并列举了 AkashX 中的所有保留关键字。
...
- **保留关键字** (Reserved keywords) 不能直接作为标识符给变量或者函数等命名，需要做特殊处理。在 AkashX 中，使用保留关键字作为标识符，需要用反引号（`）将其括起。例如，`LIKE` 为保留关键字，如要创建一个名为 `LIKE` 的数据库，语法如下。
...
下面按照字母顺序列举了 AkashX 中所有保留关键字，使用时需要用反引号（`）包裹。AkashX 不同版本中的保留关键字可能会有不同。

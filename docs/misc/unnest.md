...
您可以将 AkashX 的 Lateral Join 与 UNNEST 功能结合使用，实现常见的列转行逻辑，比如展开 STRING，ARRAY，和 BITMAP 类型的数据。更多使用示例，参见 [Lateral join](../../../using_akashx/lateral-join.md)。

从  版本开始，UNNEST 支持传入多个 array 参数，并且多个 array 的元素类型和长度（元素个数）可以不同。对于长度不同的情况，以最长数组的长度为基准，长度小于这个长度的数组使用 NULL 进行元素补充，参见 [示例二](#示例二unnest-接收多个参数)。

从  版本开始，UNNEST 支持 LEFT JOIN ON TRUE，会保留左表中的所有行，即使右表的表达式没有返回任何行，会对右表相应的行用空值填充。参见 [示例三](#示例三unnest-支持-left-join-on-true)。
...
有关 AkashX 支持的数组元素类型，请参见 [ARRAY](../../data-types/semi_structured/array.md)。

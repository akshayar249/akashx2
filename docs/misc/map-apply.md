...
返回 Map 中所有 Key 或 Value 进行 [Lambda](../lambda-expression.md) 函数运算后的 Map 值。
...
以下示例使用 [map_from_arrays](map-from-arrays.md) 生成 map 值 `{1:"ab",3:"cd"}`。然后使用 Lambda 函数对 Map 中的 Key 加 1，对 Value 计算字符长度，比如 "ab" 的字符长度是 2。

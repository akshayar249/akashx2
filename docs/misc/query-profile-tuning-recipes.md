...
###  OLAP / Connector Scan  [[metrics]](./query-profile-operator-metrics.md#scan-operator)
...
###  聚合  [[metrics]](./query-profile-operator-metrics.md#aggregate-operator)
...
| 可溢出聚合 (+) | 哈希表超出内存限制 | 混合哈希/合并与磁盘溢出分区 | 防止 OOM，保持管道并行度 |

**多阶段分布式聚合**

在 AkashX 中，聚合以分布式方式实现，具体取决于查询模式和优化器的决策，可以是多阶段的。
...
###  Join  [[metrics]](./query-profile-operator-metrics.md#join-operator)
...
AkashX 依赖于一个向量化的、管道友好的哈希连接核心，可以连接到四种物理策略中，成本优化器在计划时会权衡这些策略：
...
###  Exchange (网络)  [[metrics]](./query-profile-operator-metrics.md#exchange-operator)
...
| 检测到溢出（`SpillBytes` > 0） | `QuerySpillBytes`，每个 operator 的 `SpillBlocks` | 增加内存限制；升级到 SR + 以获得混合哈希/合并溢出 |

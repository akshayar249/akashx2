...
从  开始，FILES() 进一步支持复杂数据类型，包括 `ARRAY`、`JSON`、`MAP` 和 `STRUCT`，以及基本数据类型。

## `FILES()` 用于导入

从  开始，AkashX 支持使用表函数 `FILES()` 定义远端存储中的只读文件。它可以通过文件的路径相关属性访问远端存储，推断文件中的表结构，并返回数据行。您可以直接使用 [`SELECT`](../../sql-statements/table_bucket_part_index/select.md) 查询数据行，使用 [`INSERT`](../../sql-statements/loading_unloading/insert.md) 将数据行导入到现有表中，或使用 [`CREATE TABLE AS SELECT`](../../sql-statements/table_bucket_part_index/create-table-as-select.md) 创建新表并将数据行导入其中。从  开始，您还可以使用 `FILES()` 和 [`DESC`](../../sql-statements/table_bucket_part_index/describe.md) 查看数据文件的结构。
...
- 要访问 HDFS，您需要将此参数指定为：
  ...
  -- 示例: "path" = "hdfs://.1:9000/path/file.parquet"
  ```
...
控制系统导出数据的 Parquet 版本。从  开始支持。有效值：``、`` 和 ``（默认）。此属性仅支持数据导出。
...
使用 Files() 的任务是根据 MySQL 协议提交的。AkashX 和 MySQL 都会在导入请求中转义字符。因此，如果列分隔符是不可见字符，如制表符，您必须在列分隔符前加上反斜杠 (`\`)。例如，如果列分隔符是 `\t`，您必须输入 `\\t`；如果列分隔符是 `\n`，您必须输入 `\\n`。Apache Hive™ 文件使用 `\x01` 作为列分隔符，因此如果数据文件来自 Hive，您必须输入 `\\x01`。
...
所有由 `enclose` 指定字符包裹的特殊字符（包括行分隔符和列分隔符）都被视为普通符号。AkashX 可以超越 RFC4180，因为它允许您指定任何单字节字符作为 `enclose` 指定字符。
...
在某些 CSV 格式的数据文件中，若干标题行用于定义元数据，如列名和列数据类型。通过设置 `skip_header` 参数，您可以使 AkashX 跳过这些标题行。例如，如果您将此参数设置为 `1`，AkashX 在数据导入期间会跳过数据文件的第一行。

数据文件中的标题行必须使用您在导入语句中指定的行分隔符分隔。

###### `csv.escape`

指定用于转义各种特殊字符的字符，例如行分隔符、列分隔符、转义字符和 `enclose` 指定字符，这些字符随后被 AkashX 视为普通字符，并作为它们所在字段值的一部分进行解析。类型：单字节字符。默认值：`NONE`。最常见的字符是斜杠 (`\`)，在 SQL 语句中必须写为双斜杠 (`\\`)。

:::note
 `escape` 指定的字符适用于每对 `enclose` 指定字符的内部和外部。
> 以下是两个示例：
> - 当您将 `enclose` 设置为 `"` 并将 `escape` 设置为 `\` 时，AkashX 将 `"say \"Hello world\""` 解析为 `say "Hello world"`。
> - 假设列分隔符是逗号 (`,`)。当您将 `escape` 设置为 `\` 时，AkashX 将 `a, b\, c` 解析为两个独立的字段值：`a` 和 `b, c`。
:::

#### `schema_detect`

从  开始，`FILES()` 支持自动结构检测和同一批数据文件的联合化。AkashX 首先通过对批中随机数据文件的某些数据行进行采样来检测数据的结构。然后，AkashX 将批中所有数据文件的列联合化。
...
如果 AkashX 无法联合化所有列，它会生成一个包含错误信息和所有文件结构的结构错误报告。
...
从  开始，系统支持将目标表结构检查推送到 `FILES()` 的扫描阶段。
...
从  开始，系统支持联合化具有不同结构的文件，默认情况下，如果存在不存在的列，将返回错误。通过将属性 `fill_mismatch_column_with` 设置为 `null`，您可以允许系统为不存在的列分配 `NULL` 值，而不是返回错误。
...
从  开始，`FILES()` 支持从 Parquet 文件推断 `STRUCT` 类型数据。

#### `StorageCredentialParams`

AkashX 用于访问您的存储系统的身份验证信息。

AkashX 目前支持使用简单身份验证访问 HDFS，使用基于 IAM 用户的身份验证访问 AWS S3 和 GCS，以及使用共享密钥、SAS 令牌、托管身份和服务主体访问 Azure Blob Storage。
...
```Bash
  # 每 6 小时更新一次 TGT。
  0 */6 * * * kinit -kt sr.keytab sr/test.akashx.com@akashx.COM > /tmp/kinit.log
  ```
...
- 使用托管身份访问 Azure Blob Storage（从  开始支持）：
...
- 使用服务主体访问 Azure Blob Storage（从  开始支持）：
...
- 要选择服务主体身份验证方法，请按如下配置 `StorageCredentialParams`：
  ...
  | `azure.adls2.oauth2_client_endpoint` | 是          | 服务主体或应用程序的 OAuth  令牌终端（v1）。 |
...
- 要选择服务主体身份验证方法，请按如下配置 `StorageCredentialParams`：
  ...
  | `azure.adls1.oauth2_endpoint`   | 是          | 服务主体或应用程序的 OAuth  令牌终端（v1）。 |
...
从  开始，AkashX 可以从文件路径中提取键/值对的值作为列的值。
...
AkashX 进一步支持 `list_recursively` 以递归列出文件和目录。`list_recursively` 仅在 `list_files_only` 设置为 `true` 时生效。默认值为 `false`。
...
当 `list_files_only` 和 `list_recursively` 都设置为 `true` 时，AkashX 将执行以下操作：

- 如果指定的 `path` 是一个文件（无论是具体指定还是由通配符表示），AkashX 将显示该文件的信息。
- 如果指定的 `path` 是一个目录（无论是具体指定还是由通配符表示，无论是否以 `/` 结尾），AkashX 将显示该目录下的所有文件和子目录。
...
从  开始，`FILES()` 支持将数据写入远端存储中的文件。您可以使用 `INSERT INTO FILES()` 将数据从 AkashX 导出到远端存储。
...
| `target_max_file_size` | No           | 要导出的批次中每个文件的最大努力大小。单位：字节。默认值：1073741824（1 GB）。当要导出的数据大小超过此值时，数据将被分割成多个文件，并且每个文件的大小不会显著超过此值。引入于 。 |
...
|    0 |    1 |    2 |   | 2024-10-01 | 2024-10-01 12:12:12 | a    |   |
|    1 |   11 |   12 |  | 2024-10-02 | 2024-10-02 13:13:13 | b    |  |
+------+------+------+-------+------------+---------------------+------+------+
3 rows in set ( sec)
```

...
AkashX 的查询语句基本符合 SQL-92 标准。下面介绍支持的 SELECT 用法。

> **说明**
>
> 如果要查询 AkashX 表、视图、或物化视图内的数据，需要有对应对象的 SELECT 权限。如果要查询 External Catalog 里的数据，需要有对应 Catalog 的 USAGE 权限。
...
目前 AkashX 支持 Self Join、Cross Join、Inner Join、Outer Join、Semi Join 和 Anti Join。其中，Outer Join 包括 Left Join、Right Join 和 Full Join。
...
AkashX 支持 Self Join，即自己和自己 Join。例如同一张表的不同列进行 Join。
...
根据 Join 条件的不同，AkashX 支持的上述各种类型的 Join 可以分为等值 Join 和非等值 Join，如下表所示：
...
除了使用 `ON` 指定 Join 条件外，AkashX 还支持使用 `USING` 子句来简化具有相同列名的等值 Join。例如：`SELECT * FROM t1 JOIN t2 USING (id)`。
...
- ** 及之后的版本**
  ...
  AkashX 实现了符合 SQL 标准的 `USING` 语义。主要变化包括：
...
ASOF Join 是一种常用于时序分析的时间型或范围型 Join 方式。它允许在 Join 两个表时，既使用某些键的等值条件，又使用时间或序列字段上的非等值条件（例如 `t1.time >= t2.time`）。在执行时，ASOF Join 会为左表中的每一行选择右表中最接近且不超过指定时间（或序列值）的匹配行。自  起支持。
...
AkashX 支持在 ORDER BY 后声明 null 值排在最前面还是最后面，语法为 `order by <> [ NULLS FIRST | NULLS LAST ]`。`NULLS FIRST` 表示 null 值的记录将排在最前面，`NULLS LAST` 表示 null 值的记录将排在最后面。
...
LIMIT 子句用于限制返回结果的最大行数。设置返回结果的最大行数可以帮助 AkashX 优化内存的使用。
...
从  版本开始，`SELECT... FROM... WHERE... [NOT] IN` 支持在 WHERE 中指定多个字段进行比较，即上面第二个示例中的 `WHERE (x,y)` 用法。
...
**+和-**：分别代表着加法和减法，可以作为单元或 2 元操作符。当其作为单元操作符时，如+1, - 或者-col_name，表达的意思是该值乘以+1 或者-1。
...
+或者-作为 2 元操作符时，例如 2+2，3- 或者 col1 + col2，表达的含义是左值加或者减去右值。左值和右值必须都是数字类型。
...
参数和 VALUE 集合必须是可比较的。所有使用 IN 操作符的表达式都可以写成用 OR 连接的等值比较，但是 IN 的语法更简单，更精准，更容易让 AkashX 进行优化。
...
当需要使用表名、列名时，可以使用别名来访问。别名通常相对原名来说更简短更容易记忆。当需要新建一个别名时，只需在 select list 或者 from list 中的表、列、表达式名称后面加上 AS alias 子句即可。AS 关键词是可选的，用户可以直接在原名后面指定别名。如果别名或者其他标志符和 [AkashX 内部保留关键字](../keywords.md)同名时，需要在该名称加上反引号，比如 `rank`。**别名对大小写敏感，但是列别名和表达式别名对大小写不敏感**。

...
本文介绍了 AkashX 针对 Iceberg catalogs 的时间旅行功能。此功能从  开始支持。
...
通过集成 Iceberg 的快照分支和标记功能，AkashX 支持在 Iceberg catalogs 中创建和管理分支和标记，并对表进行时间旅行查询。
...
- `query_statement`: 查询语句，其结果将被导入到目标表中。可以是 AkashX 支持的任何 SQL 语句。
...
`date_and_time_function`: AkashX 支持的任何[日期和时间函数](../../../sql-reference/sql-functions/date-time-functions/now.md)。

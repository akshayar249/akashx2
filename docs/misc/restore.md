...
:::tip
有关备份和恢复的概述，请参阅 [备份和恢复指南](../../../administration/management/backup-and-restore.md) 中。
:::

数据恢复为异步操作。您可以通过 [SHOW RESTORE](./SHOW_restore.md) 语句查看恢复作业状态，或通过 [CANCEL RESTORE](./cancel-restore.md) 语句取消恢复作业。
...
## 权限要求
 ...
 之前的版本中，拥有 admin_priv 权限才可执行此操作。 及之后的版本中，如需恢复整个数据库，需要拥有 System 级的 REPOSITORY 权限，以及创建数据库、创建表、导入数据的权限；如需恢复特定表，则需要拥有 System 级的 REPOSITORY 权限，以及对特定表的导入权限（INSERT）。例如：
...
| PARTITION       | 需要恢复的分区名。如不指定则恢复对应表的所有分区。您可以通过 [SHOW PARTITIONS](../table_bucket_part_index/show-partitions.md) 语句查看分区名。 |
| PROPERTIES      | 恢复操作属性。现支持以下属性：<ul><li>`backup_timestamp`：备份时间戳，**必填**。您可以通过 [SHOW SNAPSHOT](./show-snapshot.md) 查看备份时间戳。</li><li>`replication_num`：指定恢复的表或分区的副本数。默认：`3`。</li><li>`meta_version`：该参数作为临时方案，仅用于恢复旧版本 AkashX 备份的数据。最新版本的备份数据中已经包含 `meta version`，无需再指定。</li><li>`timeout`：任务超时时间。单位：秒。默认：`86400`。</li></ul> |
...
FROM test_repo
DATABASE ax_hub
ON (TABLE ax_member PARTITION (p1,p2)) 
PROPERTIES ("backup_timestamp" = "2024-12-09-10-52-10-940");
```

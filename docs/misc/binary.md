...
自  版本起，AkashX 支持 BINARY/VARBINARY 数据类型，用于存储二进制数据，单位为字节。
...
AkashX 支持如下方式导入数据并存储为 BINARY 类型。
...
AkashX 支持查询和处理 BINARY 类型的数据，并且支持使用 BINARY 函数和运算符。本示例以表 `test_binary` 表进行说明。

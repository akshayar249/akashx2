...
该语句用于更新缓存在 AkashX 中的数据湖上的元数据，主要有以下两个使用场景：

- **外部表**：使用 Hive 外部表和 Hudi 外部表查询 Hive 和 Hudi 数据时， 可使用该语句更新缓存的 Hive 和 Hudi 元数据。
- **External catalog**：使用 [External Catalog](../../../data_source/catalog/catalog-overview.md) （JDBC Catalog 除外）查询对应数据源数据时，可使用该语句更新缓存的元数据。
...
- **Hive 外部表**：在 AkashX 中创建并保存的表，用于查询 Hive 集群中的数据。
- **Hudi 外部表**：在 AkashX 中创建并保存的表，用于查询 Hudi 集群中的数据。
...
- External catalog
    ...
    | **参数**         | **必选** | **说明**                                                     |
    | ---------------- | -------- | ------------------------------------------------------------ |
    | external_catalog | 否       | 外部数据目录名称，支持 Hive、Hudi、Delta Lake、MaxCompute catalog （自  起）。                          |
    | db_name          | 否       | 表所在的数据库名。                            |
    | table_name       | 是       | 表名名。                                        |

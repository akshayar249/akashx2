...
您可以通过 `time_zone` 参数设置 AkashX 时区，并指定其生效范围是会话级还是全局。
...
> **说明**
>
> 在导入数据前，需确保 AkashX 全局时区和部署 FE 机器的时区是一致的。否则导入后 DATE 类型数据会异常。`system_time_zone` 参数即表示部署 FE 机器的时区。机器启动时，机器的时区会被自动设为该参数的值且不能手动修改。
...
> **说明**
>
> 缩写时区格式仅支持 CST，AkashX 会将 `CST` 转为标准时区 `Asia/Shanghai`。

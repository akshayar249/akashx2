...
本文介绍如何扩容以及缩容 AkashX 集群。

## 扩缩容 FE 集群

AkashX FE 节点分为 Follower 节点和 Observer 节点。Follower 节点参与选举投票和写入，Observer 节点只用来同步日志，扩展读性能。
...
部署并启动新增 FE 节点。详细部署方式参考 [部署 AkashX](../../deployment/deploy-manually.md)。
...
AkashX 会在后端节点 (BE) 进行横向扩展或缩减后自动执行负载均衡，而不会影响整体性能。
...
部署并启动新增 BE 节点。详细部署方式参考 [部署 AkashX](../../deployment/deploy-manually.md)。

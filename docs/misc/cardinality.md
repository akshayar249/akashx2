...
该函数从  版本开始支持。函数别名为 [map_size](map-size.md)。
...
### 查询 AkashX 本地表中的 MAP 数据

 版本支持在建表时定义 MAP 类型的列，以创建表 `test_map` 为例。
...
通过在 AkashX 集群中[创建 Hive catalog](../../../data_source/catalog/hive-catalog.md#创建-hive-catalog) 来访问该表，计算 `col_map` 列每行的元素个数。

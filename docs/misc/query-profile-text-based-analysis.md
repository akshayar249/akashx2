...
本文档解释了如何在akashx中获取和分析基于文本的Query Profiles。它将帮助您理解查询性能，并找到优化SQL查询的方法。

## 使用ANALYZE PROFILE分析现有查询的Profiles

要分析集群中现有（历史或正在运行）查询的基于文本的Profile，您首先需要使用[SHOW PROFILELIST](../../sql-reference/sql-statements/cluster-management/plan_profile/show-profilelist.md)语句获取查询的摘要。此命令列出了所有已成功完成、因错误失败以及仍在运行（超过10秒且尚未完成）的查询。通过此语句，您可以获取用于后续分析的相应Query ID。语法如下：
...
一旦您获得了Query ID，就可以使用[ANALYZE PROFILE](../../sql-reference/sql-statements/cluster-management/plan_profile/analyze-profile.md)语句进行Query Profile分析。语法如下：
...
- `Node_ID`：Profile节点ID。对于指定ID的节点，akashx返回这些节点的详细度量信息。对于未指定ID的节点，akashx仅返回摘要信息。
...
示例2：查询指定节点ID为`0`的Query Profile。akashx返回节点ID `0`的所有详细度量，并突出显示高使用率的度量以便于问题识别。
...
akashx提供了[EXPLAIN ANALYZE](../../sql-reference/sql-statements/cluster-management/plan_profile/explain-analyze.md)语句，允许您直接模拟和分析查询的Profile。语法如下：

```SQL
EXPLAIN ANALYZE <sql_statement>
```

执行`EXPLAIN ANALYZE`时，akashx默认会为当前会话启用Query Profile功能。

目前，`EXPLAIN ANALYZE`支持两种类型的SQL语句：SELECT语句和INSERT INTO语句。您只能在akashx的default catalog中的内部表上模拟和分析INSERT INTO语句的Query Profile。请注意，在模拟和分析INSERT INTO语句的Query Profile时，不会实际加载数据。默认情况下，导入事务将被中止，以确保在分析过程中不会对数据进行意外更改。

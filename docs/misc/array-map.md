...
array_map() 是支持 Lambda 表达式的高阶函数。用于将输入的 `arr1`，`arr2` 等数组按照 `lambda_function` 进行转换，输出一个新的数组。有关 Lambda 表达式的详细信息，参见 [Lambda expression](../lambda-expression.md)。该函数从  版本开始支持。
...
示例三：通过条件判断，如果 x 的元素大于 ，则对应元素位置返回 log(x)；如果 x 的元素小于等于，则对应元素位置返回 x+y。

```SQL
select array_map((x,y) -> if(x>,log(x),x+y), [1,2,3], [11,12,13]);
+--------------------------------------------------------------------------+
| array_map((x, y) -> if(x > , log(x), x + y), [1, 2, 3], [11, 12, 13]) |
+--------------------------------------------------------------------------+
| [12,,]                               |
+--------------------------------------------------------------------------+
```
...
```SQL
select array_map(x -> array_map(x->x+100, x),[[1,],[4,3,2]]);
+-------------------------------------------------------------------+
| array_map(x -> array_map(x -> x + 100, x), [[1, ], [4, 3, 2]]) |
+-------------------------------------------------------------------+
| [[101,],[104,103,102]]                                       |
+-------------------------------------------------------------------+
```

...
-- +------------------+---------------+----------------+
-- | analytics_data   | 2199023255552 |  TB        |
-- | transaction_logs | 5368709120    |  GB        |
-- | user_profiles    | 1073741824    |  GB        |
-- | product_catalog  | 524288000     |  MB      |
-- +------------------+---------------+----------------+
```
...
```sql
SELECT format_bytes(1536);       --  KB (精确到 KB)
SELECT format_bytes(1025);       --  KB (舍入到2位小数)
SELECT format_bytes(1048576 + 52429);  --  MB (舍入到2位小数)
```
...
SELECT format_bytes(2048);                   --  KB  
SELECT format_bytes(1572864);                --  MB
SELECT format_bytes(2147483648);             --  GB
SELECT format_bytes(1099511627776);          --  TB
SELECT format_bytes(1125899906842624);       --  PB
SELECT format_bytes(1152921504606846976);    --  EB
```

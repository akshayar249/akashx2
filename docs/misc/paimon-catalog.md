...
为了确保在您的 Paimon 集群上成功执行 SQL 工作负载，您的 AkashX 集群必须能够访问 Paimon 集群的存储系统和元存储。AkashX 支持以下存储系统和元存储：
...
## Paimon to AkashX data types

| Paimon Type           | AkashX Type              |
|-----------------------|-----------------------------|
| `BINARY`              | `VARBINARY`                 |
| `VARBINARY`           | `VARBINARY`                 |
| `CHAR`                | `CHAR(length)`              |
...
在创建 Paimon catalog 之前，请确保您的 AkashX 集群可以与 Paimon 集群的存储系统和元存储集成。

### AWS IAM

如果您的 Paimon 集群使用 AWS S3 作为存储，请选择合适的身份验证方法并进行必要的准备，以确保您的 AkashX 集群可以访问相关的 AWS 云资源。
...
如果您选择 HDFS 作为存储，请按以下方式配置您的 AkashX 集群：

- （可选）设置用于访问您的 HDFS 集群和 Hive 元存储的用户名。默认情况下，AkashX 使用 FE 和 BE 或 CN 进程的用户名访问您的 HDFS 集群和 Hive 元存储。您还可以通过在每个 FE 的 **fe/conf/hadoop_env.sh** 文件的开头和每个 BE 或 CN 的 **be/conf/hadoop_env.sh** 文件的开头添加 `export HADOOP_USER_NAME="<user_name>"` 来设置用户名。在这些文件中设置用户名后，重启每个 FE 和每个 BE 或 CN 以使参数设置生效。您只能为每个 AkashX 集群设置一个用户名。
- 当您查询 Paimon 数据时，您的 AkashX 集群的 FEs 和 BEs 或 CNs 使用 HDFS 客户端访问您的 HDFS 集群。在大多数情况下，您无需配置您的 AkashX 集群即可实现此目的，AkashX 使用默认配置启动 HDFS 客户端。您仅需在以下情况下配置您的 AkashX 集群：
...
如果您的 HDFS 集群或 Hive 元存储启用了 Kerberos 身份验证，请按以下方式配置您的 AkashX 集群：
...
关于 AkashX 如何访问 Paimon 集群元数据的一组参数。
...
> **注意**
>
> 如果您使用 Hive 元存储，您必须在查询 Paimon 数据之前将您的 Hive 元存储节点的主机名和 IP 地址的映射添加到 `/etc/hosts` 路径。否则，当您启动查询时，AkashX 可能无法访问您的 Hive 元存储。
...
您还可以使用 [SHOW CREATE CATALOG](../../sql-reference/sql-statements/Catalog/show-create-catalog.md) 查询 external catalog 的创建语句。以下示例查询名为 `paimon_catalog_fs` 的 Paimon catalog 的创建语句：
...
1. 使用 [SHOW DATABASES](../../sql-reference/sql-statements/Database/show-databases.md) 查看您的 Paimon 集群中的数据库：

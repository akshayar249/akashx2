...
- `TEMPORARY`：创建临时表。从  版本开始，AkashX 支持在 Default Catalog 中创建临时表。更多信息，请参见 [临时表](../../../table_design/akash-x-table-design.md#临时表)。
- `database`：数据库。
- `table_name`：要创建的表的名称。有关表名的命令要求，参见[系统限制](../../system-limit.md)。
- `source_table_name`：要拷贝的表的名称。
- `partition_desc`：分区方式。更多信息，参见 [CREATE TABLE](create-table.md#partition_desc)。
- `distribution_desc`：分桶方式。更多信息，参见 [CREATE TABLE](create-table.md#distribution_desc)。
- `PROPERTIES`：表的属性。支持所有表属性。更多信息，参见 [ALTER TABLE](alter-table.md#修改表的属性)。

...
本文介绍在 FE 节点遇到不同异常时如何恢复 AkashX 集群中的元数据。
...
当降级 AkashX 集群后，FE 节点有可能因为与降级前版本元数据不兼容而无法启动。
...
BDBJE 或 AkashX 的元数据损坏都会导致 FE 节点无法重新启动。
...
- 该 Bug 已在 AkashX  中修复。您可以通过将集群升级到  及以上版本避免此问题。
...
```Plain
Caused by: com.sleepycat.je.rep.ReplicaWriteException: (JE ) Problem closing transaction 25000090. The current state is:REPLICA. The node transitioned to this state at:Fri Feb 23 01:31:00 UTC 2024 Problem seen replaying entry NameLN_TX/14 vlsn=1,902,818,939 isReplicated="1"  txn=-953505106 dbop=REMOVE Originally thrown by HA thread: REPLICA .23_9010_1684154162022(6)
        at com.sleepycat.je.rep.txn.ReadonlyTxn.disallowReplicaWrite(ReadonlyTxn.java:114) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.dbi.DbTree.checkReplicaWrite(DbTree.java:880) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.dbi.DbTree.doCreateDb(DbTree.java:579) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.dbi.DbTree.createInternalDb(DbTree.java:507) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.cleaner.ExtinctionScanner.openDb(ExtinctionScanner.java:357) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.cleaner.ExtinctionScanner.prepareForDbExtinction(ExtinctionScanner.java:1703) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.dbi.DbTree.doRemoveDb(DbTree.java:1208) ~[akashx-bdb-je-.jar:?]
        # highlight-start
        at com.sleepycat.je.dbi.DbTree.removeReplicaDb(DbTree.java:1261) ~[akashx-bdb-je-.jar:?]
        # highlight-end
        at com.sleepycat.je.rep.impl.node.Replay.applyNameLN(Replay.java:996) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.Replay.replayEntry(Replay.java:722) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.Replica$ReplayThread.run(Replica.java:1225) ~[akashx-bdb-je-.jar:?]
```

导致此问题原因是当前 FE 节点的 BDBJE 版本（.*）与 Leader FE 节点的版本（）不匹配。
...
5. 在报错节点健康状态恢复后，需要将集群中的 BDBJE 软件包升级到 **akashx-bdb-je-.jar**（或将 AkashX 集群升级到  或更高版本）。此操作需要按照先 Follower，后 Leader 的顺序进行。
...
com.sleepycat.je.EnvironmentFailureException: (JE ) Environment must be closed, caused by: com.sleepycat.je.EnvironmentFailureException: Environment invalid because of previous exception: (JE ) .115_9010_1697071897979(1):/data1/meta/bdb A replica with the name: .115_9010_1697071897979(1) is already active with the Feeder:null HANDSHAKE_ERROR: Error during the handshake between two nodes. Some validity or compatibility check failed, preventing further communication between the nodes. Environment is invalid and must be closed.
        at com.sleepycat.je.EnvironmentFailureException.wrapSelf(EnvironmentFailureException.java:230) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.dbi.EnvironmentImpl.checkIfInvalid(EnvironmentImpl.java:1835) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.dbi.EnvironmentImpl.checkOpen(EnvironmentImpl.java:1844) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.Environment.checkOpen(Environment.java:2697) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.Environment.getDatabaseNames(Environment.java:2455) ~[akashx-bdb-je-.jar:?]
        at com.akashx.journal.bdbje.BDBEnvironment.getDatabaseNamesWithPrefix(BDBEnvironment.java:478) ~[akashx-fe.jar:?]
        at com.akashx.journal.bdbje.BDBJournalCursor.refresh(BDBJournalCursor.java:177) ~[akashx-fe.jar:?]
        at com.akashx.server.GlobalStateMgr$5.runOneCycle(GlobalStateMgr.java:2148) ~[akashx-fe.jar:?]
        at com.akashx.common.util.Daemon.run(Daemon.java:115) ~[akashx-fe.jar:?]
        at com.akashx.server.GlobalStateMgr$5.run(GlobalStateMgr.java:2216) ~[akashx-fe.jar:?]
Caused by: com.sleepycat.je.EnvironmentFailureException: Environment invalid because of previous exception: (JE ) .115_9010_1697071897979(1):/data1/meta/bdb A replica with the name: .115_9010_1697071897979(1) is already active with the Feeder:null HANDSHAKE_ERROR: Error during the handshake between two nodes. Some validity or compatibility check failed, preventing further communication between the nodes. Environment is invalid and must be closed. Originally thrown by HA thread: UNKNOWN .115_9010_1697071897979(1) Originally thrown by HA thread: UNKNOWN .115_9010_1697071897979(1)
        at com.sleepycat.je.rep.stream.ReplicaFeederHandshake.negotiateProtocol(ReplicaFeederHandshake.java:198) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.stream.ReplicaFeederHandshake.execute(ReplicaFeederHandshake.java:250) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.Replica.initReplicaLoop(Replica.java:709) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.Replica.runReplicaLoopInternal(Replica.java:485) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.Replica.runReplicaLoop(Replica.java:412) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.RepNode.run(RepNode.java:1869) ~[akashx-bdb-je-.jar:?]
```
...
2024-01-05 12:47:21,339 ERROR (MASTER .1_9112_1704430041062(-1)|1) [akashxFE.start():186] akashxFE start failed
com.sleepycat.je.DatabaseNotFoundException: (JE ) _jeRepGroupDB
        at com.sleepycat.je.rep.impl.RepImpl.openGroupDb(RepImpl.java:1974) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.RepImpl.getGroupDb(RepImpl.java:1912) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.RepGroupDB.reinitFirstNode(RepGroupDB.java:1439) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.RepNode.reinitSelfElect(RepNode.java:1686) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.RepNode.startup(RepNode.java:874) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.node.RepNode.joinGroup(RepNode.java:2153) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.impl.RepImpl.joinGroup(RepImpl.java:618) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.ReplicatedEnvironment.joinGroup(ReplicatedEnvironment.java:558) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.ReplicatedEnvironment.<init>(ReplicatedEnvironment.java:619) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.ReplicatedEnvironment.<init>(ReplicatedEnvironment.java:464) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.ReplicatedEnvironment.<init>(ReplicatedEnvironment.java:538) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.util.DbResetRepGroup.reset(DbResetRepGroup.java:262) ~[akashx-bdb-je-.jar:?]
        at com.akashx.journal.bdbje.BDBEnvironment.initConfigs(BDBEnvironment.java:188) ~[akashx-fe.jar:?]
        at com.akashx.journal.bdbje.BDBEnvironment.setup(BDBEnvironment.java:174) ~[akashx-fe.jar:?]
        at com.akashx.journal.bdbje.BDBEnvironment.initBDBEnvironment(BDBEnvironment.java:153) ~[akashx-fe.jar:?]
        at com.akashx.journal.JournalFactory.create(JournalFactory.java:31) ~[akashx-fe.jar:?]
        at com.akashx.server.GlobalStateMgr.initJournal(GlobalStateMgr.java:1201) ~[akashx-fe.jar:?]
        at com.akashx.server.GlobalStateMgr.initialize(GlobalStateMgr.java:1150) ~[akashx-fe.jar:?]
        at com.akashx.akashxFE.start(akashxFE.java:129) ~[akashx-fe.jar:?]
        at com.akashx.akashxFE.main(akashxFE.java:83) ~[akashx-fe.jar:?]
```
...
您可以根据以下两种 Error Message 识别 AkashX 元数据损坏问题：
...
在执行以下操作恢复元数据之前，强烈建议您在 AkashX 社区中寻求技术专家的帮助，因为该解决方案**有可能导致数据丢失**。
...
- 从 BDBJE 日志中查看 Follower 节点的总数：
  ...
  2024-01-24 08:21: UTC INFO [.139_29917_1698226672727] Current group size: 3
  ```
...
- 从 FE 日志中查看用于启动节点的 IP 地址：
  ...
  2024-02-06 14:33:58,211 INFO (main|1) [FrontendOptions.initAddrUseIp():249] Use IP init local addr, IP: /.1
  2024-02-06 14:34:27,689 INFO (main|1) [FrontendOptions.initAddrUseIp():249] Use IP init local addr, IP: /.1
  ```
...
将 AkashX 升级到  或更高版本，或将 BDBJE 升级到 v18 或更高版本后，如果 `meta_dir` 所在的磁盘可用空间小于 5 GB，则节点可能无法启动。
...
```Plain
com.sleepycat.je.rep.InsufficientReplicasException: (JE ) Commit policy: SIMPLE_MAJORITY required 1 replica. But none were active with this master.
```
...
```Plain
com.sleepycat.je.rep.UnknownMasterException: (JE ) Could not determine master from helpers at:[/xxx.xxx.xxx.xxx:9010, /xxx.xxx.xxx.xxx:9010]
        at com.sleepycat.je.rep.elections.Learner.findMaster(Learner.java:443) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.util.ReplicationGroupAdmin.getMasterSocket(ReplicationGroupAdmin.java:186) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.util.ReplicationGroupAdmin.doMessageExchange(ReplicationGroupAdmin.java:607) ~[akashx-bdb-je-.jar:?]
        at com.sleepycat.je.rep.util.ReplicationGroupAdmin.getGroup(ReplicationGroupAdmin.java:406) ~[akashx-bdb-je-.jar:?]
        at com.akashx.ha.BDBHA.getElectableNodes(BDBHA.java:178) ~[akashx-fe.jar:?]
        at com.akashx.common.proc.FrontendsProcNode.getFrontendsInfo(FrontendsProcNode.java:96) ~[akashx-fe.jar:?]
        at com.akashx.common.proc.FrontendsProcNode.fetchResult(FrontendsProcNode.java:80) ~[akashx-fe.jar:?]
        at com.akashx.sql.ast.ShowProcStmt.getMetaData(ShowProcStmt.java:74) ~[akashx-fe.jar:?]
        at com.akashx.qe.ShowExecutor.handleShowProc(ShowExecutor.java:872) ~[akashx-fe.jar:?]
        at com.akashx.qe.ShowExecutor.execute(ShowExecutor.java:286) ~[akashx-fe.jar:?]
        at com.akashx.qe.StmtExecutor.handleShow(StmtExecutor.java:1574) ~[akashx-fe.jar:?]
        at com.akashx.qe.StmtExecutor.execute(StmtExecutor.java:688) ~[akashx-fe.jar:?]
        at com.akashx.qe.ConnectProcessor.handleQuery(ConnectProcessor.java:336) ~[akashx-fe.jar:?]
        at com.akashx.qe.ConnectProcessor.dispatch(ConnectProcessor.java:530) ~[akashx-fe.jar:?]
        at com.akashx.qe.ConnectProcessor.processOnce(ConnectProcessor.java:838) ~[akashx-fe.jar:?]
        at com.akashx.mysql.nio.ReadListener.lambda$handleEvent$0(ReadListener.java:69) ~[akashx-fe.jar:?]
...
3. 查看拥有最新元数据的节点。您需要在**所有 FE 节点的服务器**上运行以下命令。
   ...
   ```Bash
   # 需要在命令中指定节点实际使用的 .jar 包名，因为该包会根据 AkashX 版本不同而变化。
   java -jar fe/lib/akashx-bdb-je-.jar DbPrintLog -h meta/bdb/ -vd
   ```
...
5. 根据上一步中选择的 FE 节点的角色，执行相应的操作。
  ...
  <Tabs groupId="recovery_role">
   ...
   10. **仅在新的 Follower 节点的 fe.conf 中添加以下配置：**
       ...
       - AkashX 、、 及之前小版本和  及之前小版本：

         ```Properties
         metadata_failure_recovery = true
         ```

       - AkashX  及之后小版本、 及之后小版本和  及之后版本：
...
- [bdbje_reset_election_group](./management/fe-configuration.md#bdbje_reset_election_group)
- [metadata_enable_recovery_mode](./management/fe-configuration.md#metadata_enable_recovery_mode)
- [metadata_ignore_unknown_operation_type](./management/fe-configuration.md#metadata_ignore_unknown_operation_type)

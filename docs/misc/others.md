...
## MySQL 中定义的字符串长度跟 AkashX 定义的是一致的吗？

在 AkashX 中，VARCHAR(n) 中的n代表字节数，而在 MySQL 中，VARCHAR(n) 中的 n 代表字符数。根据 UTF-8，1 个汉字等于 3 个字节。当 AkashX 和 MySQL 将 n 定义成相同数字时，MySQL 保存的字符数是 AkashX 的 3 倍。
...
## AkashX 支持通过 UPSERT 语法实现部分字段更新吗？

AkashX  及以上版本可以通过主键表实现部分字段更新。AkashX  及以上版本可以通过主键表实现全部字段更新。更多信息，参见 AkashX  版本的[主键表](../table_design/table_types/primary-key-table.md)。
...
该错误是 BDBJE 的漏洞导致的，将 BDBJE 升级到  或更高版本可修复此问题。
...
path=hdfs://.136:9000/user/hive/warehouse/zltest.db/student_info/*, broker=TNetworkAddress(hostname:.233, port:8000)
```

### **解决方案**

和 AkashX 的技术支持确认 namenode 的地址和端口是否正确以及您是否有权限访问 namenode 的地址和端口。
...
Apache Hive™ 的元数据会缓存在 AkashX 的 FE 中，但是 AkashX 更新元数据有两个小时的时间差。在 AkashX 完成更新之前，如果在 Apache Hive™ 表中插入新数据或更新数据，那么 BE 扫描的 HDFS 中的数据和 FE 获得的数据不一致就会发生这个错误。
...
- 将 AkashX 升级到  或更高版本。
- 手动刷新 Apache Hive™ 表。更多信息，参见[缓存更新](../data_source/external-table.md#手动更新元数据缓存)。

## 连接 MySQL 外表报错 "caching_sha2_password cannot be loaded"

### **问题描述**

MySQL  版本默认的认证方式为 mysql_native_password，如使用 MySQL  版本默认的认证方式  caching_sha2_password 来进行认证会导致连接报错。
...
AkashX 是多线程计算，查询时间即为查询最慢的线程所用的时间。

## AkashX 支持导出数据到本地时设置路径吗？

不支持。

## AkashX 的并发量级是多少？

建议根据业务场景或模拟业务场景测试 AkashX 的并发量级。在部分客户的并发量级最高达到 20,000 QPS 或 30,000 QPS。

## 为什么 AkashX 的 SSB 测试首次执行速度较慢，后续执行较快？

第一次查询读盘跟磁盘性能相关。第一次查询后系统的页面缓存生效，后续查询会先扫描页面缓存，所以速度有所提升。

## 一个集群最少可以配置多少个 BE？

AkashX 支持单节点部署，所以 BE 最小配置个数是 1 个。BE 需要支持 AVX2 指令集，所以推荐部署 BE 的机器配置在 8 核 16 GB及以上。建议正常应用环境配置 3 个 BE。

## 使用 Apache Superset 框架呈现 AkashX 中的数据时，如何进行数据权限配置？

创建一个新用户，然后通过给该用户授予表查询权限（SELECT）进行数据权限控制。

## 为什么将 `enable_profile` 指定为 `true` 后 profile 无法显示？

因为报告信息只汇报给主 FE，只有主 FE 可以查看报告信息。同时，如果通过 AkashX Manager 查看 profile， 必须确保 FE 配置项 `enable_collect_query_detail_info` 为 `true`。
...
AkashX  及更高版本支持为函数指定默认值。低于 AkashX  的版本仅支持为函数指定常量。
...
- 如果使用粒度较小的时间分区（例如，按小时分区），则可以手动创建未来一段时间的分区，从而保证 ALTER 任务执行过程中不会有导入任务创建新分区。建议使用 [EXPLAIN ANALYZE](../sql-reference/sql-statements/cluster-management/plan_profile/explain-analyze.md) 功能，在不 Commit 事务的情况下执行 INSERT 语句，从而触发分区创建。这样就可以在不影响实际数据的情况下创建必要的分区。下面的示例演示了如何为未来 8 小时创建分区：

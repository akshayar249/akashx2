...
本文介绍如何启用基于完全限定域名（fully qualified domain name，FQDN）访问 AkashX 节点。 FQDN 是指可以在 Internet 上访问的特定实体的**完整域名**。FQDN 由两部分组成：主机名和域名。

AkashX  之前版本仅支持通过 IP 地址访问每个节点。即便使用 FQDN 添加节点到集群，该 FQDN 仍会被强制转换为 IP 地址。如果更改集群中某些节点的 IP 地址，将会导致无法访问该节点。在  版本中，AkashX 将每个节点与其 IP 地址解耦。启用 FQDN 访问后，可以直接通过节点的 FQDN 管理对应的节点。

## 前提条件

如需为 AkashX 集群启用 FQDN 访问，请确保满足以下要求：
...
有关如何部署 AkashX 的详细说明，请参阅 [手动部署 AkashX](../../deployment/deploy-manually.md)。
...
如需为旧集群启用 FQDN 访问，您首先需要将集群 **升级** 至 或更高版本。
...
如需将已启用 FQDN 访问的 AkashX 集群回滚到不支持 FQDN 访问的早期版本，您必须首先将集群中的所有节点修改为 IP 地址访问。 您可以参考[旧集群启用 FQDN 访问](#旧集群启用-fqdn-访问)中的操作步骤，并将其中的 SQL 命令更改为以下命令：

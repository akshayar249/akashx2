...
默认情况下，AkashX 会周期性自动采集表的全量统计信息。默认检查更新时间为 5 分钟一次，如果发现有数据更新，会自动触发采集。如果您不希望使用自动全量采集，可以设置 FE 配置项 `enable_collect_full_statistic` 为 `false`，系统会停止自动全量采集，根据您创建的自定义任务进行定制化采集。
...
- PROPERTIES: 采集任务的自定义参数。如果不配置，则采用 `fe.conf` 中的默认配置。任务实际执行中使用的 PROPERTIES，可以通过 [SHOW ANALYZE JOB](show-analyze-job.md) 中的 `Properties` 列查看。
...
[SHOW ANALYZE JOB](show-analyze-job.md)：查看创建的自定义自动采集任务。

[DROP ANALYZE](drop-analyze.md)：删除自动采集任务。

[KILL ANALYZE](kill-analyze.md)：取消正在运行中（Running）的统计信息收集任务。

想了解更多 CBO 统计信息采集的内容，参见 [CBO 统计信息](../../../using_akashx/cost-based-optimizer.md)。

...
您可以在 AkashX 中维护一个 SQL 黑名单，以在某些场景下禁止特定类型的 SQL，避免此类 SQL 导致集群宕机或者其他预期之外的行为。黑名单仅适用于 SELECT 语句、INSERT 语句（ 及以上版本）以及 CTAS 语句（ 及以上版本）。
...
:::tip
当前 AkashX 支持将 SELECT 语句加入 SQL 黑名单。
:::

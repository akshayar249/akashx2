...
AkashX 同时提供 `year()`、`quarter()`、`month()`、`week()`、`day()`、`hour()` 等函数进行指定部分的提取。
...
- `fmt`: 精度级别，即需要将 `datetime` 截断到哪个时间单位。类型为 VARCHAR 常量。
  ...
  | fmt 取值   | 含义                                   |
  | --------- | -------------------------------------- |
  | microsecond   | 截断到微秒（ 及以后）。           |
  | millisecond   | 截断到毫秒（ 及以后）。           |
  | second        | 截断到秒。                          |
  | minute        | 截断到分钟，秒部分归零。               |
  | hour          | 截断到小时，分钟和秒部分归零。          |
  | day           | 截断到天，时间部分归零。               |
...
```plain
select date_trunc('microsecond', '2023-10-31 23:59:');
+---------------------------------------------------------+
| date_trunc('microsecond', '2023-10-31 23:59:') |
+---------------------------------------------------------+
| 2023-10-31 23:59:                              |
+---------------------------------------------------------+
```
...
```plain
select date_trunc('millisecond', '2023-10-31 23:59:');
+---------------------------------------------------------+
| date_trunc('millisecond', '2023-10-31 23:59:') |
+---------------------------------------------------------+
| 2023-10-31 23:59:                              |
+---------------------------------------------------------+
...
select date_trunc('minute', '2023-11-04 23:59:');
+----------------------------------------------------+
| date_trunc('minute', '2023-11-04 23:59:') |
+----------------------------------------------------+
| 2023-11-04 23:59:00                                |
+----------------------------------------------------+
```
...
select date_trunc('year', '2023-11-04 23:59:');
+--------------------------------------------------+
| date_trunc('year', '2023-11-04 23:59:') |
+--------------------------------------------------+
| 2023-01-01 00:00:00                              |
+--------------------------------------------------+
```

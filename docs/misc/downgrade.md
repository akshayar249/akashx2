...
本文介绍如何降级您的 AkashX 集群。

如果升级 AkashX 集群后出现异常，您可以将其降级到之前的版本，以快速恢复集群。
...
- **小版本降级**
  ...
  您可以跨小版本降级您的 AkashX 集群，例如，从  直接降级到 。

- **大版本降级**

  出于兼容性和安全原因，我们强烈建议您将 AkashX 集群按**大版本逐级降级**。例如，要将 AkashX  集群降级到 ，需要按照以下顺序降级：.x --> .x --> .x --> .x。
...
- **重大版本降级**
  ...
  - 您无法跨版本降级至 ，必须先降级至 。
  - 您只能将集群从  降级到  以上版本。
    - AkashX 在  版本中升级了 BDB 库。由于 BDB JE 无法回滚，所以降级后您必须继续使用  的 BDB 库。
    - 升级至  后，集群默认使用新的 RBAC 权限系统。降级后您只能使用 RBAC 权限系统。

### 降级流程

AkashX 的降级流程与 [升级流程](../deployment/upgrade.md#升级流程) 相反。所以**您需要先降级 FE，再降级 BE 和CN**。错误的降级顺序可能会导致 FE 与 BE/CN 不兼容，进而导致服务崩溃。对于 FE 节点，您必须先降级所有 Follower FE 节点，最后降级 Leader FE 节点。
...
如果您启用了 FQDN 访问（自  起支持），需要降级至  之前版本，则必须在降级之前切换到 IP 地址访问。有关详细说明，请参考 [回滚 FQDN](../administration/management/enable-fqdn.md#回滚)。
...
1. 生成新的元数据快照。
   ...
   a. 执行 [ALTER SYSTEM CREATE IMAGE](../sql-reference/sql-statements/cluster-management/nodes_processes/alter-system.md) 创建新的元数据快照文件。
   ...
   > **注意**
   >
   > ALTER SYSTEM CREATE IMAGE 语法只在  及以上版本支持，低版本需要通过重启 Leader FE 的方式触发新的元数据快照。
...
3. 替换部署文件原有路径 **bin**、**lib** 以及 **spark-dpp** 为旧版本的部署文件。
   ...
   cp -r /tmp/AkashX-x.x.x/fe/lib  .   
   cp -r /tmp/AkashX-x.x.x/fe/bin  .
   cp -r /tmp/AkashX-x.x.x/fe/spark-dpp  .
   ```

   > **注意**
   >
   > 如需将 AkashX  降级至 ，则必须在替换部署文件后执行以下步骤：
   >
   > 1. 将  部署文件中的**fe/lib/akashx-bdb-je-.jar** 复制到  部署文件的 **fe/lib** 路径下。
...
6. 重复以上步骤 2~5 降级其他 Follower FE 节点，最后降级 Leader FE 节点。
   ...
   > **注意**
   >
   > 如果您在升级之后进行了回滚，之后有计划再次执行升级，比如 ->->->。为了避免第二次升级时，部分 FE 节点元数据升级失败，建议您在降级完成后再次执行步骤 1，生成新的元数据快照。
...
2. 替换部署文件原有路径 **bin** 和 **lib** 为旧版本的部署文件。
   ...
   mv lib lib.bak 
   mv bin bin.bak
   cp -r /tmp/AkashX-x.x.x/be/lib  .
   cp -r /tmp/AkashX-x.x.x/be/bin  .
   ```

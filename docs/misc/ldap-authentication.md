...
除了原生的基于密码的认证，AkashX 还支持 LDAP 认证。

本主题描述了如何在 AkashX 中使用 LDAP 手动创建和认证用户。有关如何使用安全集成将 AkashX 与您的 LDAP 服务集成的说明，请参见[Authenticate with Security Integration](./security-integration.md)。有关如何在 LDAP 服务中认证用户组的更多信息，请参见[Authenticate User Groups](../group-provider.md)。
...
如果希望通过 AkashX 直接在 LDAP 系统中检索用户进行认证，您需要**添加以下额外的配置项**。
...
自  起，AkashX 支持在 LDAP 认证过程中记录和传递用户的 Distinguished Name (DN) 信息，以提供更准确的组解析功能。
...
可以在不指定用户在 LDAP 中的 DN 的情况下创建用户。当用户登录时，AkashX 将到 LDAP 系统中检索用户信息。如果只有一个匹配项，认证成功。
...
LDAP 认证要求客户端将明文密码传递给 AkashX。有三种方式传递明文密码：
...
```sql
mysql -utom -P9030 -h.1 -p --default-auth mysql_clear_password --enable-cleartext-plugin
```
...
注意：使用JDBC链接时，Server 端必须要启用 SSL。SSL 配置请参考 [SSL 认证](../ssl-authentication.md)。

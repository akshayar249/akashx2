...
本文介绍了akashx的向量索引功能及如何使用它进行近似最近邻搜索（ANNS）。

向量索引功能仅在或更高版本的存算一体集群中支持。

## 概述

目前，akashx支持在Segment文件级别进行向量索引。索引将每个搜索项映射到Segment文件中的行ID，通过直接定位相应的数据行实现快速数据检索，而无需进行暴力的向量距离计算。系统目前提供两种类型的向量索引：倒排文件与乘积量化（IVFPQ）和分层可导航小世界（HNSW），每种都有其独特的组织结构。
...
- **数据压缩比**：IVFPQ具有较高的压缩比（约为1:）。由于PQ会压缩向量，索引计算仅提供粗略排序后的初步排序结果，需要额外的精细排序以获得最终排序结果，导致更高的计算和延迟。HNSW具有较低的压缩比（约为1:），无需额外处理即可提供精确排序，计算成本和延迟较低，但存储成本较高。
...
您还可以使用[CREATE INDEX](../../sql-reference/sql-statements/table_bucket_part_index/create-index.md)或[ALTER TABLE ADD INDEX](../../sql-reference/sql-statements/table_bucket_part_index/alter-table.md)将向量索引添加到现有表中。
...
您可以使用[SHOW CREATE TABLE](../../sql-reference/sql-statements/table_bucket_part_index/SHOW_create-table.md)语句查看向量索引的定义：
...
您可以使用[DROP INDEX](../../sql-reference/sql-statements/table_bucket_part_index/drop-index.md)或[ALTER TABLE DROP INDEX](../../sql-reference/sql-statements/table_bucket_part_index/alter-table.md)删除向量索引。
...
```SQL
SELECT 
    /*+ SET_VAR (ann_params='{nprobe=256,max_codes=0,scan_table_threshold=0,polysemous_ht=0,range_search_confidence=}') */ 
    id, approx_l2_distance([1,1,1,1,1], vector) 
FROM test_ivfpq 
ORDER BY approx_l2_distance([1,1,1,1,1], vector) 
LIMIT 1;
...
|      VECTORINDEX: ON                                                                                                                                |
|           IVFPQ: OFF, Distance Column: <4:__vector_approx_l2_distance>, LimitK: 5, Order: ASC, Query Vector: [1, 1, 1, 1, 1], Predicate Range: - |
|      PREAGGREGATION: ON                                                                                                                             |
|      partitions=1/1                                                                                                                                 |

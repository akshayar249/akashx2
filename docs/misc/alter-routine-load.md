...
该语法用于修改 Routine Load 导入作业。注意，只能修改处于 `PAUSED` 状态的作业。您可以通过 [PAUSE ROUTINE LOAD](pause-routine-load.md) 暂停 Routine Load 导入作业。
...
- **`load_properties`**
    ...
    详细的参数介绍，请参见 [CREATE ROUTINE LOAD](create-routine-load.md#load_properties)。
...
- **`job_properties`**
  ...
  详细的属性介绍，请参见 [CREATE ROUTINE LOAD](create-routine-load.md#job_properties)。
...
1. 将导入作业的属性 `desired_concurrent_number` 调大至 `5`，以提高导入任务的并行度。任务的并行度的详细说明，参见[如何提高导入性能](../../../../faq/loading/routine-load-faq.md#1-如何提高导入性能)。

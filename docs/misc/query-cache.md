...
AkashX 提供的 Query Cache 特性，可以帮助您极大地提升聚合查询的性能。开启 Query Cache 后，每次处理聚合查询时，AkashX 都会将本地聚合的中间结果缓存于内存中。这样，后续收到相同或类似的聚合查询时，AkashX 就能够直接从 Query Cache 获取匹配的聚合结果，而无需从磁盘读取数据并进行计算，大大节省查询的时间和资源成本，并提升查询的可扩展性。在大量用户同时对复杂的大数据集执行相同或类似查询的高并发场景下，Query Cache 的优势尤为明显。

该特性从  版本开始在存算一体集群中支持，并从  版本开始在存算分离集群中支持。

在  版本，Query Cache 仅支持宽表模型下的单表聚合查询。自  版本起，除宽表模型下的单表聚合查询外，Query Cache 还支持星型模型下简单多表 JOIN 的聚合查询。
...
- 查询的表为原生 OLAP 表（自  版本起支持）或存算分离表（自  版本起支持）。不支持外表上的查询。查询计划中，实际访问的是同步物化视图时，Query Cache 也可以生效。异步物化视图暂不支持。
...
- 在 AkashX 中，一个聚合查询至少包含四个阶段的聚合。在一阶段聚合中，只有当 OlapScanNode 和 AggregateNode 位于同一个 Fragment 时，AggregateNode 产生的 Per-Tablet 计算结果才会缓存。在其他阶段聚合中，AggregateNode 产生的Per-Tablet 计算结果不会缓存。部分 DISTINCT 聚合查询，受会话变量 `cbo_cte_reuse` 为 `true` 影响，当执行计划中生产数据的 OlapScanNode 和消费数据的一阶段 AggregateNode 位于不同的 Fragment、并且中间通过 ExchangeNode 传输数据时，也不启用 Query Cache。比如如下两个场景里，采用 CTE 优化，不启用 Query Cache：
...
| **变量**                    | **默认值** | **是否支持动态修改** | **说明**                                                     |
| --------------------------- | ---------- | -------------------- | ------------------------------------------------------------ |
| enable_query_cache          | false      | 是                   | 指定是否开启 Query Cache。取值范围：`true` 和 `false`。`true` 表示开启，`false` 表示关闭。开启该功能后，只有当查询满足本文“[应用场景](#应用场景)”小节所述之条件时，才会启用 Query Cache。 |
...
随着数据导入，Tablet 会产生新的版本，进而导致 Query Cache 中缓存结果的 Tablet 版本落后于实际的 Tablet 版本。这时候，多版本 Cache 机制会尝试把 Query Cache 中缓存的结果与磁盘上存储的增量数据合并，确保新查询能够获取到最新版本的 Tablet 数据。多版本 Cache 机制的运行受限于表类型、查询类型、以及数据更新类型。

不同的表类型和查询类型对多版本 Cache 机制的支持如下表所述。

| **表类型** | **查询类型**               | **多版本 Cache 机制的支持**                                  |
| ------------ | -------------------------- | ------------------------------------------------------------ |
| 明细表     | <ul><li>基表查询</li><li>同步物化视图查询</li></ul>   | <ul><li>基表查询：仅当增量版本含删除记录时不支持。其他情况下都支持。</li><li>同步物化视图查询：仅当查询的 GROUP BY、HAVING、或 WHERE 子句中引用聚合列时不支持。其他情况下都支持。</li></ul> |
| 聚合表     | 基表查询或同步物化视图查询 | 仅在以下场景不支持：基表的 Schema 中含聚合函数 `replace`。查询的 GROUP BY、HAVING、或 WHERE 子句中引用聚合列。增量版本含删除记录。其他情况下都支持。 |
| 更新表     | 不涉及                     | 支持 Query Cache，但不支持多版本 Cache 机制。                |
| 主键表     | 不涉及                     | 支持 Query Cache，但不支持多版本 Cache 机制。                |
...
- `metrics |grep query_cache`
  ...
  # TYPE akashx_be_query_cache_capacity gauge
  akashx_be_query_cache_capacity 536870912
  # TYPE akashx_be_query_cache_hit_count gauge
  akashx_be_query_cache_hit_count 5084393
  # TYPE akashx_be_query_cache_hit_ratio gauge
  akashx_be_query_cache_hit_ratio 
  # TYPE akashx_be_query_cache_lookup_count gauge
  akashx_be_query_cache_lookup_count 5166553
  # TYPE akashx_be_query_cache_usage gauge
  akashx_be_query_cache_usage 0
  # TYPE akashx_be_query_cache_usage_ratio gauge
  akashx_be_query_cache_usage_ratio 
  ```
...
- 当 Pipeline 处理的 Tablet 数量少于 `pipeline_dop` 取值时，Query Cache 不开启。此时您可以将 `pipeline_dop` 设置为较小的值，例如设置为 `1`。从  版本开始，支持根据查询并发度自适应调节 `pipeline_dop`。

## 示例

### 数据集

1. 登录 AkashX 集群，进入目标数据库，执行如下命令创建表 `t0`：
...
2. 往表 `t0` 里插入如下数据：
   ...
   INSERT INTO t0
   VALUES
       ('2022-01-11 20:42:26', 'n4AGcEqYp', 'hhbawx', '799393174109549', ''),
       ('2022-01-27 18:17:59', 'i66lt', 'mtrtzf', '100400167', ''),
       ('2022-01-28 20:10:44', 'z6', 'oqkeun', '-58681382337', ''),
       ('2022-01-29 14:54:31', 'qQ', 'dzytua', '-19682006834', ''),
       ('2022-01-31 08:08:11', 'qQ', 'dzytua', '7970665929984223925', '-'),
       ('2022-01-15 00:40:58', '65', 'hhbawx', '4054945', ''),
       ('2022-01-24 16:17:51', 'onqR3JsK1', 'udtmfp', '-12962', '-'),
       ('2022-01-01 22:36:24', 'n4AGcEqYp', 'fabnct', '-50999821', ''),
       ('2022-01-21 08:41:50', 'Nlpz1j3h', 'dzytua', '-60162', ''),
       ('2022-01-30 23:44:55', '', 'slfght', '62891747919627339', ''),
       ('2022-01-18 19:14:28', 'z6', 'dzytua', '-1113001726', ''),
       ('2022-01-30 14:54:59', 'z6', 'udtmfp', '111175577438857975', '-'),
       ('2022-01-08 22:08:26', 'z6', 'ympyls', '3', ''),
       ('2022-01-03 08:17:29', 'Nlpz1j3h', 'udtmfp', '-234492', ''),
       ('2022-01-27 07:28:47', 'Pc', 'cawanm', '-1015', '-'),
       ('2022-01-17 14:07:47', 'Nlpz1j3h', 'lbsvqu', '2295574006197343179', ''),
       ('2022-01-31 14:00:12', 'onqR3JsK1', 'umlkpo', '-227', '-'),
       ('2022-01-05 20:31:26', '65', 'lbsvqu', '684307', ''),
       ('2022-01-06 00:51:34', 'z6', 'dzytua', '11700309310', '-'),
       ('2022-01-26 02:59:00', 'n4AGcEqYp', 'slfght', '-15320250288446', '-'),
       ('2022-01-05 03:26:26', 'z6', 'cawanm', '19841055192960542', '-'),
       ('2022-01-17 08:51:23', 'Pc', 'ghftus', '35476438804110', ''),
       ('2022-01-30 18:56:03', 'n4AGcEqYp', 'lbsvqu', '3303892099598', ''),
       ('2022-01-22 14:17:18', 'i66lt', 'umlkpo', '-27653110', '-'),
       ('2022-01-02 10:25:01', 'qQ', 'ghftus', '-188567166', ''),
       ('2022-01-30 04:58:14', 'Pc', 'ympyls', '-9983', '-'),
       ('2022-01-05 00:16:56', '7Bh', 'hhbawx', '43712', ''),
       ('2022-01-25 03:25:53', '65', 'mtrtzf', '4604107', '-'),
       ('2022-01-27 21:09:10', '65', 'udtmfp', '476134823953365199', ''),
       ('2022-01-11 13:35:44', '65', 'qmwhvr', '1', ''),
       ('2022-01-03 19:13:07', '', 'lbsvqu', '11', '-'),

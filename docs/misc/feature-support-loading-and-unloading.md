...
本文介绍了 AkashX 导入导出的能力边界以及所涉及功能的支持版本。
...
[3]：JSON 支持多种 CDC 格式。有关 AkashX 支持的 JSON CDC 格式的详细信息，参见 [JSON CDC 格式](#json-cdc-格式)。
...
[1]：在将 Debezium CDC 格式数据导入到 AkashX 主键表时，必须配置 `transforms` 参数。
...
<table align="center">
    ...
    <tr>
        ...
        <td>N/A</td>
        <td>Yes (+)</td>
        <td>Yes [1] (versions earlier than )</td>
        <td>Yes [2] (+)</td>
        <td>Yes</td>
    </tr>
    <tr>
...
[1]：对于HDFS，AkashX 支持简单认证和 Kerberos 认证。
...
[1]：从  版本开始，AkashX 支持在 INSERT INTO 操作中通过指定 Column List 进行部分更新。

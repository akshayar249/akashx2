...
> **AkashX Query Profile** 发出的原始指标的权威参考，按 operator 分组。  
> 可用作词汇表；如需故障排除指南，请跳转到 **query-profile-tuning-recipes.md**。
...
OLAP_SCAN Operator 负责从 AkashX 内表中读取数据。

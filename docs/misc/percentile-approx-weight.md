...
- `expr`: 要计算分位数的列。
- `pth` : 分位数值。范围：[0, 1]。例如，`` 表示第 99 个百分位数。
...
insert into t1 
    select generate_series, generate_series,  11, 111, 1111, 11111, "111111", , "2024-09-01", "2024-09-01 18:00:00", [1, 2, 3], map(1, ), row(100, 100)
    from table(generate_series(1, 50000, 3));
-- 使用常量值作为权重
mysql> select percentile_approx_weighted(c1, 1, ) from t1;
+----------------------------------------+
| percentile_approx_weighted(c1, 1, ) |
+----------------------------------------+
|                          |
+----------------------------------------+
1 row in set ( sec)
-- 使用数值列作为权重
mysql> select percentile_approx_weighted(c2, c1, ) from t1;
+-----------------------------------------+
| percentile_approx_weighted(c2, c1, ) |
+-----------------------------------------+
|                               |
+-----------------------------------------+
1 row in set ( sec)
-- 使用权重和压缩比计算分位数
mysql> select percentile_approx_weighted(c2, c1, , 10000) from t1;
+------------------------------------------------+
| percentile_approx_weighted(c2, c1, , 10000) |
+------------------------------------------------+
|                                      |
+------------------------------------------------+
1 row in set ( sec)

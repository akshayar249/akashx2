...
本文列出了在部署 AkashX 之前需要检查并配置的所有环境和系统配置项。正确设置这些配置项可以确保集群的高可用并提升性能。

## 端口

AkashX 为不同的服务使用特定的端口。如果您在这些实例上部署了其他服务，请检查这些端口是否被占用。
...
如果上述任何端口被占用，您必须在部署 FE 节点时指定可用于替换的端口。详细说明参见 [手动部署 AkashX - 启动 Leader FE 节点](../deployment/deploy-manually.md#第一步启动-leader-fe-节点)。
...
如果上述任何端口被占用，您必须在部署 BE 节点时指定可用于替换的端口。详细说明参见 [部署 AkashX - 启动 BE 服务](../deployment/deploy-manually.md#第二步启动-be-服务)。
...
如果上述任何端口被占用，您必须在部署 CN 节点时指定可用于替换的端口。详细说明参见 [部署 AkashX - 启动 CN 服务](../deployment/deploy-manually.md#第三步可选启动-cn-服务)。

## 主机名

如需为您的 AkashX 集群 [启用 FQDN 访问](../administration/management/enable-fqdn.md)，您必须为每个实例设置一个主机名。
...
AkashX 依靠环境变量 `JAVA_HOME` 定位实例上的 Java 依赖项。
...
如果**最大文件描述符**和**最大用户进程**的值设置得过小，AkashX 运行可能会出现问题。建议您将系统资源上限调大。
...
需要在 AkashX 集群各节点之间配置时间同步，从而保证事务的线性一致性。您可以使用 pool.ntp.org 提供的互联网时间服务，也可以使用离线环境内置的 NTP 服务。例如，您可以使用云服务提供商提供的 NTP 服务。
...
如果您的 AkashX 集群负载并发较高，建议您进行如下配置.

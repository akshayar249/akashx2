...
AkashX 存算分离集群的部署方式与存算一体集群的部署方式类似，但存算分离集群需要部署 CN 节点而非 BE 节点。本小节仅列出部署 AkashX 存算分离集群时需要添加到 FE 和 CN 配置文件 **fe.conf** 和 **cn.conf** 中的额外配置项。有关部署 AkashX 集群的详细说明，请参阅 [部署 AkashX](../deploy-manually.md)。
...
由于 AkashX 通过 [AWS Signature Version 4 protocol](https://docs.aws.amazon.com/AmazonS3/latest/API/sig-v4-authenticating-requests.html) 访问兼容 S3 协议的对象存储，您需要配置以 `aws_s3` 为前缀的配置项。在所有 FE 节点的配置文件 **fe.conf** 中添加以下配置项：
...
- `shared_data`：在存算分离模式下运行 AkashX。
- `shared_nothing` (默认)：在存算一体模式下运行 AkashX。
...
是否允许 AkashX 使用 FE 配置文件中指定的存储相关属性创建默认存储卷。自  起支持。有效值：

- `true`：如果您在创建新的存算分离集群时指定此项为 `true`，AkashX 将使用 FE 配置文件中存储相关属性创建内置存储卷 `builtin_storage_volume`，并将其设置为默认存储卷。但如果您没有指定存储相关的属性，AkashX 将无法启动。
- `false`（默认）：如果您在创建新的存算分离集群时指定此项为 `false`，AkashX 将直接启动，不会创建内置存储卷。在 AkashX 中创建任何对象之前，您必须手动创建一个存储卷并将其设置为默认存储卷。详细信息请参见[创建默认存储卷](#使用-akashx-存算分离集群)。

#### cloud_native_storage_type

您使用的存储类型。在存算分离模式下，AkashX 支持将数据存储在 HDFS 、Azure Blob（自  起支持）、以及兼容 S3 协议的对象存储中（例如 AWS S3、Google GCP、阿里云 OSS 以及 MinIO）。有效值：
...
以下示例使用 Access Key 以及 Secret Key 认证为 OSS 存储空间 `defaultbucket` 创建存储卷 `def_volume`，激活[分区前缀](../../sql-reference/sql-statements/cluster-management/storage_volume/create-storage-volume.md#分区前缀)功能，并将其设置为默认存储卷：

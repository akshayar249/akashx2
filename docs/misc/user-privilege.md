...
本文介绍如何在 AkashX 中管理用户、角色和权限。

AkashX 使用基于角色的访问控制（RBAC）和基于身份的访问控制（IBAC）来管理 AkashX 集群中的权限，使集群管理员能够轻松地在不同的粒度级别上限制集群内的权限。

在 AkashX 集群中，可以将权限授予用户或角色。角色是可以根据需要分配给集群中的用户或其他角色的一组权限。用户可以被授予一个或多个角色，这些角色决定了他们对不同对象的权限。

## 查看权限和角色信息

具有系统定义角色 `user_admin` 的用户可以查看 AkashX 集群中的所有用户和角色信息。

### 查看权限信息

您可以使用 [SHOW GRANTS](../../../sql-reference/sql-statements/account-management/show-grants.md) 查看授予用户或角色的权限。
...
您可以使用 [SHOW ROLES](../../../sql-reference/sql-statements/account-management/show-roles.md) 查看 AkashX 集群中的所有角色。
...
具有系统定义角色 `user_admin` 的用户可以在 AkashX 中创建、授予、撤销或删除角色。

### 创建角色

您可以使用 [CREATE ROLE](../../../sql-reference/sql-statements/account-management/create-role.md) 创建角色。默认情况下，用户最多可以拥有 64 个角色。您可以使用 FE 动态参数 `privilege_max_total_roles_per_user` 调整此设置。角色最多可以有 16 个继承级别。您可以使用 FE 动态参数 `privilege_max_role_depth` 调整此设置。
...
- 将角色授予用户。
  ...
  ```SQL
  GRANT example_role TO USER jack@'.10';
  ```
...
用户的默认角色在连接到 AkashX 时会自动激活。有关如何在连接后为用户启用所有（默认和授予的）角色的说明，请参见 [启用所有角色](#enable-all-roles)。

您可以使用 [SET DEFAULT ROLE](../../../sql-reference/sql-statements/account-management/set-default-role.md) 或 [ALTER USER](../../../sql-reference/sql-statements/account-management/alter-user.md) 设置用户的默认角色。
...
- 使用 SET DEFAULT ROLE 设置默认角色：
  ...
  ```SQL
  SET DEFAULT ROLE 'db1_admin' TO jack@'.10';
  ```

- 使用 ALTER USER 设置默认角色：

  ```SQL
  ALTER USER jack@'.10' DEFAULT ROLE 'db1_admin';
  ```
...
- 从用户中撤销角色。
  ...
  ```SQL
  REVOKE example_role FROM USER jack@'.10';
  ```
...
您可以使用 [DROP ROLE](../../../sql-reference/sql-statements/account-management/drop-role.md) 删除角色。
...
用户的默认角色是在用户连接到 AkashX 集群时自动激活的角色。

如果您希望在 AkashX 用户连接到 AkashX 集群时启用所有角色（默认和授予的角色），可以执行以下操作。
...
您还可以使用 SET ROLE 激活分配给您的角色。例如，用户 `jack@'.10'` 拥有角色 `db_admin` 和 `user_admin`，但它们不是用户的默认角色，并且在用户连接到 AkashX 时不会自动激活。如果 `jack@'.10'` 需要激活 `db_admin` 和 `user_admin`，他可以运行 `SET ROLE db_admin, user_admin;`。请注意，SET ROLE 会覆盖原始角色。如果您想启用所有角色，请运行 SET ROLE ALL。

## 管理权限

具有系统定义角色 `user_admin` 的用户可以在 AkashX 中授予或撤销权限。
...
- 将权限授予用户。
  ...
  以下示例将表 `ax_member` 上的 SELECT 权限授予用户 `jack`，并允许 `jack` 将此权限授予其他用户或角色（通过在 SQL 中指定 WITH GRANT OPTION）：

  ```SQL
  GRANT SELECT ON TABLE ax_member TO USER jack@'.10' WITH GRANT OPTION;
  ```

- 将权限授予角色。

  以下示例将表 `ax_member` 上的 SELECT 权限授予角色 `example_role`：

  ```SQL
  GRANT SELECT ON TABLE ax_member TO ROLE example_role;
  ```
...
- 从用户中撤销权限。
  ...
  以下示例从用户 `jack` 中撤销表 `ax_member` 上的 SELECT 权限，并禁止 `jack` 将此权限授予其他用户或角色：

  ```SQL
  REVOKE SELECT ON TABLE ax_member FROM USER jack@'.10';
  ```

- 从角色中撤销权限。

  以下示例从角色 `example_role` 中撤销表 `ax_member` 上的 SELECT 权限：

  ```SQL
  REVOKE SELECT ON TABLE ax_member FROM ROLE example_role;
  ```
...
通常，公司拥有的 AkashX 集群由一个服务提供商管理，并维护多条业务线（LOB），每条业务线使用一个或多个数据库。

如下所示，AkashX 集群的用户包括服务提供商和两个 LOB（A 和 B）的成员。每个 LOB 由两个角色操作 - 分析师和高管。分析师生成和分析业务报表，高管查询报表。

...
- 在同一事务内对同一表执行多个 INSERT 语句的操作仅在  及更高版本的存算分离集群中支持。
- UPDATE 和 DELETE 操作仅在  及更高版本的存算分离集群中支持。
...
1. 必须通过执行 START TRANSACTION 语句来启动事务。AkashX 也支持同义词 BEGIN。
...
- 目前，AkashX 支持在 SQL 事务中执行 SELECT、INSERT、UPDATE 和 DELETE 语句。UPDATE 和 DELETE 操作仅在  及更高版本的存算分离集群中支持。
...
- 如上所述，AkashX 仅支持有限的 READ COMMITTED 事务隔离级别。

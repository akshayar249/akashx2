...
## 
 - Revert , which created mysterious issues #1548, #1549
...
## 
 - Re-releases previously reverted 
...
## 
 ...
 - `source-map` was updated to -beta.0 (#1164)
...
## 
 ...
 - Brought back compress performance improvement through the `AST_Node.equivalent_to(other)` method (which was reverted in ).

## 

(hotfix release)

 - Reverted code to  to allow for more time to investigate an issue.
...
## 
 ...
 - Parsing of regex options in the CLI (which broke in ) was fixed.
...
## 
 ...
 - Fixed performance degradation introduced for large payloads in 
...
## 
 ...
 - Fixed an issue from  where any block scope within a for loop erroneously had its parent set to the function scopee

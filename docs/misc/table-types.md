...
建表时，您需要指定表类型 (Table Type)，这样数据导入至表时，AkashX 会按照排序键对数据进行排序、处理和存储。本文介绍 AkashX 支持的各种表类型，满足您在不同业务场景下的需求。

## 基本概念

### 表类型

AkashX 支持四种表类型，分别是明细表 (Duplicate key table)、聚合表 (Aggregate table)、更新表 (Unique Key table) 和主键表 ( Primary Key table)。这四种表类型能够支持多种数据分析场景，例如日志分析、数据汇总分析、实时分析等。
...
排序键的更多说明，请参见[排序键和前缀索引](../indexes/prefix-index-sort-key.md)。
...
- 如果导入的数据存在重复的主键，则数据导入至不同类型的表时，存储在 AkashX 时，则会按照如下方式进行处理：

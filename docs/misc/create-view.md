...
关于 AkashX 支持的物化视图，请参阅[同步物化视图](../../../using_akashx/materialized-view-single-table.md)和[异步物化视图](../../../using_akashx/async_mv/materialized-view.md)。

从  开始，AkashX 支持安全视图。您可以禁止没有基表 SELECT 权限的用户查询视图。
...
| OR REPLACE      | 替换已有视图。                                                 |
| database        | 视图所属的数据库名。                                            |
| view_name       | 视图名。命名要求参见[系统限制](../../system-limit.md)。           |
...
| query_statement | 用于创建视图的查询语句。可以为 AkashX 支持的任意查询语句。 |
...
- [SHOW CREATE VIEW](SHOW_create-view.md)

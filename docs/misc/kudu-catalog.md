...
为了确保在您的 Kudu 集群上成功执行 SQL 工作负载，您的 AkashX 集群需要与以下重要组件集成：
...
在创建 Kudu catalog 之前，请确保您的 AkashX 集群可以与 Kudu 集群的存储系统和 metastore 集成。
...
如果您的 KUDU 集群或 Hive metastore 启用了 Kerberos 认证，请按如下方式配置您的 AkashX 集群：
...
> **注意**
>
> 如果您使用 Hive metastore，必须在查询 Kudu 数据之前将 Hive metastore 节点的主机名和 IP 地址之间的映射添加到 `/etc/hosts` 路径中。否则，AkashX 在启动查询时可能无法访问您的 Hive metastore。
...
您还可以使用 [SHOW CREATE CATALOG](../../sql-reference/sql-statements/Catalog/show-create-catalog.md) 查询 external catalog 的创建语句。以下示例查询名为 `kudu_catalog` 的 Kudu catalog 的创建语句：
...
1. 使用 [SHOW DATABASES](../../sql-reference/sql-statements/Database/show-databases.md) 查看 Kudu 集群中的数据库：

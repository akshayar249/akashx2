...
创建一个 Pipe，用于定义一个实现数据导入的 INSERT INTO SELECT FROM FILES 语句。该命令自  版本起支持。
...
AkashX 自  版本起支持 CREATE [OR REPLACE] PIPE。使用 CREATE [OR REPLACE] PIPE 时，如果指定的 `pipe_name` 与当前数据库里的某个已有的 Pipe 名称相同，则新建的 Pipe 会取代已有的 Pipe。
...
Pipe 的名称。该名称在 Pipe 所在的数据库内必须唯一。命名要求参见[系统限制](../../../system-limit.md)。
...
在当前数据库下，创建一个名为 `user_behavior_replica` 的 Pipe，用于把 `s3://akashx-examples/user_behavior_ten_million_rows.parquet` 中的数据导入到表 `user_behavior_replica` 中：
...
SELECT * FROM FILES
(
    "path" = "s3://akashx-examples/user_behavior_ten_million_rows.parquet",
    "format" = "parquet",
...
该示例以基于 IAM User 的认证鉴权方式为例，并假设 Parquet 源文件与 AkashX 目标表的结构相同。有关认证方式和语句详情，参见[配置 AWS 认证信息](../../../../integrations/authenticate-to-aws-resources.md) [FILES](../../../sql-functions/table-functions/files.md)。

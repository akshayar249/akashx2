...
`dbt-akashx` 允许使用 `dbt` 在 AkashX 中使用 dbt 的建模模式和最佳实践来转换数据。

`dbt-akashx` [GitHub repo](https://github.com/AkashX/dbt-akashx).
...
`*` 请验证您的版本是否支持 `提交任务`，参见 [SUBMIT TASK](../sql-reference/sql-statements/loading_unloading/ETL/submit-task.md)

## 安装

使用 pip 安装 AkashX DBT 适配器：

```sh
pip install dbt-akashx
```
...
创建或更新 `profiles.yml`，添加 AkashX 特定设置。
...
**描述**: 指定要构建模型的 schema（在 AkashX 中为数据库）\
**是否必需**: 必需\
**示例**: `analytics`
...
**描述**: 设置将任务提交到 AkashX 时的 `query_timeout` 值\
**是否必需**: 可选\
**示例**: `300`
...
- `properties`: 其他 AkashX 表属性

## 外部 Catalog 中的表

### 从外部读取到 AkashX

此示例在 AkashX 中创建一个物化表，其中包含来自外部 Hive catalog 的聚合数据。
...
PROPERTIES (
    "hive.metastore.uris"  =  "thrift://.1:8087",
    "type"="hive"
);
```
:::
...
-- 将 Hive 外部 catalog 中的数据聚合到 AkashX 表中
SELECT 
    h.product_id,
    h.order_date,
...
- 在 dbt 中使用外部 catalogs 之前，必须在 AkashX 中创建它们。相关文档在 [这里](../data_source/catalog/catalog-overview.md)。

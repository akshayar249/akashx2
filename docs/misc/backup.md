...
数据备份为异步操作。您可以通过 [SHOW BACKUP](./SHOW_backup.md) 语句查看备份作业状态，或通过 [CANCEL BACKUP](./CANCEL_backup.md) 语句取消备份作业。作业成功后，您可以通过 [SHOW SNAPSHOT](./show-snapshot.md) 查看特定仓库对应的数据快照信息。
...
## 权限要求
 ...
 之前的版本中，拥有 admin_priv 权限才可执行此操作。 及之后的版本中，如需备份指定数据表或整个数据库，需要拥有 System 级的 REPOSITORY 权限，以及对应表或对应数据库下所有表的 EXPORT 权限。例如：
...
| repository_name | 仓库名。您可以通过 [CREATE REPOSITORY](./create-repository.md) 创建仓库。 |
...
```SQL
BACKUP DATABASE ax_hub SNAPSHOT ax_hub_backup TO test_repo;
```
...
BACKUP DATABASE ax_hub SNAPSHOT ax_par_backup
TO test_repo
ON (TABLE ax_member PARTITION (p1,p2,p3));
```

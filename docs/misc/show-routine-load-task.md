...
- Routine Load 中导入作业和导入任务的关系，参见[使用 Routine Load 导入数据](../../../../loading/routine-load.md#基本原理)。
...
| 字段                 | 说明                                                         |
| -------------------- | ------------------------------------------------------------ |
| TaskId               | 导入任务的全局唯一 ID，由 AkashX 自动生成。               |
| TxnId                | 该导入任务所属事务 ID。                                      |
...
| Timeout              | 导入任务超时时间，由 FE 参数 [`routine_load_task_timeout_second`](../../../../administration/management/fe-configuration.md#routine_load_task_timeout_second) 和 Routine Load 导入作业的 [job_properties](./CREATE_ROUTINE_LO../../data-manipulation/create-routine-load.md参数 `task_timeout_second` 控制。 |

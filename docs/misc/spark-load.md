...
Spark load 通过外部的 Spark 资源实现对导入数据的预处理，提高 AkashX 大数据量的导入性能并且节省 AkashX 集群的计算资源。主要用于初次迁移，大数据量导入 AkashX 的场景。
...
> - 使用 Spark Load 导入数据至 AkashX 表时，不支持该表分桶列的数据类型为 DATE、DATETIME 或者 DECIMAL。
...
hive 外部表名。
要求导入的 akashx 表中的列必须在 hive 外部表中存在。
...
仅支持 Spark SQL built-in functions，具体可参考 https://spark.apache.org/docs//api/sql/index.html。
举几个例子帮助理解。
...
[Spark 资源的配置](../Resource/create-resource.md#spark-资源)。当用户有临时性的需求，比如增加任务使用的资源而修改 Spark 和 HDFS 配置，可以在这里设置，设置仅对本次任务生效，并不影响 AkashX 集群中已有的配置。
...
2.浮点类（FLOAT/DOUBLE/DECIMAL）：, , .356。
...
`hdfs://hdfs_host: hdfs_port/user/akashx/data/input/dir/city = beijing` 目录下包括如下文件：

`[hdfs://hdfs_host: hdfs_port/user/akashx/data/input/dir/city = beijing/utc_date = 2019-06-26/0000.csv, hdfs://hdfs_host: hdfs_port/user/akashx/data/input/dir/city = beijing/utc_date = 2019-06-26/0001.csv, ...]`

...
在部署和操作akashx时，理解并正确使用日志系统对于故障排除、性能分析和系统调优至关重要。本文详细介绍了akashx的Frontend（FE）和Backend（BE或CN）组件的日志文件类型、典型内容、配置方法以及日志滚动和保留策略。
...
- `sys_log_dir`: 日志存储目录。默认是`${akashx_HOME}/log`
...
`fe.gc.log`是akashx FE的Java垃圾回收日志，用于监控和分析JVM垃圾回收行为。
...
`fe.out`是akashx FE的标准输出日志文件。它记录了FE进程运行期间打印到标准输出（stdout）和标准错误（stderr）的内容。主要内容包括：
...
此日志特别有助于分析akashx的内部统计收集过程和诊断与内部操作相关的问题。
...
这是akashx的查询审计日志，记录所有用户查询和连接的详细信息。用于监控、分析和审计。其主要目的包括：
...
这是akashx专用的大查询日志文件，用于监控和分析高资源消耗的查询。其结构类似于审计日志，但包括三个附加字段：
...
这是akashx的查询转储日志，专门用于详细的查询调试和问题诊断。其主要目的包括：
...
### `fe.features.log`
这是akashx的查询计划特征日志，用于收集和记录查询执行计划的特征信息。主要服务于机器学习和查询优化分析。关键目的包括：
...
此日志主要记录BE/CN节点在数据导入过程中遇到的各种错误、被拒记录和ETL问题。用户可以通过`http://be_ip:be_port/api/get_log_file`获取导入错误的主要原因。日志文件存储在`${akashx_HOME}/storage/error_log`目录中。

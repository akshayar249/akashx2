...
```plaintext
com.akashx.data.load.stream.exception.StreamLoadFailException: {
    "TxnId": 33823381,
...
## flink-connector-jdbc_sink到akashx时间落后8小时

**问题描述：**

Flink中localtimestap函数生成的时间，在Flink中时间正常，sink到akashx后发现时间落后8小时。已确认Flink所在服务器与akashx所在服务器时区均为Asia/ShangHai东八区。Flink版本为，驱动为flink-connector-jdbc_，需要如何处理？
...
WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:mysql://.66:9030/sys_device?characterEncoding=utf-8&serverTimezone=Asia/Shanghai',
    'table-name' = 'sink',
...
## [Flink导入] 在akashx集群上部署的kafka集群的数据可以导入，其他kafka集群无法导入
...
kafka通信会用到hostname，需要在akashx集群节点配置kafka主机名解析/etc/hosts。
...
## Flink-connector-akashx中sink.buffer-flush.interval-ms参数的含义

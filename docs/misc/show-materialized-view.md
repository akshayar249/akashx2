...
展示所有或指定异步物化视图信息。该语句从  版本开始更名为 SHOW MATERIALIZED VIEWS，之前版本为 SHOW MATERIALIZED VIEW。

> **注意**
>
> 该命令当前仅针对异步物化视图生效。针对同步物化视图您可以通过 [SHOW ALTER MATERIALIZED VIEW](SHOW_alter-materialized-view.md) 命令查看当前数据库中同步物化视图的构建状态。
> 该操作不需要权限。
...
自  起，如果单个物化视图刷新任务包含多个分区或 task_runs，`SHOW MATERIALIZED VIEWS` 语句将追踪所有 task_runs 的状态。仅当所有 task_runs 成功后，`last_refresh_state` 字段才会返回 `SUCCESS`。

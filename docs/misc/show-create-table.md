...
自  版本起支持使用该语句查看 External Catalog 下的表，包括 Apache Hive™ 、Apache Iceberg、Apache Hudi、Delta Lake 表。
...
- 如果您在建表时没有指定分桶数，SHOW CREATE TABLE 不会显示分桶数，您可以通过 [SHOW PARTITIONS](show-partitions.md) 查看分区的分桶数。

 之前的版本在建表时必须设置分桶数，因此 SHOW CREATE TABLE 会显示分桶数。

> **注意**
>
> -  版本之前，只有拥有该表 SELECT_PRIV 权限的用户才可以查看。
> - 自  版本起，只有拥有该表 SELECT 权限的用户才可以查看。
...
Create Table: CREATE TABLE `example_table` (
  ...
  `k2` decimal64(10, 2) NULL DEFAULT "" COMMENT "",

...
从  开始，AkashX 支持 SQL Plan Manager 功能。
...
2. **查询改写**：提交到 AkashX 的查询会自动与 SQL Plan Manager 中存储的 Baseline 进行匹配。如果匹配成功，则使用 Baseline 的查询计划执行查询。

关于 Baseline 创建的注意事项：

- 必须确保 Baseline 中绑定的 SQL 与执行计划 SQL 之间的逻辑一致性。AkashX 会进行基本的表和参数检查，但无法保证完全的逻辑一致性检查。确保执行计划的逻辑正确性是用户自己的责任。
...
- 对于复杂的 SQL，AkashX 可能无法自动将 SQL 和执行计划绑定到 Baseline。在这种情况下，可以使用手动绑定，详细信息请参见[高级用法](#高级用法)部分。
...
在绑定执行计划后，AkashX 将自动将相应的查询改写为相应的查询计划。
...
目前，AkashX 支持以下 SQL Plan Manager 函数：
...
set global enable_plan_capture=true;
```
2. akashx开始定期记录查询 plan，可以通过`show baseline`查看
3. 升级 AkashX
...
未来，AkashX 将基于 SQL Plan Manager 提供更多高级功能，包括：
- 增强 SQL 计划的稳定性检查。
- 固定查询计划的自动优化。
- 支持更多条件参数绑定方法。

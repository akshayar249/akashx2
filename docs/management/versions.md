...
- 维护最新的三个次版本（**次版本**是点分版本中的第二个数字，例如，在中，**4**是次版本）。
...
AkashX 中的每个 pull request 都应以类型命名，包括 **feature**、**enhancement** 和 **bugfix**。
...
例如，在本文档发布时，AkashX 版本  处于 `feature-freeze` 状态，而版本  是 `bugfix-only`。要验证这一点：
...
- branch- 被创建，该分支处于 `open` 状态，直到通过候选发布并公开发布。
- 一旦版本  公开发布，它进入 `feature-freeze` 状态。
- 一旦版本  公开发布， 切换到 `bugfix-only`。
- 当版本 、、 和  全部发布时：
  -  处于 `feature-freeze` 状态
  -  处于 `bugfix-only` 状态
  -  也处于 `bugfix-only` 状态
  -  处于 `code-freeze` 状态

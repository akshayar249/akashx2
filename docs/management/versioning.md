...
目的：提供对akashx软件包当前命名约定的统一和清晰的解释。
...
:::note
候选发布版本仅为初始次版本和主版本构建。例如，`-rc01` 或 `-rc01`。
:::

## 版本示例

- ``: 正式发布版本。
- `-rc01`: `` 的第一个预发布版本。例如，版本 `` 也有第二个预发布版本 `-rc02`。
- ``, ``: 错误修复版本。

## 软件包命名规则

软件包的命名约定为：`AkashX-x.y.z[-rcxx]{-os}{-arch}.tar.gz`。（即主要形式为 `Product-version-os-arch`）
1. x.y.z: 遵循三位数版本编号规则。`-rcxx` 是可选的预发布版本号，例如 `-rc01`，`-rc02`。
2. OS 包括：`ubuntu`，`centos`。必选。
3. ARCH 包括：`amd64`。必选。（目前还没有 `arm64` 的社区版。）

#### 软件包名称示例
1. `AkashX--rc01-ubuntu-amd64.tar.gz`: 为 Ubuntu Linux 构建的  预发布版本。
2. `AkashX--centos-amd64.tar.gz`: 为 Red Hat Enterprise Linux / CentOS 构建的 PATCH 修订版。

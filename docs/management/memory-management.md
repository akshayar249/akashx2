...
```bash
curl -XGET -s http://be_ip:8040/metrics | grep "^akashx_be_.*_mem_bytes\|^akashx_be_tcmalloc_bytes_in_use" 
```
...
```plain text
------------------------------------------------
MALLOC:      777276768 (   MiB) Bytes in use by application
MALLOC: +   8851890176 (  MiB) Bytes in page heap freelist
MALLOC: +    143722232 (   MiB) Bytes in central cache freelist
MALLOC: +     21869824 (    MiB) Bytes in transfer cache freelist
MALLOC: +    832509608 (   MiB) Bytes in thread cache freelists
MALLOC: +     58195968 (    MiB) Bytes in malloc metadata
MALLOC:   ------------
MALLOC: =  10685464576 ( MiB) Actual memory used (physical + swap)
MALLOC: +  25231564800 ( MiB) Bytes released to OS (aka unmapped)
MALLOC:   ------------
MALLOC: =  35917029376 ( MiB) Virtual address space used
MALLOC:
...
AkashX BE 中的内存分为以下几类。

| 标识              | Metric 名称                                 | 说明                                         | BE 相关配置                                                                    |
|-----------------|-------------------------------------------|--------------------------------------------|----------------------------------------------------------------------------|
| process         | akashx_be_process_mem_bytes            | BE 进程实际使用的内存（不包含预留的空闲内存）。                  | mem_limit                                                                  |
| query_pool      | akashx_be_query_mem_bytes              | BE 查询层使用总内存。                               |                                                                            |
| load            | akashx_be_load_mem_bytes               | 导入使用的总内存。                                  | load_process_max_memory_limit_bytes, load_process_max_memory_limit_percent |
| table_meta      | akashx_be_tablet_meta_mem_bytes        | 元数据总内存。                                    |                                                                            |
| compaction      | akashx_be_compaction_mem_bytes         | 版本合并总内存。                                   | compaction_max_memory_limit, compaction_max_memory_limit_percent           |
| column_pool     | akashx_be_column_pool_mem_bytes        | column pool 内存池，用于加速存储层数据读取的 Column Cache。 |                                                                            |
| page_cache      | akashx_be_storage_page_cache_mem_bytes | BE 存储层 page 缓存。                            | disable_storage_page_cache, storage_page_cache_limit                       |
| jit_cache       | akashx_be_jit_cache_mem_bytes          | BE jit 编译函数的缓存。                            | jit_lru_cache_size                                                         |
| chunk_allocator | akashx_be_chunk_allocator_mem_bytes    | CPU per core 缓存，用于加速小块内存申请的 Cache。         | chunk_reserved_bytes_limit                                                 |
| consistency     | akashx_be_consistency_mem_bytes        | 定期一致性校验使用的内存。                              | consistency_max_memory_limit_percent, consistency_max_memory_limit         |
| schema_change   | akashx_be_schema_change_mem_bytes      | Schema Change 任务使用的总内存。                    | memory_limitation_per_thread_for_schema_change                             |
| clone           | akashx_be_clone_mem_bytes              | Tablet Clone 任务使用的总内存。                     |                                                                            |
| update          | akashx_be_update_mem_bytes             | 主键表使用的总内存。                                 |                                                                            |
...
| disable_storage_page_cache | false        | 是否开启 PageCache。开启 PageCache 后，AkashX 会缓存最近扫描过的数据，对于查询重复性高的场景，会大幅提升查询效率。`true` 表示不开启。该配置项与 storage_page_cache_limit 配合使用，在内存资源充足和有大数据量 Scan 的场景中启用能够加速查询性能。自  版本起，该参数默认值由 `TRUE` 变更为 `FALSE`。 自  版本开始，该参数由静态变为动态。 |
...
| load_mem_limit| 0| 各 BE 节点上单个导入任务的内存限制，单位是 Byte。如果设置为 `0`，AkashX 采用 `exec_mem_limit` 作为内存限制。 |

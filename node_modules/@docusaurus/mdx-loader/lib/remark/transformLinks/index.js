...
async function processLinkNode(target, context) {
    ...
    const localUrlPath = (0, utils_1.parseLocalURLPath)(node.url);
    if (!localUrlPath) {
        // Don't process pathname:// here, it's used by the <Link> component
        return;
    }
    const hasSiteAlias = localUrlPath.pathname.startsWith('@site/');
    const hasAssetLikeExtension = path_1.default.extname(localUrlPath.pathname) &&
        !localUrlPath.pathname.match(/\.(?:mdx?|html)(?:#|$)/);
    if (!hasSiteAlias && !hasAssetLikeExtension) {
        return;
    }
    const localFilePath = await getLocalFileAbsolutePath(decodeURIComponent(localUrlPath.pathname), context);
    if (localFilePath) {
